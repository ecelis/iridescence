...
public class FinStatement extends SvrProcess
{
	...
	private void createBalanceLine() {
		StringBuffer sb = new StringBuffer ("INSERT INTO T_ReportStatement ");
			sb.append("(AD_PInstance_ID, Fact_Acct_ID, LevelNo,");
			sb.append("DateAcct, Name, Description,");
			sb.append("AmtAcctDr, AmtAcctCr, Balance, Qty) ");
		sb.append("SELECT ").append(getAD_PInstance_ID()).append(",0,0,")
			...
			sb.append("COALESCE(SUM(AmtAcctDr),0), COALESCE(SUM(AmtAcctCr),0), COALESCE(SUM(AmtAcctDr-AmtAcctCr),0), COALESCE(SUM(Qty),0) ");
			sb.append("FROM Fact_Acct ");
			sb.append("WHERE ").append(m_parameterWhere);
			sb.append(" AND ");
			
			if (DB.isOracle()) {
				sb.append("TRUNC(DateAcct, 'DD')");
			} else if (DB.isPostgreSQL()) {
				sb.append("DATE_TRUNC('day', DateAcct)");
			}
			
			sb.append(" < ").append(DB.TO_DATE(p_DateAcct_From));
		...
		if (p_Account_ID > 0) {
			...
			if (!m_acct.isBalanceSheet()) {
				...
				if (first != null) {
					...
					sb.append(">= ").append(DB.TO_DATE(first.getStartDate()));
				} else {
