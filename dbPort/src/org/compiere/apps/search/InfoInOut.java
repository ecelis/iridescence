...
public class InfoInOut extends Info
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8374030305583636857L;
	...
	String getSQLWhere() {
		...
		if (fDateFrom.getValue() != null || fDateTo.getValue() != null)	{
			...
			if (DB.isOracle()) {
				...
				if (from == null && to != null) {
					sql.append(" AND DATE_TRUNC('day', i.MovementDate) <= ?");
				} else if (from != null && to == null) {
					sql.append(" AND DATE_TRUNC('day', i.MovementDate) >= ?");
				} else if (from != null && to != null) {
					sql.append(" AND DATE_TRUNC('day', i.MovementDate) BETWEEN ? AND ?");
				}
			}
		}
