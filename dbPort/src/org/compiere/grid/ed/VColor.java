...
//	private int             m_AD_Color_ID = 0;
	...
	{
		Integer AD_Color_ID = null;
		try {
			AD_Color_ID = Integer.parseInt(m_mTab.getValue("AD_Color_ID").toString());
		} catch (Exception e) {
			log.log(Level.SEVERE, "AD_Color_ID="+AD_Color_ID, e);
		} 
	...
	private Color getColor (boolean primary)
	{
		...
		int AD_Color_ID = 0;
		MColor color = null; //Lama
		if(m_mTab.getValue("AD_Color_ID") == null){
			String value = m_mTab.getQuery().toString();
			if(value.contains("AD_Color_ID")){
				value = value.substring(value.indexOf("=")+1,value.length());
				AD_Color_ID= Integer.parseInt(value);
				if (AD_Color_ID > 0)
					color = new MColor(Env.getCtx(), AD_Color_ID, null);
			}
		}
		//is either BD or Int
		Object objRed = color!= null ? color.getRed()+ add : m_mTab.getValue("Red" + add);
		Object objGreen = color!= null ? color.getGreen()+ add : m_mTab.getValue("Green" + add);
		Object objBlue = color!= null ? color.getBlue()+ add : m_mTab.getValue("Blue" + add);
		
		//
		int red = objRed == null ? 0 : Integer.parseInt(objRed.toString());
		int green = objRed == null ? 0 : Integer.parseInt(objGreen.toString());
		int blue = objRed == null ? 0 : Integer.parseInt(objBlue.toString());
		//
