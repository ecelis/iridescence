 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MSystem.java,v 1.2 2006/07/30 00:58:18 jjanke Exp $
 */
public class MSystem extends X_AD_System
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (userChange)
		{
			...
			{
				throw new MedsysException("Define a unique System name (e.g. Company name) not " + name);
			}
			if (getUserName().equals("?") || getUserName().length() < 2)
			{
				throw new MedsysException("Use the same EMail address as in the Adempiere Web Store");
			}
			if (getPassword().equals("?") || getPassword().length() < 2)
			{
				throw new MedsysException("Use the same Password as in the Adempiere Web Store");
			}
		}
		//
		setInfo();
