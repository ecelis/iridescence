...
import org.compiere.model.MEXMEPeriodDoc;
import org.compiere.model.MEXMEPolicyInfo;
...
public final class FactLine extends X_Fact_Acct
{
	...
	/** Conversion   */
	private int	mConversionTypeID = 0;
	...
	public void setAccount (MAcctSchema acctSchema, MAccount acct)
	{
		...
		// User Defined
		if (m_docLine != null && m_doc!=null && m_acctSchema!=null){
			setUserElements(m_docLine, m_acctSchema.getC_AcctSchema_ID());
		}
	}   //  setAccount
	...
	public BigDecimal getSourceBalance()
	{
		...
		//card 1195 Es el monto en moneda del cliente
		if(getC_Currency_ID() == m_acctSchema.getC_Currency_ID()){
	...
	public boolean convert ()
	{
		...
		}//AKI LAS CONversiones no son correctas
		
		setAmtAcctDr (MConversionRate.convert (getCtx()
			, getAmtSourceDr()
			, getC_Currency_ID()
			, m_acctSchema.getC_Currency_ID()
			, m_doc.getDateConvert()==null?getDateAcct():m_doc.getDateConvert()
			, C_ConversionType_ID
			, m_doc.getAD_Client_ID()
			, AD_Org_ID));
	...
	protected boolean beforeSave (boolean newRecord)
	{
		if (newRecord)
		{
			...
			if (getUserElement1_ID()==0){
				setUserElement1_ID(m_acct.getUserElement1_ID());
			}
			if (getUserElement2_ID()==0){
				setUserElement2_ID(m_acct.getUserElement2_ID());
			}

			//  Revenue Recognition for AR Invoices
			if (m_docLine != null
				&& (m_doc.getDocumentType().equals(Doc.DOCTYPE_ARInvoice)
						||	m_doc.getDocumentType().equals(Doc.DOCTYPE_ARDebitMemo)) //Expert. TTPR # Posteo Notas de Debito
		...
		//----------------------consideracion para numero de poliza------------------
				String diario = "DocumentoNo_Fact_Acct_Diario";   //Otros documentos
				String ingreso = "DocumentoNo_Fact_Acct_Ingreso"; //Estados de cuenta bancarios
				String egreso = "DocumentoNo_Fact_Acct_Egreso";   //Estados de cuenta bancarios

				MTable table = new MTable(this.getCtx(), this.getAD_Table_ID(), null);
				int record = this.getRecord_ID(); // Para el mismo record_id se crea el mismo numero de documento

				String documento = existeRecord(this.getAD_Table_ID(), record);

				if(table != null){
					if(table.getTableName().equals("C_BankStatement")){ //Estados de cuenta bancarios
						MBankStatement bank = new MBankStatement(this.getCtx(), this.getRecord_ID(), null);

						if(bank != null)
						if(bank.getStatementDifference().compareTo(BigDecimal.ZERO) == 1){ //Positivos = Ingresos
							//Secuencia para Ingresos
							if(documento != null){
								this.setDocumentoNo(documento);
							}else{
								MSequence seq = MSequence.getSequence(this.getCtx(), ingreso);
								String doc = seq.getPrefix() + "" + seq.getCurrentNext();
								this.setDocumentoNo(doc);
						...
						}else{ //Negativos = Egresos
							//Secuencia para Egresos
							if(bank.getStatementDifference().compareTo(BigDecimal.ZERO) == -1){

								if(documento != null){
									this.setDocumentoNo(documento);
								}else{
									MSequence seq = MSequence.getSequence(this.getCtx(), egreso);
									String doc = seq.getPrefix() + "" + seq.getCurrentNext();
									this.setDocumentoNo(doc);
							...
							//Secuencia para Diario
						if(table.getTableName().equals("M_InOut") || table.getTableName().equals("C_Invoice") || table.getTableName().equals("M_MatchInv") || table.getTableName().equals("C_Payment") || table.getTableName().equals("M_Inventory") || table.getTableName().equals("M_MatchPO") || table.getTableName().equals("M_MatchPO") || table.getTableName().equals("M_Movement") || table.getTableName().equals("GL_JournalBatch") || table.getTableName().equals("C_Order") || table.getTableName().equals("C_AllocationHdr")){

							if(documento != null){
								this.setDocumentoNo(documento);

							}else{
								MSequence seq = MSequence.getSequence(this.getCtx(), diario);
								String doc = seq.getPrefix() + "" + seq.getCurrentNext();
								this.setDocumentoNo(doc);
	...
	/**
	 * Verifica si existe el Record_ID
	 * @param AD_Table_id
	 * @param record_id
	 * @return documentoNo
	 *
	 * **/
	public String existeRecord(int tableID, int recordID){
		String documento_no=null;

		String sql =
				"SELECT documentoNo FROM Fact_Acct  WHERE isActive= 'Y' AND AD_Table_ID = ? AND Record_ID = ? ";

		documento_no = DB.getSQLValueStr(get_TrxName(), sql, tableID, recordID);
		...
		BigDecimal multiplier)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				// Si la moneda del esquema y la moneda de la factura son diferentes
				if(mConversionTypeID>0){
					if(crSourceAmt.compareTo(Env.ZERO)>0){
						if (m_acctSchema.getC_Currency_ID() != fact.getC_Currency_ID()){
							crSourceAmt = MConversionRate.convert(getCtx(), crSourceAmt,
									fact.getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
									fact.getDateAcct(),      mConversionTypeID,
									fact.getAD_Client_ID(),  fact.getAD_Org_ID());
						}
					} else {
						if (m_acctSchema.getC_Currency_ID() != fact.getC_Currency_ID()){
							drSourceAmt = MConversionRate.convert(getCtx(), drSourceAmt,
									fact.getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
									fact.getDateAcct(),      mConversionTypeID,
									fact.getAD_Client_ID(),  fact.getAD_Org_ID());
						}
	...
	public void setUserElements(final DocLine docLine, final int C_AcctSchema_ID){
		setUserElement1_ID(docLine.getUserElement1_ID(C_AcctSchema_ID));
		setUserElement2_ID(docLine.getUserElement2_ID(C_AcctSchema_ID));
	}

	public int getmConversionTypeID() {
		return mConversionTypeID;
	}

	public void setmConversionTypeID(int mConversionTypeID) {
		this.mConversionTypeID = mConversionTypeID;
	}
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		if (success && newRecord) {
			try {
				...
				if (glCategory.getAD_Org_ID() > 0) {
					orgId = glCategory.getAD_Org_ID();
				} else if (m_doc != null && m_doc.getAD_Org_ID() > 0) {
					orgId = m_doc.getAD_Org_ID();
				} else {
					log.saveError("Error", Msg.parseTranslation(getCtx(), "@Org0NotAllowed@"));
				}
				
				if (glCategory != null && MEXMEPeriodDoc.hasDocuments(getC_Period_ID(), glCategory.getPolicyType(), orgId, null)) {
					// Intentamos obtener la información de póliza
					MEXMEPolicyInfo policyInfo = MEXMEPolicyInfo.get(getCtx(), getAD_Table_ID(), getRecord_ID(), orgId, get_TrxName());

					// Si no hay información, se crea
					if (policyInfo == null) {
						policyInfo = new MEXMEPolicyInfo(getCtx(), 0, null);
						policyInfo.setAD_Table_ID(getAD_Table_ID());
						policyInfo.setRecord_ID(getRecord_ID());
						policyInfo.setDateTrx(getDateAcct());

						policyInfo.setAD_Org_ID(orgId);
					...
					// Si el número de póliza no ha sido asignado, generarlo
					if (policyInfo.getPolicyNo() <= 0) {
						// Obtener el número de póliza por periodo y tipo de póliza
						int currentNext = MEXMEPeriodDoc.getCurrentNext(getCtx(), getC_Period_ID(), glCategory.getPolicyType(), orgId);

						if (currentNext > 0) {
							policyInfo.setPolicyNo(currentNext);
						}
					...
					if (getEXME_PolicyInfo_ID() <= 0) {
						setEXME_PolicyInfo_ID(policyInfo.getEXME_PolicyInfo_ID());
						save(get_TrxName());
					}
				}
			} catch (Exception ex) {
