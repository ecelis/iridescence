	...
	implements Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4935359714445598758L;
	...
	private String createSelectSql()
	{
		...
		for (int i = 0; i < m_fields.size(); i++)
		{
			...
			if(field.getVO().IsKey)
				keyColumn = field.getColumnName();
		}
		//
		...
		// HIGH VOLUME TABLES .- Expert: Lama
		createHighVolumeSql();

		//	ORDER BY
		if (!m_orderClause.equals("") && !m_IsHighVolume)// only regular tables
		...
		try
		{
			...
			for (int col = 0; col < size; col++)
			{
				...
				if (field.isVirtualColumn())
				{
					...
					// Expert: Lama - Mask
					if (m_rowData[col] != null && field.getFormat() != null) {
						m_rowData[col] = field.getFormat().replaceChars(m_rowData[col].toString());
					}
				}
			...
			if(!manualUpdate){
				if(m_inserting){
					DBLogger.logForcedTRL(DBLogger.INSERT,m_tableName,whereClause);
				}else{
					DBLogger.logForcedTRL(DBLogger.UPDATE,m_tableName,whereClause);
				}			
			}
	...
	private char dataSavePO (int Record_ID) throws Exception
	{
		...
		for (int col = 0; col < size; col++)
		{
			  ...
			  || !oldValue.equals (value)) 			//	changed
			{
				...
				if (m_inserting 
					...
					|| (oldValue != null && oldValue.toString().equals (dbValue.toString()))
					//	Target == DB (changed by trigger to new value already)
					|| (value == null && dbValue == null)
					|| (value != null && value.equals (dbValue)) )
				{
					// Expert: Lama - Mask
					if (value != null && field.getFormat() != null) {
						value = field.getFormat().replaceChars(value.toString());
					}
				...
				{
					StringBuilder msg = new StringBuilder(columnName).append("= ").append(oldValue)//
							.append(oldValue == null ? "" : ("(" + oldValue.getClass().getName() + ")"))//
							.append(" != DB: ").append(dbValue).append(dbValue == null ? "" : ("(" + dbValue.getClass().getName() + ")"))//
							.append(" -> New: ").append(value).append(value == null ? "" : ("(" + value.getClass().getName() + ")"));
					// CLogMgt.setLevel(Level.FINEST);
					// po.dump();
					fireDataStatusEEvent("SaveErrorDataChanged", msg.toString(), true);
					dataRefresh(m_rowChanged);
					return SAVE_ERROR;
				}
			}	//	Data changed
		...
		if(confirmPanel != null){
			po.setConfirmPanel(confirmPanel);
		}
	...
	public boolean dataNew (int currentRow, boolean copyCurrent)
	{
		...
		if (copyCurrent)
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				//Lama: No copiar los registros que sean de tipos de dato de mtto.
				else if(DisplayType.isMaintenance(field.getDisplayType())) { 
					rowData[i] = field.getDefault();
					field.setValue(rowData[i], m_inserting);
				} else {
			...
			for (int j = 0; j < size; j++)
			{
					...
					|| columnName.endsWith("atedBy"))
				{
					...
					try {
						...
						if (s != null) {
							if (s.length() >= 10) {
								rowData[j] = new BigDecimal(s);
							} else {
								rowData[j] = new Integer(s);
							}
						}
					} catch (Exception e) {
						rowData[j] = null;
				...
				else if (columnName.toUpperCase().equals("EDAD")) {//edelagarza
					String str = rs.getString(j + 1);//rs.getInt(j + 1);
					str = Utilerias.getAgeFromSubstrings(m_ctx, str);
					rowData[j] = str;
				}
				//	Number
	...
	class Loader extends Thread implements Serializable
	{
		...
		public void run()
		{
			log.info("Loading " + m_tableName);
			if (m_rs == null)
				return;
			setBlocks();// Expert: Lama - high volume
			try
			{
				while (m_rs.next())
				{
					...
					setIdxBlock(m_rs);// Expert: Lama - high volume
					//  Get Data
	...
	public boolean m_IsHighVolume = false;
	public boolean m_prev = false;
	public boolean m_next = false;
	public int m_maxHighVolRows = -1;
	public int m_totalBlocks = -1;
	public int m_totalRows = -1;
	public int m_curBlock = 0;
	private int m_indexIni = -1;
	private int m_indexFin = -1;
	private int m_indexTmp = 0;
	private String m_SQL_Count_mx = null;
	private Integer[] idxarr = null;
	private String keyColumn = null;
	
	/**
	 * @author Lorena Lama
	 */
	public void createHighVolumeSql() {
		if(!testCount() && !m_IsHighVolume)
			return;
		StringBuffer highVolume = new StringBuffer();
		if (m_prev) {
			if (m_indexIni > 0)
				highVolume.append(keyColumn + " < " + m_indexIni).append(" AND ");
			if (idxarr[m_curBlock - 1] != null)
				highVolume.append(keyColumn + " >= " + idxarr[m_curBlock - 1]).append(" AND ");
		}
		else if (m_next) {
			if (m_indexFin > 0)
				highVolume.append(keyColumn + " > " + m_indexFin).append(" AND ");
		}
		
		if (DB.isOracle()) {
			highVolume.append(" rowNum <= ");
			highVolume.append(m_maxHighVolRows);
		}


		StringBuffer sqlOrder = new StringBuffer();
		sqlOrder.append(" ORDER BY ");
		sqlOrder.append(keyColumn);
		sqlOrder.append(" ASC ");

		m_SQL = new StringBuffer("SELECT * FROM (" + m_SQL + sqlOrder.toString() + ")").toString(); 
		
		if (DB.isPostgreSQL()) {
			m_SQL = m_SQL + " AS producto";
		}
		...
		if (DB.isOracle()) {
			m_SQL = m_SQL + highVolume;
		} else if (DB.isPostgreSQL()) {
			if (StringUtils.trim(highVolume.toString()).endsWith("AND")) {
				int end = StringUtils.trim(highVolume.toString()).lastIndexOf(StringUtils.trim("AND"));
				m_SQL = m_SQL + highVolume.toString().substring(0, end);
			} else if (highVolume.length() <= 0) {
				//Si esta vacio entoncess hay que quitar el ultimo where de m_SQL
				int endWhere = StringUtils.trim(m_SQL).lastIndexOf(StringUtils.trim("WHERE"));
				m_SQL = m_SQL.substring(0, endWhere);
			}
			
			m_SQL = new StringBuffer(DB.getDatabase().addPagingSQL(m_SQL, 1, m_maxHighVolRows)).toString();
		}
		
		m_SQL_Count_mx = m_SQL_Count; // total
		
		m_SQL_Count += (m_SQL_Count.indexOf(" WHERE ") == -1 ? " WHERE " : " AND ");
		
		//Adecuaciones al manejo de postgresql y Oracle.
		if (DB.isOracle()) {
			m_SQL_Count = m_SQL_Count + highVolume.toString();
		} else if (DB.isPostgreSQL()) {
			if (StringUtils.trim(highVolume.toString()).endsWith("AND")) {
				//Remover el AND
				int end = StringUtils.trim(highVolume.toString()).lastIndexOf(StringUtils.trim("AND"));
				m_SQL_Count = m_SQL_Count + highVolume.toString().substring(0, end);
				
			} else if (highVolume.length() <= 0) {
				// Si tiene AND el m_SQL_Count, removerlo
				if (StringUtils.trim(m_SQL_Count).endsWith("AND")) {
					int endAnd = StringUtils.trim(m_SQL_Count.toString()).lastIndexOf(StringUtils.trim("AND"));
					m_SQL_Count = m_SQL_Count.toString().substring(0, endAnd);
					
				} else if (StringUtils.trim(m_SQL_Count).endsWith("WHERE")) {
					//Si tiene where, el m_SQL_Count removerlo
					int endWh = StringUtils.trim(m_SQL_Count.toString()).lastIndexOf(StringUtils.trim("WHERE"));
					m_SQL_Count = m_SQL_Count.toString().substring(0, endWh);
				}
			}
			
			m_SQL_Count = new StringBuffer(DB.getDatabase().addPagingSQL(m_SQL_Count, 1, m_maxHighVolRows)).toString();
		}
	...
	public boolean testCount(){
		m_IsHighVolume = false;
		if (keyColumn == null || keyColumn.equals("")) {
			return m_IsHighVolume;
		}
		m_totalRows = DB.getSQLValue(null, m_SQL_Count);
		if(m_totalRows >= Constantes.MAX_QUERY_RECORDS)
			m_IsHighVolume = true;
		return m_IsHighVolume;
	}
	...
	public void setBlocks() {
		if (!m_IsHighVolume)
			return;
		if (m_totalBlocks <= 0) {
			int mxRows = DB.getSQLValue(null, m_SQL_Count_mx);
			double value = mxRows / m_maxHighVolRows;
			m_totalBlocks = new Double(Math.ceil(value)).intValue();
			if (m_totalBlocks > 0)
				idxarr = new Integer[m_totalBlocks];
		}
		if (m_totalBlocks > 1) {
			if (m_prev)
				m_curBlock = m_curBlock - 1;
			else
				m_curBlock = m_curBlock + 1;
		}
		log.fine("Total blocks: " + m_totalBlocks);
	}
	 ...
	 * @param m_rs
	 */
	public void setIdxBlock(ResultSet m_rs) {
		if (!m_IsHighVolume)
			return;
		try {
			if (m_rs.isFirst()) {
				if (m_indexIni >= 0 && m_next){
					m_indexTmp = m_indexIni;
					if (idxarr.length > 0 && idxarr[m_curBlock - 1] == null)
						idxarr[m_curBlock - 1] = m_indexTmp;
				}
				m_indexIni = m_rs.getInt(keyColumn);
			}
			m_indexFin = m_rs.getInt(keyColumn);
		} catch (Exception e) {
			log.log(Level.SEVERE, "setIdxBlock", e);
		}
	}
}
