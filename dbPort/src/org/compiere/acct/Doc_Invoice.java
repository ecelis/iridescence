...
public class Doc_Invoice extends Doc
{
	...
	private DocTax[] loadTaxes()
	{
		...
		String sql = "SELECT it.C_Tax_ID, t.Name, t.Rate, it.TaxBaseAmt, it.TaxAmt, t.IsSalesTax "
			...
			+ "WHERE t.C_Tax_ID=it.C_Tax_ID AND it.C_Invoice_ID=? AND it.AD_Client_ID = ? ";
	...
	public BigDecimal getBalance()
	{
		...
		for (int i = 0; i < m_taxes.length; i++)
		{
			retValue = retValue.subtract(m_taxes[i].getAmount().setScale(2, BigDecimal.ROUND_HALF_UP));
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			sb.append("-").append(p_lines[i].getAmtSource().setScale(2, BigDecimal.ROUND_HALF_UP));
		}
		sb.append("]");
		//
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		if (getDocumentType().equals(DOCTYPE_ARInvoice) 
			|| getDocumentType().equals(DOCTYPE_ARProForma)
				|| getDocumentType().equals(DOCTYPE_ARDebitMemo) //Expert. TTPR # Posteo Notas de Debito 
				)
		{
		...
		//  ** API
				...
				|| getDocumentType().equals(DOCTYPE_APDebitMemo)  //Expert. TTPR # Posteo Notas de Debito
				)
		{
			BigDecimal grossAmt = getAmount(Doc.AMTTYPE_Gross);
			BigDecimal serviceAmt = Env.ZERO;
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (!landedCost)
				{//MAccount=[10004635,Hospital San José de Monclova-5104-_-_-_-_]
			...
			if(isEmployee()){
				payables_ID = getValidCombination_ID (Doc.ACCTTYPE_E_Expense_Employee, as);
				payablesServices_ID = getValidCombination_ID (Doc.ACCTTYPE_E_Expense_Employee, as);
		...
		else if (getDocumentType().equals(DOCTYPE_APCredit))
		{
			...
			//  Liability       DR
			int payables_ID = 0;//getValidCombination_ID (Doc.ACCTTYPE_V_Liability, as);
			int payablesServices_ID = 0;//getValidCombination_ID (Doc.ACCTTYPE_V_Liability_Services, as);
	...
	private boolean landedCost (MAcctSchema as, Fact fact, DocLine line, boolean dr) 
	{
		...
		for (int i = 0; i < lcas.length; i++)
		{
			...
			if (getC_Currency_ID() != as.getC_Currency_ID()) {
				...
				allocationAmt = MConversionRate.convert(getCtx()
					, allocationAmt 
					, getC_Currency_ID()
					, as.getC_Currency_ID()
					, getDateConvert()==null?getDateAcct():getDateConvert()
	...
	/**
	 * #1249 - Factura de Proveedor en ZK
	 * @return si es Empleado (deudor diverso)
	 * @author twry
	 */
	public boolean isEmployee() {
		final MBPartner partner = new MBPartner(getCtx(), getC_BPartner_ID(), getTrxName());
		return partner.isEmployee() && !partner.isVendor();
	}
}   //  Doc_Invoice
