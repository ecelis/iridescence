 ...
 *  </pre>
 *  @author     Jorg Janke
 *  @version    $Id: DisplayType.java,v 1.5 2006/08/09 05:25:26 jjanke Exp $
 */
public final class DisplayType
{
	...
	/**Expert: Lama - Employment Information*/
	public static final int Employment = 43;
	//	Candidates: 
	...
	public static boolean isID (int displayType)
	{
		if (displayType == ID || displayType == Table || displayType == TableDir
			...
			|| displayType == Image || displayType == Employment /*Expert:Lama*/
			/*|| displayType == Color*/) //expert : mrojas : integracion zk
			return true;
		return false;
	}	//	isID
	...
	/** Indica si el tipo de dato manda llamar una ventana de tipo mantenimiento
	 * @author Lorena Lama
	 */
	public static boolean isMaintenance(int displayType) {
		return displayType == Locator || displayType == Location || displayType == Employment 
		|| displayType == Account || displayType == PAttribute || displayType == Assignment;
	}
	...
	public static SimpleDateFormat getDateFormat (int displayType, Language language, String pattern)
	{
		...
		//
		if ( pattern != null && pattern.length() > 0) {
			retVal = (SimpleDateFormat)DateFormat.getInstance();
			
			try {
				retVal.applyPattern(pattern);
			} catch (IllegalArgumentException e) {
				s_log.log(Level.WARNING, "Invalid date pattern: " + pattern);
				retVal = myLanguage.getDateFormat();
			}
			
		} else if (displayType == DateTime) {
			retVal = myLanguage.getDateTimeFormat();
		} else if (displayType == Time) {
			retVal =  myLanguage.getTimeFormat();
		} else {
			retVal = myLanguage.getDateFormat();
		}
		// else if (displayType == Date)
		retVal.setTimeZone(TimeZone.getTimeZone(Env.getTimezone(Env.getCtx())));
	...
	{
		if (isText(displayType) || displayType == List || displayType == Color) { //Color String (hexadecimal)
			return String.class;
	...
	public static String getDescription (int displayType)
	{
		...
		if (displayType == PrinterName)
			return "PrinterName";
		if (displayType == Employment)
			return "Employment";// Expert:Lama
		//
