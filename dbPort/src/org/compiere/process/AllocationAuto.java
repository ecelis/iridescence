	...
	protected String doIt() throws Exception
	{
		...
		// Lama - codigo duplicado
			...
			if (p_C_BP_Group_ID != 0) {
				...
				sql.append(" SELECT bp.C_BPartner_ID FROM C_BPartner bp ");
				// Lama - Partner_Cte
				sql.append(" LEFT JOIN C_BPartner_Cte pcte ON (bp.C_BPartner_ID=pcte.C_BPartner_ID  ");
				sql.append(" AND pcte.AD_Client_ID = ? ) ");// #1
				sql.append(" WHERE NVL(pcte.C_BP_Group_ID,bp.C_BP_Group_ID)=? ");
				sql.append(" ORDER BY Value ");
			} else {
				sql.append("SELECT C_BPartner_ID FROM C_BPartner WHERE AD_Client_ID=? ORDER BY Value");
			}
		...
		int C_BPartner_ID, int C_Payment_ID, int C_Invoice_ID, int AD_Org_ID)
	{
		...
		if (m_allocation == null)
		{
			m_allocation = new MAllocationHdr (getCtx(), false, dateAcct,	//	automatic 
				C_Currency_ID, "Auto " + description + " [w]", get_TrxName());
