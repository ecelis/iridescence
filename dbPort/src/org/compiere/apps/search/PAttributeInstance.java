	...
	implements ListSelectionListener
{
	...
	private static ColumnInfo[] s_layout = new ColumnInfo[] 
	{
		...
		//	See RV_Storage
		new ColumnInfo(Msg.translate(Env.getCtx(), "GoodForDays"), "(TRUNC(asi.GuaranteeDate)-TRUNC("+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + "))-p.GuaranteeDaysMin", Integer.class, true, true, null),
		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeDays"), "TRUNC(asi.GuaranteeDate)-TRUNC("+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ")", Integer.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeRemainingPct"), "CASE WHEN p.GuaranteeDays > 0 THEN TRUNC(((TRUNC(asi.GuaranteeDate)-TRUNC("+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + "))/p.GuaranteeDays)*100) ELSE 0 END", Integer.class),
	};
	
	/**	Table Column Layout Info			*/
	private static ColumnInfo[] s_layoutPostgreSQL = new ColumnInfo[]
	{
		new ColumnInfo(" ", "s.M_AttributeSetInstance_ID", IDColumn.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Description"), "asi.Description", String.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "Lot"), "asi.Lot", String.class),
		...
		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyOrdered"), "s.QtyOrdered", Double.class),
		//	See RV_Storage
		new ColumnInfo(Msg.translate(Env.getCtx(), "GoodForDays"), "(DATE_TRUNC('day', asi.GuaranteeDate)-DATE_TRUNC('day', "+ DB.TO_DATE(new Timestamp(System.currentTimeMillis())) + "))-p.GuaranteeDaysMin", Integer.class, true, true, null),
		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeDays"), "DATE_TRUNC('day', asi.GuaranteeDate)-DATE_TRUNC('day', "+ DB.TO_DATE(new Timestamp(System.currentTimeMillis())) + ")", Integer.class),
		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeRemainingPct"), "CASE WHEN p.GuaranteeDays > 0 THEN DATE_TRUNC('day',((DATE_TRUNC('day',asi.GuaranteeDate)-DATE_TRUNC('day', "+ DB.TO_DATE(new Timestamp(System.currentTimeMillis())) + "))/p.GuaranteeDays)*100) ELSE 0 END", Integer.class),
	};
	...
	private void dynInit(int C_BPartner_ID)
	{
		...
		if (C_BPartner_ID != 0)
		{
			...
			{
				if (DB.isOracle()) {
					m_sqlMinLife = " AND COALESCE(TRUNC(((TRUNC(asi.GuaranteeDate)-TRUNC("+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + "))/p.GuaranteeDays)*100),0)>=" + ShelfLifeMinPct;
				} else if (DB.isPostgreSQL()) {
					m_sqlMinLife = " AND COALESCE(DATE_TRUNC('day',((DATE_TRUNC('day', asi.GuaranteeDate)-DATE_TRUNC('day', "+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + "))/p.GuaranteeDays)*100),0)>=" + ShelfLifeMinPct;
				}
			...
			{	
				if (DB.isOracle()) {
					m_sqlMinLife += " AND COALESCE((TRUNC(asi.GuaranteeDate)-TRUNC("+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ")),0)>=" + ShelfLifeMinDays;
				} else if (DB.isPostgreSQL()) {
					m_sqlMinLife += " AND COALESCE((DATE_TRUNC('day',asi.GuaranteeDate)-DATE_TRUNC('day', "+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ")),0)>=" + ShelfLifeMinDays;
				}
		...
		if (DB.isOracle()) {
			...
			m_sql = m_table.prepareTable(s_layoutPostgreSQL, s_sqlFrom,
					s_sqlWhere, false, "s")
					+ " ORDER BY asi.GuaranteeDate, s.QtyOnHand"; // oldest, smallest first
		}
