...
public class MProcess extends X_AD_Process
{
	...
	public static MProcess get (Properties ctx, int AD_Process_ID)
	{
		...
		if (retValue != null 
			...
			return retValue;*/
		MProcess retValue = new MProcess (ctx, AD_Process_ID, null);
	...
	public boolean processIt (ProcessInfo pi, Trx trx, boolean managedTrx)
	{
		...
		if (Classname != null && Classname.length() > 0)
		{
			...
			ok = startClass(pi, trx, managedTrx);// Merge ADempiere trunk_rev14653
		}
		else
		{
			//	PL/SQL Procedure
			...
			{
				ok = startProcess (ProcedureName, pi, trx, managedTrx);// Merge ADempiere trunk_rev14653
			}
			else
			{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		if (newRecord)	//	Add to all automatic roles
		{
			...
			for (int i = 0; i < roles.length; i++)
			{
				...
				pa.saveEx();// Merge ADempiere trunk_rev14653
			}
		}
		//	Menu/Workflow
			...
			|| is_ValueChanged("Description") || is_ValueChanged("Help"))
		{
			...
			for (int i = 0; i < menues.length; i++)
			{
				...
				menues[i].saveEx();// Merge ADempiere trunk_rev14653
			}
			...
			for (int i = 0; i < nodes.length; i++)
			{
				...
				if (changed)
					nodes[i].saveEx();// Merge ADempiere trunk_rev14653
			}
		}
		return success;
	}	//	afterSave
	...
	/**
	 * Metodo que regresa la traduccion de una columna en especifico
	 * 
	 * @param columnName
	 *            el nombre de la columna atraducir
	 **/
	public String getTranslate(String columnName){
		return get_Translation(columnName);
	}
