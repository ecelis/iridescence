...
public class MLandedCost extends X_C_LandedCost
{
	...
	public MLandedCost[] getLandeCostByInvoiceLineID (int invoiceLineID)
	{		
		//ArrayList<MInvoiceLine> list = new ArrayList<MInvoiceLine>();
		ArrayList<MLandedCost> list = new ArrayList<MLandedCost>();
		
		String sql = "SELECT * from c_landedcost WHERE  processing = 'N' AND c_invoiceline_id = ? ";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		//
		MLandedCost[] lines = new MLandedCost[list.size()];
		list.toArray(lines);
		return lines;
	}	//	getLines
