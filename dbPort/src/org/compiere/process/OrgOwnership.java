...
public class OrgOwnership extends SvrProcess
{
	...
	private String bPartnerOwnership ()
	{
		...
		if (p_C_BP_Group_ID > 0){
			set += " WHERE EXISTS (SELECT bp.* FROM C_BPartner bp "
				// Lama - Partner Cte
				//+ " LEFT JOIN C_BPartner_Cte pcte ON (bp.C_BPartner_ID=pcte.C_BPartner_ID "
				//+ " AND pcte.AD_Client_ID =" + Env.getAD_Client_ID(getCtx()) + " )" //#1 - FIXME
				+" WHERE bp.C_BPartner_ID=x.C_BPartner_ID " 
				+ "AND bp.C_BP_Group_ID=" + p_C_BP_Group_ID + ")"; //#2 - FIXME
		}else
