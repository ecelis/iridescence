	...
	protected String doIt () throws Exception
	{
		...
		sql = "SELECT * "
			...
			+ " AND EXISTS (SELECT * FROM M_Product p"
			    + " INNER JOIN EXME_ProductoOrg po ON (p.M_Product_ID=po.M_Product_ID AND po.IsActive='Y' AND po.AD_Org_ID = "+Env.getAD_Org_ID(Env.getCtx())+ ") "
				+ " INNER JOIN M_AttributeSet  mas ON (po.M_AttributeSet_ID=mas.M_AttributeSet_ID) "
	...
	private void eliminateReservation(MStorage target)
	{
		...
		if (target.getQtyReserved().signum() != 0 || target.getQtyOrdered().signum() != 0)
		{
			...
			if (storage0 != null)
			{
				...
				{
					if (MStorage.add(getCtx(), target.getMWarehouseID(), target.getM_Locator_ID(), 
					...
					{
						if (MStorage.add(getCtx(), storage0.getMWarehouseID(), storage0.getM_Locator_ID(), 
