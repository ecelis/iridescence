...
public class MReportSource extends X_PA_ReportSource
{
	...
	private static CLogger slog = CLogger.getCLogger(MReportSource.class);
	...
	/**
	 *
	 * @param elementValue The element value to be used for the report line source
	 * creation.
	 */
	public static void createSourceForElement(MElementValue elementValue) {

		// look for lines containing the parent for the new element value
		String sql =
				"SELECT PA_ReportLine_ID FROM PA_ReportLine WHERE AD_Client_ID = ? AND C_ElementValue_ID = ?";
		PreparedStatement pstmt = DB.prepareStatement(sql, null);
		ResultSet rs = null;

		try {

			pstmt.setInt(1, elementValue.getAD_Client_ID());
			pstmt.setInt(2, elementValue.getParentElementValue_ID());

			rs = pstmt.executeQuery();

			while(rs.next()) {
				MReportSource reportSource =
						new MReportSource(elementValue.getCtx(), 0, elementValue.get_TrxName());

				reportSource.setPA_ReportLine_ID(rs.getInt(1));
				reportSource.setC_ElementValue_ID(elementValue.getC_ElementValue_ID());
				reportSource.setDescription(elementValue.getValue() + " - " + elementValue.getName());
				reportSource.setElementType(X_PA_ReportSource.ELEMENTTYPE_Account);

				if(!reportSource.save()) {
					slog.warning(
							"Can't create report source for report line "
									+ rs.getInt(1)
					);
				}
			}
		} catch (SQLException e) {
			slog.log(Level.WARNING, "SQL : " + sql, e);
		} finally {
			DB.close(rs, pstmt);
		}
	}
	 ...
	 * @param cElementValueId The Element Value being disabled / deleted.
	 */
	public static void deleteForElementValue(Properties ctx, int cElementValueId) {

		String sql =
				"DELETE FROM PA_ReportSource where AD_Client_ID = ? AND C_ElementValue_ID = ?";

		int no =
				DB.executeUpdate(
						sql,
						new Object[]{Env.getAD_Client_ID(ctx), cElementValueId},
						null
				);

		slog.log(
				Level.WARNING,
				"Deleting " + no + " sources for C_ElementValue_ID = " + cElementValueId
		);
	}
