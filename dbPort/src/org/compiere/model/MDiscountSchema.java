...
public class MDiscountSchema extends X_M_DiscountSchema{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 673023897959190052L;
	
	private static CLogger		s_log = CLogger.getCLogger(MDiscountSchema.class);
	...
	*/
	...
	{
		String sql = "SELECT Name, M_DiscountSchema_ID FROM M_DiscountSchema WHERE IsActive = 'Y' ";
		sql = MEXMELookupInfo.addAccessLevelSQL(ctx, sql, "M_DiscountSchema");
	...
	/**
	 * regresa el listado de esquema de descuentos dependiendo el nivel de acceso
	 * @param ctx
	 * @param trxName
	 * @return
	 */
	public static List<MDiscountSchema> getList(Properties ctx, String trxName){
		List<MDiscountSchema> list = new ArrayList<MDiscountSchema>();
		
		StringBuilder sql = new StringBuilder(); 
		
		sql.append("SELECT * FROM M_DiscountSchema WHERE IsActive = 'Y' ");
		sql.append(MEXMELookupInfo.addAccessLevelSQL(ctx, " ", MDiscountSchema.Table_Name));
