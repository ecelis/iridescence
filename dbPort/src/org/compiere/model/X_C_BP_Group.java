...
// N
	...
	/** Set Payment without voucher.
		@param Payment_NotVoucher 
		Payment without voucher
	  */
	public void setPayment_NotVoucher (boolean Payment_NotVoucher)
	{
		set_Value (COLUMNNAME_Payment_NotVoucher, Boolean.valueOf(Payment_NotVoucher));
	}

	/** Get Payment without voucher.
		@return Payment without voucher
	  */
	public boolean isPayment_NotVoucher () 
	{
		Object oo = get_Value(COLUMNNAME_Payment_NotVoucher);
		if (oo != null) 
		{
			 if (oo instanceof Boolean) 
				 return ((Boolean)oo).booleanValue(); 
