...
public class GridTabVO implements Evaluatee, Serializable
{
	...
	private static boolean loadTabDetails (GridTabVO vo, ResultSet rs)
	{
		...
		/*se evalua si tiene acceso desde el perfil no de el econtexto de la aplicacion
		 * ya que en web no se puede modificar el contexto por medio de preferencias*/
		boolean showAcct = role.isShowAcct();//"Y".equals(Env.getContext(vo.ctx, "#ShowAcct"));pf
	...
	//	CLogger.get().warning("ShowTrl=" + showTrl + ", showAcct=" + showAcct);
		try
		{
			...
			// Expert: Lama
			vo.IsCopyValues = rs.getString("IsCopyValues").equals("Y");
			//
	...
	/** Copy values	Expert:Lama*/
	public  boolean     IsCopyValues = false;
	...
	protected GridTabVO clone(Properties Ctx, int windowNo)
	{
		...
		// Expert: Lama
		clone.IsCopyValues = IsCopyValues;
