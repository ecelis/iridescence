...
public class MQuery implements Serializable
{
	...
	static public MQuery get (Properties ctx, int AD_PInstance_ID, String TableName)
	{
		...
		try
		{
			...
			for (int row = 0; rs.next(); row++)
			{
				...
				if (DB.isOracle()) {
					trunc = "TRUNC("+ParameterName+")";
				} else if (DB.isPostgreSQL()) {
					trunc = "DATE_TRUNC('day', " + ParameterName + ")";
				}
				...
				else if (P_Number != null || P_Number_To != null)
				{
					if (P_Number_To == null)
					{
						...
						if (P_Number == null) {
							query.addRestriction(trunc, MQuery.LESS_EQUAL, 
				...
				//	Date
					...
					if (P_Date_To == null) {
						...
						if (isRange) {
							query.addRestriction(trunc, MQuery.GREATER_EQUAL, 
								P_Date, Name, Info);
						} else {
							query.addRestriction(trunc, MQuery.EQUAL, 
								P_Date, Name, Info);
						}
					} else {	//	P_Date_To != null
						...
						if (P_Date == null) {
							...
							query.addRangeRestriction(trunc, 
	...
	public static String getZoomColumnName (String columnName)
	{
		...
		// Fix "*_To" columns ** 
		// Expert: Lama --> Se cambia a CaseSensitive por Error en tablas como: Asunto, Concepto, Formato, Proyecto etc
	...
	/**	Operators for Strings				*/
	public static final ValueNamePair[] OPERATORSTRANSLATED = new ValueNamePair[] { //
			new ValueNamePair(EQUAL, 					Msg.getMsg(Env.getCtx(), "="		)), // 0
			new ValueNamePair(NOT_EQUAL, 			Msg.getMsg(Env.getCtx(), "!="		)), //
			new ValueNamePair(LIKE, 						Msg.getMsg(Env.getCtx(), "~"		)), //
			new ValueNamePair(NOT_LIKE, 				Msg.getMsg(Env.getCtx(), "!~"		)),//
			new ValueNamePair(GREATER, 				Msg.getMsg(Env.getCtx(), ">"		)), //
			new ValueNamePair(GREATER_EQUAL, 	Msg.getMsg(Env.getCtx(), ">="	)), // 5
			new ValueNamePair(LESS, 						Msg.getMsg(Env.getCtx(), "<"		)), //
			new ValueNamePair(LESS_EQUAL, 			Msg.getMsg(Env.getCtx(), "<="	)), //
			new ValueNamePair(BETWEEN, 				Msg.getMsg(Env.getCtx(), ">-<"	)) // 8
	};
	...
	/**	Operators for IDs					*/
	public static final ValueNamePair[]	OPERATORSTRANSLATED_ID = new ValueNamePair[] {//
		new ValueNamePair(EQUAL, 					Msg.getMsg(Env.getCtx(), "="		)), // 0
		new ValueNamePair(NOT_EQUAL, 			Msg.getMsg(Env.getCtx(), "!="		))//
	};
	/**	Operators for Boolean					*/
	public static final ValueNamePair[]	OPERATORSTRANSLATED_YN = new ValueNamePair[] {
		new ValueNamePair(EQUAL, 					Msg.getMsg(Env.getCtx(), "="		)) // 0
	};
	...
	// Expert: Lama
	public void addOrRestriction(String ColumnName, String Operator, Object Code, String InfoName, String InfoDisplay) {
	...
	// Expert: Lama
	public void addOrRestriction (String whereClause) {
		if (whereClause == null || whereClause.trim().length() == 0)
			return;
		Restriction r = new Restriction (whereClause);
		r.andCondition=false;
...
class Restriction  implements Serializable
{
	...
	public String getSQL (String tableName)
	{
		...
		if (tableName != null && tableName.length() > 0)
		{
			...
			// Revisar si tiene Date_Trunc. Jesus Cantu. RQM 3045.
			if (DB.isPostgreSQL()) {
				//Cuando la BD es postgresql coloca en los campos de fecha DATE_TRUNC('day', XX);
				//o DATE_TRUNC('minute', XX), etc, entonces hay que cambiar la posicion inicial.
				if (ColumnName.toUpperCase().indexOf("DATE_TRUNC") > -1) {
					// Si contiene DATE_TRUNC entonces obtener la posicion hasta la , + 1
					// DATE_TRUNC('day', XX);
					pos = ColumnName.lastIndexOf(',')+1;
				}
			} 
