	...
	implements Serializable, Evaluatee
{
	...
	public void loadLookup()
	{
		...
		//Expert:Lama
		else if (m_vo.displayType == DisplayType.Employment) {  //  not cached
			MEmploymentLookup ml = new  MEmploymentLookup(m_vo.ctx, m_vo.WindowNo);//TODO
			m_lookup = ml;
		}
	...
	//		retValue = false;
			...
			|| m_vo.displayType == DisplayType.Employment // Expert:Lama
			)
			retValue = true;
...
//	  Do we have a mandatory rule
	...
	public Object getDefault()
	{
		...
		if (!m_vo.DefaultValue.equals("") && !m_vo.DefaultValue.startsWith("@SQL="))
		{
			...
			while (st.hasMoreTokens())
			{
				...
				if ("@SysDate@".equals(defStr)) { 				//	System Time
					return new 
						Timestamp (
							new LocalDateTime(
									DateTimeZone.forID(
											Env.getTimezone(m_vo.ctx)
									)).toDateTime().getMillis()
						);
				} else if (defStr.indexOf('@') != -1) {			//	it is a variable
					defStr = 
						Env.getContext(m_vo.ctx, m_vo.WindowNo, defStr.replace('@',' ').trim());
				} else if (defStr.indexOf("'") != -1) {			//	it is a 'String'
					defStr = defStr.replace('\'', ' ').trim();
				}

				if (StringUtils.isNotEmpty(defStr)) {
		...
		// Expert Alejandro	Locator
		if (m_vo.displayType == DisplayType.Locator)
		{
			log.fine("[Locator=null] " + m_vo.ColumnName);
			return null;
		}
	...
	public boolean validateValue()
	{
		...
		if (!isLookup()
			...
			|| getDisplayType() == DisplayType.Location) //to avoid loss of location
			return true;
	...
	public String getColumnSQL(boolean withAS)
	{
		...
		{
			Object str = m_vo.ColumnSQL; // Expert: Lama
			if (m_vo.ColumnSQL.indexOf('@') != -1) {
				// replace variables
				str = Env.parseContext(m_vo.ctx, m_vo.WindowNo, str.toString(), false);
			}
			if (withAS) {
				return str + " AS " + m_vo.ColumnName;
			} else {
				return str.toString();
			}
		}
	...
	*/
	...
	{
		return m_vo.IsSearchColumn;
	}
	
	private HashMap<String, Callout> calloutMap= new HashMap<String, Callout>();
	
	public HashMap<String, Callout> getCalloutMap() {
		return calloutMap;
	}

	public void setCalloutMap(HashMap<String, Callout> calloutMap) {
