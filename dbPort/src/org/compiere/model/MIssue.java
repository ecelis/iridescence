...
import org.compiere.util.vo.IssueVO;
...
public class MIssue extends X_AD_Issue
{
	...
	/**
	 * Obtiene el log de errores según los parámetros
	 * @param ctx
	 * @param userIni
	 * @param userFin
	 * @param fechaIni
	 ...
	 * @return
	 */
	public static List<IssueVO> getLog(Properties ctx, String userIni, String userFin, String fechaIni, String fechaFin, String trxName) {
		StringBuilder st = new StringBuilder("select s.ad_issue_id as id, s.created, u.name, s.local_host, s.issueSummary, s.stackTrace from AD_Issue s ");
		...
		List<IssueVO> lista = new ArrayList<IssueVO>();
		...
		try {
			...
			while (rs.next()) {
				...
				vo.setIssueSummary(rs.getString("issueSummary"));
				vo.setLocalHost(rs.getString("local_host"));
				vo.setStackTrace(rs.getString("stackTrace"));
				vo.setUserN(rs.getString("name"));
				lista.add(vo);
			}
			rs.close();
			pstmt.close();
