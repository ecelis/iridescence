 ...
 */
public class MLanguage extends X_AD_Language implements Selectable
{
	...
	public static List<LabelValueBean> get(Properties ctx) {
		...
		sql.append("SELECT ");
		sql.append("  ad_language_id, ");
		sql.append("  descpreflanguage ");
		sql.append("FROM ");
		sql.append("  ad_language ");
		sql.append("WHERE ");
		sql.append("  PrefLanguage = 'Y' AND ");
		sql.append("  isActive = 'Y' ");
		sql = new StringBuilder(MEXMELookupInfo.addAccessLevelSQL(ctx, sql.toString(), "AD_Language"));
		sql.append("  order by DescPrefLanguage ASC");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			while (rs.next()) {
				lst.add(new LabelValueBean(rs.getString(2), rs.getString(1)));
			}
			rs.close();
			pstmt.close();
			pstmt = null;
	 ...
	 * @param ctx
	 *            Contexto de la app
	 * @return Listado de Lenguajes
	 */
	public static List<MLanguage> getLanguages(Properties ctx) {
		List<MLanguage> lst = new ArrayList<MLanguage>();
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT ");
		sql.append("  * ");
		sql.append("FROM ");
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				lst.add(new MLanguage(ctx, rs, null));
			}
			rs.close();
			pstmt.close();
			pstmt = null;
