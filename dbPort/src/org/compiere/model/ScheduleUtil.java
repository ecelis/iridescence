...
public class ScheduleUtil
{
	...
	//	log.fine( "- NonBusinessDay -");
		...
		StringBuilder objSql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY);
		objSql.append("SELECT Name, Date1 FROM C_NonBusinessDay ");
		objSql.append("WHERE ");
		
		if (DB.isOracle()) {
			objSql.append("TRUNC(Date1) ");
		} else if (DB.isPostgreSQL()) {
			objSql.append("DATE_TRUNC('day', Date1) ");
		}
		
		
		
		objSql.append("BETWEEN ? AND ?");
				
		sql = MRole.getDefault(m_ctx, false).addAccessSQL (
				objSql.toString(), "C_NonBusinessDay", false, false);	// not qualified - RO
		try
		{
	...
	//		log.fine( sql, "Start=" + startDay + ", End=" + endDay);
			PreparedStatement pstmt = DB.prepareStatement(objSql.toString(), trxName);
		...
		{
			log.log(Level.SEVERE, objSql.toString(), e);
			ma = new MAssignmentSlot (EARLIEST, LATEST,
