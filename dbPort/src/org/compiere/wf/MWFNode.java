...
public class MWFNode extends X_AD_WF_Node
{
	...
	public boolean isUserManual()
	{
		if (ACTION_UserForm.equals(getAction())
			...
			/*|| ACTION_UserWorkbench.equals(getAction()) Merge ADempiere trunk_rev14653*/)
			return true;
		return false;
	}	//	isUserManual
	 ...
	 * 	Get Workflow
	 *	@return workflow
	 * @deprecated please use {@link #getAD_Window()} - Merge ADempiere trunk_rev14653
	 */
...
//		}
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		if (!success)
			return success;
		// TranslationTable.save(this, newRecord); Merge ADempiere trunk_rev14653
		return true;
	}	//	afterSave
	...
	protected boolean afterDelete (boolean success)
	{
		...
		//	TranslationTable.delete(this); // Merge ADempiere trunk_rev14653
		return success;
	}	//	afterDelete
	...
	/** Gets tooltip for each node. Expert:Lama*/
	public String getTooltipText() {
		final StringBuffer tooltip = new StringBuffer();
		final String desc = getDescription(true);
		if(StringUtils.isNotEmpty(desc)){
			tooltip.append(desc);
			tooltip.append(". ");
		}
		final String help = getHelp(true);
	...
	public static boolean isNodeResponsible(MWFNode node, int adUserID){
		boolean isResp = false;
		
		if(MWFResponsible.RESPONSIBLETYPE_Human.equals(node.getAD_WF_Responsible().getResponsibleType())){
			isResp = node.getAD_WF_Responsible().getAD_User_ID() == adUserID;
		}else if(MWFResponsible.RESPONSIBLETYPE_Role.equals(node.getAD_WF_Responsible().getResponsibleType())){
			MUserRoles[] roles = MUserRoles.getOfUser(Env.getCtx(), adUserID);
			if(roles!=null){
				List<MUserRoles> rolesLst = Arrays.asList(roles);
				for(MUserRoles role: rolesLst){
					if(node.getAD_WF_Responsible().getAD_Role_ID() == role.getAD_Role_ID()){
						isResp = true;
						break;
					}
				}
			}
		}else if(MWFResponsible.RESPONSIBLETYPE_RoleType.equals(node.getAD_WF_Responsible().getResponsibleType())){
			MUserRoles[] roles = MUserRoles.getOfUser(Env.getCtx(), adUserID);
			if(roles!=null){
				List<MUserRoles> rolesLst = Arrays.asList(roles);
				for(MUserRoles role: rolesLst){
					if(node.getAD_WF_Responsible().getUserType().equals(role.getAD_Role().getUserType())){
						isResp = true;
						break;
					}
				}
