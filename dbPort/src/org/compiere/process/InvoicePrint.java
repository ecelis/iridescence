...
public class InvoicePrint extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		StringBuffer sql = new StringBuffer (
			...
			+ " COALESCE(NVL(pcte.Invoice_PrintFormat_ID,bp.Invoice_PrintFormat_ID), dt.AD_PrintFormat_ID, pf.Invoice_PrintFormat_ID),"	//	4 
			...
			// Lama - Partner Cte
			+ " LEFT JOIN C_BPartner_Cte pcte ON (bp.C_BPartner_ID=pcte.C_BPartner_ID AND pcte.AD_Client_ID = ? )" //#1
		...
		else
		{
			...
			if (m_dateInvoiced_From != null && m_dateInvoiced_To != null)
			{
				...
				if (DB.isOracle()) {
					...
					sql.append("DATE_TRUNC('day', i.DateInvoiced) BETWEEN ");
				}
					sql.append(DB.TO_DATE(m_dateInvoiced_From, true)).append(" AND ")
			...
			else if (m_dateInvoiced_From != null)
			{
				...
				if (DB.isOracle()) {
					...
					sql.append("DATE_TRUNC('day', i.DateInvoiced) >= ");
				}
					sql.append(DB.TO_DATE(m_dateInvoiced_From, true));
				needAnd = true;
			}
			else if (m_dateInvoiced_To != null)
			{
				...
				if (DB.isOracle()) {
					...
					sql.append(DB.TO_DATE(m_dateInvoiced_To, true));
				needAnd = true;
			}
		...
		try
		{
			...
			stmt.setInt(1, Env.getAD_Client_ID(Env.getCtx()));//Lama
			...
			while (rs.next())
			{
				...
				if (printed)
				{
					StringBuffer sb = new StringBuffer ("UPDATE C_Invoice "
						+ "SET DatePrinted=").append(DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx()))).append(", IsPrinted='Y' WHERE C_Invoice_ID=")
						.append (C_Invoice_ID);
					//int no = 
