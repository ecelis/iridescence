 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: VPayPrint.java,v 1.8 2006/08/23 00:33:36 mrojas Exp $
 */
	...
	private void cmd_print()
	{
		...
		try {
			...
			if (rs.next()) {
				...
				log.info("VPayPrint.loadPaySelectInfo - No existe la cuenta");
			}
			rs.close();
			pstmt.close();
		} catch (SQLException e) {
			log.log(Level.SEVERE, "VPayPrint.loadPaySelectInfo", (Exception)e);
		}
		...
		//Expert:Juan Sistemas Comentado por Juan 9/oct/2004
		//if (somethingPrinted && ADialog.ask(m_WindowNo, this, "VPayPrintSuccess?"))
		if(true) // Agregado Juan
		{
			int lastDocumentNo = MPaySelectionCheck.confirmPrint (m_checks, m_batch,"");
			if (lastDocumentNo != 0)
			{
