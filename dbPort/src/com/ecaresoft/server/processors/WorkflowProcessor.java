...
public class WorkflowProcessor extends CompiereServer
{
	...
	protected void doWork ()
	{
		...
		pLog.setReference("#" + String.valueOf(p_runCount) 
			+ " - " + TimeUtil.formatElapsed(new Timestamp(p_startWork)));//TODO (revisar CompiereServer)
		pLog.save();
	}	//	doWork
			...
			try
			{
				...
				while (rs.next())
				{
					...
					countEMails += sendEmail (activity, "ActivityOverPriority",
						escalate, true);
					activity.setDateLastAlert(DB.getTimestampForOrg(getCtx()));//new Timestamp(System.currentTimeMillis()) Lama TimeZone changes
					activity.save();
					count++;
				}
				rs.close();
			...
			try
			{
				...
				while (rs.next())
				{
					...
					countEMails += sendEmail (activity, "ActivityInactivity",
						escalate, false);
					activity.setDateLastAlert(DB.getTimestampForOrg(getCtx()));//new Timestamp(System.currentTimeMillis()) Lama TimeZone changesnew Timestamp(System.currentTimeMillis()));
					activity.save();
					count++;
				}
				rs.close();
