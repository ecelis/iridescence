...
public class ImportReportLine extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		no = DB.executeUpdate(sql.toString(), get_TrxName());//1638
		log.fine("Reset=" + no);
		...
		//	Line Type - (S) C
		sql = new StringBuffer ("UPDATE I_ReportLine "
			+ "SET LineType=(SELECT VALUE FROM AD_REF_LIST WHERE AD_REFERENCE_ID = "
			+ X_PA_ReportLine.LINETYPE_AD_Reference_ID
			+ " AND NAME = I_ReportLine.LineType_Name)  "
			+ " WHERE LineType IS NULL  AND LineType_Name IS NOT NULL "
			+ " AND I_IsImported<>'Y' ").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.fine("Set LineType Default=" + no);//TODO Considerar la traducción

		// Calculation Type - A P R S
		sql = new StringBuffer ("UPDATE I_ReportLine "
			+ "SET CalculationType = (SELECT VALUE FROM AD_REF_LIST WHERE AD_REFERENCE_ID = "
			+ X_PA_ReportLine.CALCULATIONTYPE_AD_Reference_ID 
			+ " AND NAME = I_ReportLine.CalculationType_Name) "
			+ "WHERE CalculationType IS NULL AND CalculationType_Name IS NOT NULL "
			+ "AND I_IsImported<>'Y' ").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.config("Set CalculationTy pe=" + no);
		/********************/
...
//		log.fine("Sync from first Row of Line=" + no);
		...
		// Description, SeqNo, IsSummary, IsPrinted, LineType, CalculationType, AmountType, PostingType
		updateAll(clientCheck, "Description");
		updateAll(clientCheck, "SeqNo");
		updateAll(clientCheck, "IsSummary");
		updateAll(clientCheck, "IsPrinted");
		updateAll(clientCheck, "LineType");
		updateAll(clientCheck, "CalculationType");
		updateAll(clientCheck, "AmountType");
		updateAll(clientCheck, "PostingType");
		...
		//	Validate Optional Calculation Type - A P R S N
		sql = new StringBuffer ("UPDATE I_ReportLine "
			+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid CalculationType, ' "
			+ "WHERE CalculationType IS NOT NULL AND CalculationType NOT IN ('A','P','R','S','N')"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql.append("WHERE PA_ReportLine_ID IS NULL AND PA_ReportLineSet_ID IS NOT NULL"
			+ " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		try
		{
			StringBuilder sqlInsert = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY);
			
			sqlInsert.append("INSERT INTO PA_ReportLine ");
			sqlInsert.append("(PA_ReportLine_ID,PA_ReportLineSet_ID,");
			sqlInsert.append("AD_Client_ID,AD_Org_ID,IsActive,Created,CreatedBy,Updated,UpdatedBy,");
			sqlInsert.append("Name,SeqNo,IsPrinted,IsSummary,LineType)");
			sqlInsert.append("SELECT ?,PA_ReportLineSet_ID,");
			sqlInsert.append("AD_Client_ID,AD_Org_ID,'Y',"+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) +
					",CreatedBy,"+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ",UpdatedBy,");
			sqlInsert.append("Name,SeqNo,IsPrinted,IsSummary,LineType ");
			sqlInsert.append("FROM I_ReportLine ");
			sqlInsert.append("WHERE PA_ReportLineSet_ID=? AND Name=? ");
			
			if (DB.isOracle()) {
				sqlInsert.append("AND ROWNUM=1"); 
			}
								
			sqlInsert.append(clientCheck);
			
			if (DB.isPostgreSQL()) {
				sqlInsert = new StringBuilder(DB.getDatabase().addPagingSQL(sqlInsert.toString(), 1, 1));
			}
			...
			PreparedStatement pstmt_insertLine = DB.prepareStatement(
				sqlInsert.toString(), get_TrxName());
		...
		sql = new StringBuffer ("UPDATE PA_ReportLine r "
			+ "SET (Description,SeqNo,IsSummary,IsPrinted,LineType,CalculationType,AmountType,PostingType,Updated,UpdatedBy,OPER_1_ID,OPER_2_ID)="
			+ " (SELECT Description,SeqNo,IsSummary,IsPrinted,LineType,CalculationType,AmountType,PostingType,"+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ",UpdatedBy,OPER_1_ID,OPER_2_ID"
		...
		try
		{
			...
			PreparedStatement pstmt_updateSource = DB.prepareStatement
				...
				+ " (SELECT 'AC',C_ElementValue_ID,"+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ",UpdatedBy"
				+ " FROM I_ReportLine"
				+ " WHERE I_ReportLine_ID=?) "
			...
			PreparedStatement pstmt_setImported = DB.prepareStatement
				...
				+ " PA_ReportSource_ID=?, "
				+ " Updated="+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ", Processed='Y' WHERE I_ReportLine_ID=?", get_TrxName());
	...
	private int updateAll(String pclientCheck, String column){
		//		Copy/Sync from first Row of Line
		StringBuffer sql = new StringBuffer ("UPDATE I_ReportLine i "
				//Description, SeqNo, IsSummary, IsPrinted, LineType, CalculationType, AmountType, PostingType
				+ " SET "+column+" = "
				//Description, SeqNo, IsSummary, IsPrinted, LineType, CalculationType, AmountType, PostingType
				+ " ( SELECT " + column
				...
				+ "                                           AND i.PA_ReportLineSet_ID=iii.PA_ReportLineSet_ID) "
				+ "              )"
				+ " AND I_IsImported='N' ").append(pclientCheck);		//	 not if previous error
	...
	private void updateOper() {
		...
		sql = new StringBuffer ("UPDATE I_ReportLine "
				+ " SET OPER_1_ID  = ( "
				+ "     SELECT   tieneElId.PA_REPORTLINE_ID "
				+ " 	FROM     I_ReportLine tieneElId "
				+ " 	WHERE    tieneElId.NAME = I_ReportLine.oper_1_name AND tieneElId.PA_REPORTLINE_ID > 0 "
				+ "     AND      tieneElId.AD_Client_ID = " + m_AD_Client_ID
				+ "     GROUP BY tieneElId.PA_REPORTLINE_ID "
				+ " ) "
				+ " WHERE OPER_1_NAME is not null "
				+ " AND   I_IsImported='N' ").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.fine("Set OPER_1_ID =" + no);
		
		sql = new StringBuffer ("UPDATE I_ReportLine        "
				+ " SET OPER_2_ID  = (                      "
				+ "     SELECT   tieneElId.PA_REPORTLINE_ID "
				+ " 	FROM     I_ReportLine tieneElId     "
				+ " 	WHERE    tieneElId.NAME = I_ReportLine.oper_2_name AND tieneElId.PA_REPORTLINE_ID > 0 "
				+ "     AND      tieneElId.AD_Client_ID = " + m_AD_Client_ID
				+ "     GROUP BY tieneElId.PA_REPORTLINE_ID "
				+ " )                                       "
				+ " WHERE OPER_2_NAME is not null           "
				+ " AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.fine("Set OPER_2_ID =" + no);
		/*******************************/
	}
}	//	ImportReportLine
