		...
		try
		{
			...
			while (rs.next())
			{
				...
				//  -- Special Lookups --
						...
						|| AD_Reference_ID == DisplayType.Employment //Expert: Lama
				)
				{
					if (ColumnSQL.length() > 0)
					{
					...
					//Expert: Lama
					if (AD_Reference_ID == DisplayType.Employment)
					{
						table = "EXME_Employment";
						key = "EXME_Employment_ID";
						display = "Company||'.'";	//	in case Company is empty
						synonym = "Employment";
					}
					if (synonym == null)
						synonym = display;
