...
public class MOrderTax extends X_C_OrderTax
{
	...
	/**
	 *  Method to obtain the tax amount of the current Order
	 *  @param int c_order,c_tax_id
	 *  @return Double 
	 */
	public double getTaxAmount (int c_order, int c_tax_id)
	{
		Double amount=0.0;	
		StringBuilder sb = new StringBuilder("SELECT Taxamt ");		
		sb.append("FROM C_OrderTax ");
		sb.append("WHERE C_Order_id = ? and C_Tax_id = ?");
	...
	/**
	 *  Method to obtain the tax base amount of the current Order
	 *  @param int c_order,c_tax_id
	 *  @return Double 
	 */
	public double getTaxBaseAmount (int c_order, int c_tax_id)
	{
		Double bamount=0.0;		
		StringBuilder sb = new StringBuilder("SELECT TaxBaseAmt ");		
		sb.append("FROM C_OrderTax ");
		sb.append("WHERE C_Order_id = ? and C_Tax_id = ?");
		StringBuilder sql = new StringBuilder(MEXMELookupInfo.addAccessLevelSQL(getCtx(), sb.toString(),this.get_TableName()));
		PreparedStatement pstmt = null;
	...
	public boolean calculateTaxFromLines ()
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				// #08568, genera errores en factura proveedor
