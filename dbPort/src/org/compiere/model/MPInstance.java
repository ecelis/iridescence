...
public class MPInstance extends X_AD_PInstance
{
	...
	public void setAD_Process_ID (int AD_Process_ID)
	{
		...
		if (AD_Role_ID != 0)
		{
			...
			Boolean access = true; //role.getProcessAccess(AD_Process_ID);
	 ...
	 *	@return 
	 */
	public static int getIDPInstanceProcess(int processID, String trxName)
	{   CLogger s_log = CLogger.getCLogger(MPInstance.class);
	
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT AD_PInstance_ID FROM AD_PInstance WHERE AD_Process_ID = ? ")
		...
		try
		{
			...
			if (rs.next())
			{
				return rs.getInt("AD_PInstance_ID");
			}
		}
		catch (Exception e)
		{
			s_log.log(Level.SEVERE, "Error: " + e.getMessage() + " " + sql, e);
		}
		finally
		{
			DB.close(rs, pstmt);
