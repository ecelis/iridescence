 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MMeasureCalc.java,v 1.3 2006/07/30 00:51:03 jjanke Exp $
 */
public class MMeasureCalc extends X_PA_MeasureCalc
{
	...
	public String getSqlPI (MGoalRestriction[] restrictions, 
		...
		if (getDateColumn() != null 
			...
			if (reportDate == null) {
				reportDate = DB.getTimestampForOrg(getCtx()); 
			}
		...
		//		;
				...
				sb.append(" AND DATE_TRUNC('");
				sb.append(trunc);
				sb.append("', ");
				sb.append(getDateColumn());
				sb.append(")");
				sb.append(" = DATE_TRUNC('");
				sb.append(trunc);
				sb.append("', ");
				sb.append(DB.TO_DATE(reportDate));
				sb.append(")");
			}
		}
		//	date
		...
		log.log(Level.FINEST, "MMeasureCalc - getSqlPI: sql= " + sql);
	...
	public String getSqlBarChart (MGoalRestriction[] restrictions, 
		...
		if (getDateColumn() != null 
			...
			//	else if (MGoal.MEASUREDISPLAY_Day.equals(MeasureDisplay))
			//		;
				dateCol = "DATE_TRUNC('" + trunc + "', " + getDateColumn() + ") ";
			}
	...
	public MQuery getQuery(MGoalRestriction[] restrictions, 
		...
		if (getDateColumn() != null 
			...
			//		trunc = "D";
				...
				sql.append(" AND DATE_TRUNC('");
				sql.append(trunc);
				sql.append("', ");
				sql.append(getDateColumn());
				sql.append(") ");
				sql.append("= DATE_TRUNC('");
				sql.append(trunc);
				sql.append("', ");
				sql.append(DB.TO_DATE(date));
				sql.append(") ");
			}
		}
		...
		try	{
			...
			log.log (Level.SEVERE, "MMeasureCalc.getQuery: sql: " + finalSQL, e);
		} finally {
			try {
				if (pstmt != null) {
					pstmt.close ();
				...
				log.log (Level.SEVERE, "While closing objects", e);
			}			
		}
