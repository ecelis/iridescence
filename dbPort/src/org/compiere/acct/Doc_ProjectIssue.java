...
/**
 * Project Issue. Note: Will load the default GL Category. Set up a document
 * type to set the GL Category.
 * 
 * @author Jorg Janke
...
public class Doc_ProjectIssue extends Doc {
	...
	protected String loadDocumentDetails() {
		...
		m_line.setQty(m_issue.getMovementQty(), true); // sets Trx and Storage
														// Qty
	...
	public ArrayList<Fact> createFacts(MAcctSchema as) {
		...
		MProject project = new MProject(getCtx(), m_issue.getC_Project_ID(),
				null);
		...
		dr = fact.createLine(m_line, getAccount(acctType, as),
		...
		cr = fact.createLine(m_line, m_line.getAccount(acctType, as),
	...
	private BigDecimal getPOCost(MAcctSchema as) {
		...
		try {
			...
			if (rs.next()) {
				...
				log.warning("Not found for M_InOutLine_ID="
						+ m_issue.getM_InOutLine_ID());
			rs.close();
			pstmt.close();
			pstmt = null;
		} catch (Exception e) {
