 ...
 *
 *  @author  Jorg Janke
 *  @version $Id: VCreateFrom.java,v 1.2 2006/08/23 00:36:14 mrojas Exp $
 */
	...
	implements ActionListener, TableModelListener
{
	...
	protected void initBPartnerOIS (int C_BPartner_ID, boolean forInvoice)
	{
		...
		//Solo se necesitan las ordenes de compra que han sido posteadas, expert: eruiz
		StringBuffer sql = new StringBuffer("SELECT o.C_Order_ID,").append(display)
			.append(" FROM C_Order o "
			+ "WHERE o.C_BPartner_ID=? AND o.IsSOTrx='N' AND o.DocStatus IN ('CL','CO') AND o.Posted = 'Y'"
			+ " AND o.C_Order_ID IN "
	...
	protected void loadOrder (int C_Order_ID, boolean forInvoice)
	{
		...
		//
		sql.append(" WHERE l.C_Order_ID=? ");           //  #1 //expert:
		//Expert : Gisela Lee : para filtrar las lineas por org access
		sql = new StringBuffer(MRole.getDefault().addAccessSQL(sql.toString(), 
                	"l", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO));
		//Expert : Gisela Lee : para filtrar las lineas por org access
		sql.append("GROUP BY l.QtyOrdered,CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END, " //expert
