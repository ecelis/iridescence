...
public class MBPBankAccount extends X_C_BP_BankAccount {
	...
	{	
		...
		// El registro se puede crear desde la vantana Socio de Negocio
		// o desde la ventana Banco
		// si se crea por socio, debe crear la cuenta bancaria Cards #1624, #1623
		if(getC_Bank_ID() != 0 && getBankAccountType() != null && getAccountNo() != null){

			if(newRecord && getC_BankAccount_ID() <=0){
				// Entra cuando se crea por socio de negocio
				final MBankAccount bank = new MBankAccount(getCtx(), 0, get_TrxName());

				bank.setC_Bank_ID(getC_Bank_ID());
				bank.setBankAccountType(getBankAccountType());
				bank.setAccountNo(getAccountNo());
				bank.setDescription(getA_Name());
				bank.setC_Currency_ID(getC_Currency_ID());		
				bank.setCurrentBalance(Env.ZERO);
				bank.setCreditLimit(Env.ZERO);
				bank.setIsOwnBank(false);

				success = bank.save();
				
				if(success){
					// Guardamos la referencia
					setC_BankAccount_ID(bank.getC_BankAccount_ID());
				}
				
					
				
			} else if(!newRecord && (is_ValueChanged(COLUMNNAME_BankAccountType)
					|| is_ValueChanged(COLUMNNAME_AccountNo)
					|| is_ValueChanged(COLUMNNAME_C_Bank_ID))){
				// Solo debe entrar cuando se crea por socio de negocio (En teoria nunca cambian los datos)
				final MBankAccount bank = new MBankAccount(getCtx(), getC_BankAccount_ID(), get_TrxName());
		...
		} else if (newRecord && getC_BankAccount_ID() > 0) {
			// Entra cuando se crea por banco
			final MBankAccount bank = new MBankAccount(getCtx(), getC_BankAccount_ID(), get_TrxName());

			setC_Bank_ID(bank.getC_Bank_ID());
			setBankAccountType(bank.getBankAccountType());
			setAccountNo(bank.getAccountNo());
			setC_Currency_ID(bank.getC_Currency_ID());
		}// fin cambios cards
