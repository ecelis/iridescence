 ...
 *
 *  @author Jorg Janke
 *  @version $Id: POInfo.java,v 1.7 2006/08/23 01:32:38 mrojas Exp $
 */
public class POInfo implements Serializable
{
	...
	private String m_TablePrintName = null;
	...
	private void loadInfo (boolean baseLanguage, String trxName)
	{
		...
		sql.append("SELECT t.TableName, c.ColumnName,c.AD_Reference_ID,"    //  1..3
			...
			+ "c.IsAllowLogging,t.IsChangeLog, c.IsSearchColumn, "			//  21..23
			+ "t.name, c.name ");                                           //  24..25
		...
		try
		{
			...
			while (rs.next())
			{
				if (m_TableName == null) {
					...
					m_TablePrintName = rs.getString(24);
				}
				...
				boolean isSearchColumn = "Y".equals(rs.getString(23));
				
				String columnPrintName = rs.getString(25);
				
				POInfoColumn col = new POInfoColumn (
					AD_Column_ID, ColumnName, columnPrintName, ColumnSQL,
					AD_Reference_ID, IsMandatory,
					IsUpdateable, DefaultLogic, Name,
					Description, IsKey,
					IsParent, AD_Reference_Value_ID,
					ValidationCode, FieldLength, ValueMin,
					ValueMax, IsTranslated,
					IsEncrypted, IsAllowLogging, isSearchColumn);
				list.add(col);
			}
		}
	...
	/**
	 *  Get Table Print Name
	 *  @return Table Print Name
	 */
	public String getTablePrintName(){
		return m_TablePrintName;
	}
	...
	/**
	 * Es columna de busqueda
	 * 
	 * @param index
	 *            indice de columna
	 * @return
	 */
	public boolean isSearchColumn(int index) {
		return m_columns[index].isSearchColumn;
	}
	...
	/*
	 * Expert:Hassan - IsSelectionColumn. 
	 */
	public boolean isSelectionColumn(int index){
	    
		if (m_columns[index].IsSelectionColumn)
			return true;
		return false;
	...
	/**
	 * Hacemos visible el metodo para obtener la columna.
	 * @param idx
	 * @return
	 */
	public POInfoColumn getColumnInfo(int idx){
	...
	public StringBuffer buildSelect()
	{
		...
		StringBuffer sql = new StringBuffer();
		sql.append(buildSelect(false, false));
		sql.append(buildFrom());
		
		return sql;
	}
	
	public StringBuffer buildSelect(boolean blank, boolean multiTables)
	{
		StringBuffer sql = new StringBuffer(blank?" ":" SELECT ");
		...
		{
			if (i != 0)// apartir de la segunda columna 
				sql.append(",");

			if (StringUtils.isBlank(getColumn(i).ColumnSQL)) {
				sql.append(nickname == null ? getTableName() : nickname);
				sql.append(".");
			}
			String columnSql = getColumnSQL(i);
			sql.append(columnSql);	//	Normal and Virtual Column
			
			// alias column = nick table + _ + id column
			if(multiTables && nickname!=null){
				sql.append(" AS \"");	//	
				sql.append(nickname).append("_");
				sql.append(getAD_Column_ID(columnSql)).append("\"");
			}
		}
		return sql;
	}
	
	public StringBuffer buildFrom()
	{
		StringBuffer sql = new StringBuffer(" FROM ");
		sql.append(getTableName()).append(" ");
		sql.append(nickname!=null?nickname:"");
	 ...
	 * Key
	 */
	public POInfoColumn getKeyColumnInfo() {
		for (int i = 0; i < m_columns.length; i++)
		{
			if (m_columns[i].IsKey)
			{
				if(columnKey==null)
					columnKey = m_columns[i].ColumnName;
	...
	/**
	 * Columnas para Mostrar como parte del objeto
	 * @return
	 */
	public String[] getSearchColumns() {
		List<String> namesColumns = new ArrayList<String>();
		
		for (int i = 0; i < m_columns.length; i++)
		{
			if (m_columns[i].isSearchColumn || m_columns[i].IsSelectionColumn)
			{
				namesColumns.add(m_columns[i].ColumnName);
			}
		}
		
		String[] liststring = new String[namesColumns.size()];
		namesColumns.toArray(liststring);
		return liststring;
	}
	
	
	//TODO 
	private boolean multiTables;

	public boolean isMultiTables() {
		return multiTables;
	}

	public void setMultiTables(boolean multiTables) {
