...
public class Doc_Payment extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		//	Cash Transfer
//		if (X_C_Payment.TENDERTYPE_Cash.equals(m_TenderType))
//		{
//			ArrayList<Fact> facts = new ArrayList<Fact>();
//			facts.add(fact);
//			return facts;
//		}// Se comenta por que toda forma de pago debe ser posteada (GC)
		...
		else if (getDocumentType().equals(DOCTYPE_APPayment))
		{
			...
			} else if (isClient()) {// Card #1099 Lama
				acct = getAccount(Doc.ACCTTYPE_C_Prepayment, as);
			} else if (isEmployee()) {// Card #1317 
				acct = getAccount(Doc.ACCTTYPE_C_Prepayment_Employee, as);
			} else {
			...
			//	Asset
			int accType = 0;
			if (MPayment.PAYMENTTYPE_ACH.equals(pay.getPaymentType())) {
				accType = Doc.ACCTTYPE_BankAsset;
			} else {
				accType = Doc.ACCTTYPE_BankInTransit;
			}

			fl = fact.createLine(null, getAccount(accType, as),
				getC_Currency_ID(), null, getAmount());
			if (fl != null && AD_Org_ID != 0)
				fl.setAD_Org_ID(AD_Org_ID);
		}
	...
	public static boolean isClient(MPayment pay) {
		return pay != null && pay.getC_BPartner_ID() > 0 //
			&& DB.getSQLValue(null, new StringBuilder("SELECT DISTINCT COALESCE(")//
				.append("(SELECT C_BPartner_ID FROM EXME_ConfigEC ")//
				.append("WHERE AD_Org_ID=? AND C_BPartner_ID=bp.C_BPartner_ID)")//
				.append(",(SELECT C_BPartner_ID FROM EXME_ConfigPre ")//
				.append("WHERE AD_Org_ID=? AND C_BPartner_ID=bp.C_BPartner_ID)")//
				.append(",0) as client FROM C_BPartner bp ")//
				.append("WHERE bp.C_BPartner_ID=? ")//
				.append("AND bp.IsCustomer='Y' AND bp.IsVendor='N' ").toString(),//
				pay.getAD_Org_ID(), pay.getAD_Org_ID(), pay.getC_BPartner_ID()) > 0;
	}
	/**
	 * Card #1099 - Corrección en Devolución de Anticipos Asiento contable
	 * @return si es Cliente (no proveedor) y esta en la configuracion de EC y Pre
	 * @author lama
	 */
	...
	/**
	 * Card #1317 - Modificar el proceso de Contabilización de pagos CxP
	 * @return si es Empleado (deudor diverso)
	 * @author lama
	 */
	public boolean isEmployee() {
		return pay.getBPartner().isEmployee() && !pay.getBPartner().isVendor();
	}
