...
public class InfoCashLine extends Info
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7127607610745081803L;
	...
	String getSQLWhere()
	{
		...
		if (fDateFrom.getValue() != null || fDateTo.getValue() != null)
		{
			...
			if (DB.isOracle()) {
				...
				if (from == null && to != null)
					sql.append(" AND DATE_TRUNC('day', c.StatementDate) <= ?");
				else if (from != null && to == null)
					sql.append(" AND DATE_TRUNC('day', c.StatementDate) >= ?");
				else if (from != null && to != null)
					sql.append(" AND DATE_TRUNC('day', c.StatementDate) BETWEEN ? AND ?");
			}
		}
		//
