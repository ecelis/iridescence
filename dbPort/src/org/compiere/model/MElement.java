...
public class MElement extends X_C_Element
{
	...
	/**
	 * Gets the AD_Element.Help / Description according to the column name
	 * 
	 * @param ctx
	 * @param columnName
	 * @return translated column tooltip message
	 */
	public static String getTooltip(final Properties ctx, final String columnName) {
		String retValue;
		if (Language.isBaseLanguage(Env.getAD_Language(ctx))) {
			retValue = DB.getSQLValueString(null, "SELECT COALESCE(Help,Description) FROM AD_Element WHERE ColumnName=?", columnName);
		} else {
			retValue = DB.getSQLValueString(null, new StringBuffer("SELECT COALESCE(t.Help,t.Description) ")//
				.append(" FROM AD_Element e INNER JOIN AD_Element_Trl t ")//
				.append(" ON ( e.AD_Element_ID=t.AD_Element_ID AND t.AD_Language=? ) ")//
				.append(" WHERE e.ColumnName=? ").toString(), Env.getAD_Language(ctx), columnName);
		}
		return retValue;
	} // getTooltip
