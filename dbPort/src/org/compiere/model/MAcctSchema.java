...
public class MAcctSchema extends X_C_AcctSchema
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7536086052486772998L;
	...
	public static MAcctSchema[] getClientAcctSchema (Properties ctx, int AD_Client_ID, String trxName)
	{
		...
		//mrojas : como no se recibe un nombre de transaccion desde la importacion
		//de productos, nunca carga el esquema contable (correccion tomada de
		//Adempiere)
		...
		// Considerar que system tiene un esquema contable
		if (AD_Client_ID >= 0) { 
	...
	public MAcctSchema (Properties ctx, int C_AcctSchema_ID, String trxName)
	{
		...
		//	setName (null);
			...
			//22 Nov 2013. Revisado con Gerardo Camargo. Jesus Cantu.
			//Los esquemas ahora serán de Tipo Promerdio.
			//setCostingMethod (COSTINGMETHOD_StandardCosting);
			setCostingMethod (COSTINGMETHOD_AveragePO);
	...
	public boolean isCreateCommitment()
	{
		...
		return COMMITMENTTYPE_CommitmentOnly.equals(s)
			...
			|| COMMITMENTTYPE_ReservedCommittedAndEarned.equals(s)
			//|| COMMITMENTTYPE_ReservedCommittedAndExercisedDue.equals(s)//No se encuentra en la lista de referencia .- Lama
			;
		    // CAMBIO DE CONDICION
	}	//	isCreateCommitment
	...
	public boolean isCreatePOCommitment()
	{
		...
		if (s == null)
			return false;
		return COMMITMENTTYPE_CommitmentOnly.equals(s) //TODO : revisar al generar modelos
			|| COMMITMENTTYPE_CommitmentReservation.equals(s) //TODO : revisar al generar modelos
	 ...
	 */
	public static int loadDefaultAcctSchema ()
	{
		//
	 ...
	 * @return un entero que representa el C_Currency_ID encontrado.
	 * @author jcantu Creado el 20 de Junio del 2012.
	 */
	public static int getCurrencyFromSchema(String saTrxName, 
			int iaAd_Client_id, int iaAD_Org_id) {
		
		StringBuilder sql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY); 
		sql.append("Select C_Currency_ID from C_AcctSchema where AD_Client_id = ?"); 
		sql.append(" and AD_Org_ID = ? And isactive='Y' ");
	  ...
	  * @return
	  */
	public static int getClientSchema(int clientId, String saTrxName) {
		
		StringBuilder sql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY); 
		sql.append("Select C_Acctschema_Id from C_AcctSchema where AD_Client_id = ?"); 
		sql.append(" And isactive='Y' ");
		return DB.getSQLValue(saTrxName, sql.toString(), clientId);
		
	}
	
	/**
	 * Revisa si el esquema actual maneja o no contabilidad electronica
	 * 
	 * @param ctx
	 *            Contexto
	 * @return Si/No maneja contabilidad electronica
	 */
	public static boolean useElectronicAccounting(Properties ctx) {
		boolean retValue = false;

		MAcctSchema acctSchema = new MAcctSchema(ctx, Env.getC_AcctSchema_ID(ctx), null);

		if (acctSchema != null && acctSchema.getC_AcctSchema_ID() > 0) {
			retValue = acctSchema.isElectronicAccounting();
		}
