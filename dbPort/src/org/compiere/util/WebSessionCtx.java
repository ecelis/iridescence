 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: WebSessionCtx.java,v 1.4 2006/08/23 00:54:39 mrojas Exp $
 */
public class WebSessionCtx implements Serializable, HttpSessionBindingListener 
{
	/**
	...
	public static WebSessionCtx get (HttpServletRequest request)
	{
		...
		if (wsc == null)
		{
			wsc = new WebSessionCtx (session); //expert : mrojas : por integracion con zk
			session.setAttribute(NAME, wsc);
		}
		
		wsc.ctx = (Properties)request.getSession().getAttribute("WebUISessionContext");
		
		return wsc;
	}	//	get
	
	//expert : mrojas : por integracion zk
	/**
	 * 	Get/create Web Session Context
	 *	@param request request
	 *	@return ctx or null
	 */
	public static WebSessionCtx get (HttpSession session)
	{
		if (session == null)
			return null;
		WebSessionCtx wsc = (WebSessionCtx)session.getAttribute(NAME);
	...
	public static WebSessionCtx get (HttpServletRequest request, String user, String pass)
	{
		HttpSession session = request.getSession(false);
		if (session == null)
			session = request.getSession(true);
	...
	private WebSessionCtx (HttpServletRequest request)
	{
		...
		//expert : mrojas : integrado en zk
		//ctx = getDefaults (); 
		ctx = (Properties)request.getSession().getAttribute("WebUISessionContext");
		//expert : mrojas : integrado en zk
	 ...
	 * @param session
	 */
	private WebSessionCtx (HttpSession session)
	{
		...
		//	ServerContext	- dev2/wstore
		ctx.put(CTX_SERVER_CONTEXT, session.getServletContext().getServerInfo());
		//	Make Context directly availabe to jsp's
		session.setAttribute("ctx", ctx);
		//
		log.fine("#" + ctx.size());
	...
	private void setLanguage (Language lang)
	{
		...
		//expert : miguel rojas
		timeFormat = DisplayType.getDateFormat(DisplayType.Time, language);
		//expert : miguel rojas
		//
	...
	//expert : miguel rojas : para detectar eventos de expiracion de sesion
	 ...
	 */
	 public void valueUnbound(HttpSessionBindingEvent arg0) {
		 // esta a punto de destruirse la sesion, 
		 //verificar si es un timeout o una salida normal
		 String modoSalida = ctx.getProperty("#SalidaNormal");
		 
		 if(modoSalida == null) {
			 Login login = new Login(ctx);
			 login.expiroSesion(arg0.getSession().getId());
		 }
	 }
	 //expert : miguel rojas : para detectar eventos de expiracion de sesion
