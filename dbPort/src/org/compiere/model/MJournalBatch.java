...
public class MJournalBatch extends X_GL_JournalBatch implements DocAction
{
	...
	private static final long serialVersionUID = -2247591522327956331L;
	...
	public int copyDetailsFrom (MJournalBatch jb)
	{
		...
		for (int i = 0; i < fromJournals.length; i++)
		{
			...
			//toJournal.getC_Period_ID(0);//	Corregido por Armando IVan Garcia causa error debido a que setea
			//el C_Period_ID en ceros lo cual causa que en la validacion dentro del metodo cause error, ya que no 
			//puede ser <= 0
			...
			for (MJournal journal : journals) {
				if (journal.voidIt()) {
					if (journal.save(get_TrxName())) {
						MEXMEPolicyInfo policyInfo = MEXMEPolicyInfo.get(getCtx(), MJournal.Table_ID, journal.getGL_Journal_ID(), 0, null);
	...
	/**
	 * Afert save
	 * Para provision de ventas se agrego esta validacion para actualizar la secuencia del numero
	 * de documento si este fue modificado por el usuario
	 * @param newRecord
	 * @return true
	 */
	protected boolean afterSave(boolean newRecord, boolean success) {
		if(success){
			MDocType type = new MDocType(getCtx(),getC_DocType_ID(),null);
			MSequence seq = new MSequence(getCtx(),type.getDocNoSequence_ID(),get_TrxName());
			if(seq != null){
				try{
					//Si el numero de secuencia es menor al brindado por el usuario
					//lo cambiamos
					if(seq.getCurrentNext() <= Integer.parseInt(getDocumentNo())){
						seq.setCurrentNext(Integer.parseInt(getDocumentNo())+1);
