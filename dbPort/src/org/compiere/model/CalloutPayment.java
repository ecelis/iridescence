...
public class CalloutPayment extends CalloutEngine
{
	...
	public String invoice (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if(value instanceof BigDecimal)
			C_Invoice_ID = Integer.valueOf(value.toString());
		...
		if (Env.getContextAsInt(ctx, Env.WINDOW_INFO, Env.TAB_INFO, "C_Invoice_ID") == C_Invoice_ID.intValue()){
			if( Env.getContextAsInt(ctx, Env.WINDOW_INFO, Env.TAB_INFO, "C_InvoicePaySchedule_ID") != 0){
	...
	{
		Integer C_Project_ID = null;
		if(value instanceof BigDecimal)
			C_Project_ID = Integer.valueOf(value.toString());
		else if(value instanceof Integer)
			C_Project_ID = (Integer)value;
		if (isCalloutActive()		//	assuming it is resetting value
	...
	public String docType (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (C_DocType_ID != 0)
		{
			...
			final MWindow mMWindow = new MWindow (ctx, WindowNo, null);
			Env.setContext(ctx, WindowNo, "IsSOTrx", mMWindow.isSOTrx() ? "Y" : "N");
		}
		//	Invoice
		if (C_Invoice_ID != 0)
		{
	...
	public String encounter (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue)
	{
		if (isCalloutActive())		//	assuming it is resetting value
			return "";
		setCalloutActive(true);
	...
	public String patient (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue)
	{
		if (isCalloutActive())		//	assuming it is resetting value
			return "";
		setCalloutActive(true);
	...
	public String tenderType (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		setCalloutActive(true);
		if(X_C_Payment.TENDERTYPE_Cash.equals(tenderT)){
			mTab.setValue("TenderType", X_C_Payment.TENDERTYPE_Check);
			//mField.setOldValue(X_C_Payment.TENDERTYPE_Check);
		}
		setCalloutActive(false);
		return "";
	}	//	project
