...
public class MPaymentTerm extends X_C_PaymentTerm {
	...
	private static CLogger s_log = CLogger.getCLogger(MPaymentTerm.class);
	/**
	...
	public MPaySchedule[] getSchedule(boolean requery) {
		if (m_schedule == null || requery) {
			ArrayList<MPaySchedule> list = getScheduleList();
	 ...
	 *	@return array of schedule
	 */
	private ArrayList<MPaySchedule> getScheduleList() {
		...
		try {
			...
			log.log(Level.SEVERE, "getSchedule"+sql, e);
		} finally {
			DB.close(rs, pstmt);
		}
		return list;
	...
	/**
	 *  Method to obtain the Payment Terms	
	 * @param ctx  Contexto
	 * @param trxName Transaccion
	 * @return List<KeyNamePair>
	 */
	public static List<KeyNamePair> getPaymentTerms(Properties ctx, String trxName) {
		final StringBuilder sql = new StringBuilder();
		sql.append("SELECT C_Paymentterm_id,Name ");
		sql.append("FROM C_Paymentterm ");
	 ...
	 *	@return true if payment schedule is valid
	 */
	private boolean applySchedule(final MInvoice invoice) {
		deleteInvoicePaySchedule(invoice.getC_Invoice_ID(), invoice.get_TrxName());
		// Create Schedule
		...
		for (int i = 0; i < m_schedule.length; i++) {
			...
			if (ips.save(invoice.get_TrxName())) {
				...
				log.severe("Imposible PaySchedule doc:" + invoice.getDocumentNo());
			}
		} // for all schedules
		...
		return invoice.validatePaySchedule(invoice.get_TrxName());
	} // applySchedule
	...
	public String toString() {
		...
		sb.append(get_ID()).append("-").append(getName()).append(",Valid=").append(isValid()).append("]");
		return sb.toString();
	} // toString
	...
	/**
	 * Obtiene el termino de pago del contexto, si no, el default
	 * 
	 * @param ctx
	 *            Contexto
	 * @return Termino de pago
	 */
	public static int getDefaultPaymentTerm(Properties ctx) {
		int iiPayTerm = Env.getContextAsInt(ctx, "#C_PaymentTerm_ID");
		if (iiPayTerm <= 0) {
			String sql = "SELECT C_PaymentTerm_ID FROM C_PaymentTerm WHERE AD_Client_ID=? AND IsDefault='Y'";
			iiPayTerm = DB.getSQLValue(null, sql, Env.getAD_Client_ID(ctx));
		}
		return iiPayTerm;
	}
	
	/**
	 * Payment term por value
	 * @param ctx
	 * @param name
	 * @param trxName
	 * @return
	 */
	public static MPaymentTerm get(Properties ctx, String name, String trxName) {
		return new Query(ctx, Table_Name, " value=? ", trxName)//
	 ...
	 * @return
	 */
	public static MPaymentTerm getDefault(Properties ctx, String trxName) {
		return new Query(ctx, Table_Name, " IsDefault='Y' ", trxName)//
			.setOnlyActiveRecords(true) //
			.addAccessLevelSQL(true)//
			.setOrderBy(" C_PaymentTerm_ID ASC ")//
			.first();
	}
