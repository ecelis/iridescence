 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: InventoryValue.java,v 1.5 2006/08/23 00:51:13 mrojas Exp $
 */
public class InventoryValue extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			/*Expert:Fin Alejandro Organizacion*/
		}	
		if (p_DateValue == null)
			p_DateValue = DB.getTimestampForOrg(Env.getCtx());
	}   //  prepare
	...
	protected String doIt() throws Exception
	{
		...
		sql = new StringBuffer ("INSERT INTO T_InventoryValue "
			...
			//Expert:Alejandro Organizacion
			+ " INNER JOIN AD_Org org ON (org.ad_org_id = w.ad_org_id)"
			+ " WHERE c.Ad_Org_ID = ").append(p_AD_Org_ID)
			//Expert:Alejandro Fin Organizacion
			.append(" AND w.M_Warehouse_ID=").append(p_M_Warehouse_ID) ; 
		...
		sql = new StringBuffer("UPDATE T_InventoryValue iv ");
		sql.append("SET PricePO = ");
			sql.append("(SELECT currencyConvert (po.PriceList,po.C_Currency_ID,iv.C_Currency_ID,iv.DateValue,null, po.AD_Client_ID,po.AD_Org_ID)");
			sql.append(" FROM M_Product_PO po WHERE po.M_Product_ID=iv.M_Product_ID");
			sql.append(" AND po.IsCurrentVendor='Y' ");
		...
		sql.append("PriceList = ");
			sql.append("(SELECT currencyConvert(pp.PriceList,pl.C_Currency_ID,iv.C_Currency_ID,iv.DateValue,null, pl.AD_Client_ID,pl.AD_Org_ID)");
			sql.append(" FROM M_PriceList pl, M_PriceList_Version plv, M_ProductPrice pp");
			sql.append(" WHERE pp.M_Product_ID=iv.M_Product_ID AND pp.M_PriceList_Version_ID=iv.M_PriceList_Version_ID");
			sql.append(" AND pp.M_PriceList_Version_ID=plv.M_PriceList_Version_ID");
			sql.append(" AND plv.M_PriceList_ID=pl.M_PriceList_ID), ");
		sql.append("PriceStd = ");
			sql.append("(SELECT currencyConvert(pp.PriceStd,pl.C_Currency_ID,iv.C_Currency_ID,iv.DateValue,null, pl.AD_Client_ID,pl.AD_Org_ID)");
			sql.append(" FROM M_PriceList pl, M_PriceList_Version plv, M_ProductPrice pp");
			sql.append(" WHERE pp.M_Product_ID=iv.M_Product_ID AND pp.M_PriceList_Version_ID=iv.M_PriceList_Version_ID");
			sql.append(" AND pp.M_PriceList_Version_ID=plv.M_PriceList_Version_ID");
			sql.append(" AND plv.M_PriceList_ID=pl.M_PriceList_ID), ");
		sql.append("PriceLimit = ");
			sql.append("(SELECT currencyConvert(pp.PriceLimit,pl.C_Currency_ID,iv.C_Currency_ID,iv.DateValue,null, pl.AD_Client_ID,pl.AD_Org_ID)");
			sql.append(" FROM M_PriceList pl, M_PriceList_Version plv, M_ProductPrice pp");
			sql.append(" WHERE pp.M_Product_ID=iv.M_Product_ID AND pp.M_PriceList_Version_ID=iv.M_PriceList_Version_ID");
			sql.append(" AND pp.M_PriceList_Version_ID=plv.M_PriceList_Version_ID");
			sql.append(" AND plv.M_PriceList_ID=pl.M_PriceList_ID)");
		...
		no = DB.executeUpdate("UPDATE T_InventoryValue SET "
            ...
            //expert : gl : actualizar el esquema contable
            + ", C_AcctSchema_ID = " + m_C_AcctSchema_ID + " "
            //expert : gl : actualizar el esquema contable
			+ "WHERE AD_PInstance_ID=" + getAD_PInstance_ID()
			, get_TrxName());
