...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	 ...
	 * @param confirmPanel TODO
	 */
	public GridTab(GridTabVO vo, POConfirmPanel confirmPanel)
	{
		m_vo = vo;
		//  Create MTable
		...
		/* Expert: Lama */|| isReadOnly());
		m_mTable.setDeleteable(m_vo.IsDeleteable);
		m_mTable.setConfirmPanel(confirmPanel);
		//  Load Tab
		//initTab(false);
	}	//	GridTab
	...
	private boolean loadFields()
	{
		...
		for (int f = 0; f < m_vo.getFields().size(); f++)
		{
			...
			if (voF != null)
			{
				...
				field.setGridTab(this);// Lama: Merge con Adempiere
	...
	public void query(boolean onlyCurrentRows, int onlyCurrentDays, int maxRows) {
		...
		// is it same query?
		boolean refresh =
			m_oldQuery.equals(m_query.getWhereClause()) && m_vo.onlyCurrentRows == onlyCurrentRows && m_vo.onlyCurrentDays == onlyCurrentDays;
	...
	/**************************************************************************
	 * Refresh all data
	 * @param fireStatusEvent // Merge ADempiere trunk_rev14653
	 */
	public void dataRefreshAll (boolean fireStatusEvent)
	{
		...
		m_mTable.dataRefreshAll(fireStatusEvent);
		...
		setCurrentRow(m_currentRow, fireStatusEvent);
		if (fireStatusEvent) {// Merge ADempiere trunk_rev14653
	...
	public int navigate (int targetRow)
	{
		...
		if (!m_mTable.isReadOnly())//Expert: Lama
		...
		//  new position
		 int currrow = setCurrentRow(newRow, true);
		 patientLog();// Expert: Lama
		 return currrow;
	}   //  navigate
...
//					value = mField.getDefault();
	...
	public String processCallout(GridField field)
	{
		...
		// Expert : Lama
		if(!field.getCalloutMap().isEmpty()){
			Iterator<Entry<String, Callout>> it = field.getCalloutMap().entrySet().iterator();
			while (it.hasNext()) {
				Entry<String, Callout> element = it.next();
				String cmd = element.getKey();
				// detect infinite loop
				if (activeCallouts.contains(cmd))
					continue;
				final String msg = processCall(field, cmd, value, oldValue, element.getValue());// Expert: Lama
			...
			while (st.hasMoreTokens()) // for each callout
			{
				...
				if (activeCallouts.contains(cmd))
					continue;
				final String msg = processCall(field, cmd, value, oldValue, null);// Expert: Lama 
				if (StringUtils.isNotBlank(msg)) {
					return msg;
				}
			} // for each callout
	...
	private String processCall(GridField field, String cmd, Object value, Object oldValue, Callout call) {
		String retValue = "";
		// FR [1877902]
	 ...
	 * @param prev
	 */
	public void findBlck(boolean prev) {
		m_mTable.m_prev = prev;
		m_mTable.m_next = !prev;
		block = true;
	}

	/**
	 * for tables configured as High Volume
	 * 
	 * @author Lorena Lama
	 */
	public void highVolume() {
		m_mTable.m_maxHighVolRows = MEXMEMejoras.getMaxQueryRecords(m_vo.ctx);
		if (!m_vo.IsHighVolume)
			return;
		// 
		m_mTable.m_IsHighVolume = m_vo.IsHighVolume;
		log.log(Level.CONFIG, "High Volume Table", m_mTable.getTableName());
		if (!block)
			m_mTable.reset();// Expert: Lorena Lama
	}

	/**
	 * @author Lorena Lama
	 */
	public GridTable getMTableModel() {
		return m_mTable;
	}
	...
	private void patientLog() {
		try {
			...
			if (getTabNo() == 0) {
				// EXME_Paciente table
				if (m_mTable.getTableName().equals(MEXMEPaciente.Table_Name) && m_keyColumnName != null) {
					// saved record
					Object obj = getValue(m_keyColumnName);
					if (obj != null) {
						MEXMEPatientLog log = new MEXMEPatientLog(m_vo.ctx, null);
						log.saveLog((Integer) obj, MEXMEPatientLog.EVENTCHANGELOG_View, MEXMEPatientLog.ACTION_Window, m_vo.AD_Window_ID);
					}
				}
			}
		} catch (Exception e) {
			log.log(Level.SEVERE, "patientLog", e);
		}
	}
