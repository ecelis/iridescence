...
public class Doc_Allocation extends Doc
{
	...
	//MAllocationLine[1000463,C_Payment_ID=1000461,C_Invoice_ID=1000365,C_BPartner_ID=10004933, Amount=-14500.0000000,Discount=0E-7,WriteOff=0E-7,OverUnder=0]
	public BigDecimal utilidadPerdidaCambiaria(DocLine_Allocation line, MInvoice invoice){
		BigDecimal amtDiff  = Env.ZERO;// De pesos a dolar //$1160
		if(invoice!=null){
			final MAllocationLine mAllocation = new MAllocationLine(getCtx(), line.get_ID(), null);

			BigDecimal rateInv  = Env.ZERO;
			if(invoice.getRate().compareTo(Env.ZERO)>0){
				rateInv = invoice.getRate();
			} else {
				rateInv = MConversionRate.getRate(
						invoice.getC_Currency_ID()
						, getC_Currency_ID()
						, invoice.getDateInvoiced()
						, invoice.getC_ConversionType_ID()
						, invoice.getAD_Client_ID()
						, invoice.getAD_Org_ID() );//12.5
			}
			
			BigDecimal rateAlloc;
			if(mAllocation.getRate().compareTo(Env.ZERO)>0){
				rateAlloc = mAllocation.getRate();//12.5
			} else {
				rateAlloc = MConversionRate.getRate(
						mAllocation.getPayment().getC_Currency_ID()
						, getC_Currency_ID()
						, mAllocation.getC_AllocationHdr().getDateAcct()
						, mAllocation.getPayment().getC_ConversionType_ID()
						, mAllocation.getAD_Client_ID()
						, mAllocation.getAD_Org_ID() );
			}
			...
			if(rateAlloc == null){
				...
				//14500 - 12.5 || 21750.000000
				BigDecimal amt      = mAllocation.getAmount().abs().multiply(Env.ONE.divide(rateAlloc,6,BigDecimal.ROUND_HALF_UP ));
				
				BigDecimal rateDiff = Env.ZERO;
				if(invoice.isSOTrx()){
					rateDiff = rateAlloc.subtract(rateInv);
				} else {
					rateDiff = rateInv.subtract(rateAlloc);
				}
				amtDiff = rateDiff.multiply(amt);
			}
			
		}
		return amtDiff.setScale(2, BigDecimal.ROUND_HALF_UP);
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			else if (invoice.isSOTrx())
			{
				//
				// Charge		    DR
				if ( line.getC_Charge_ID()>0 && MCharge.createFactCharge(getCtx(), line.getC_Charge_ID()) && Env.ZERO.compareTo(line.getAmtSourceDr()) != 0) //setAmount()- m_AmtSourceDr
				{
					MAccount acct = null;
					if (line.getC_Charge_ID() != 0)
						acct = MCharge.getAccount(line.getC_Charge_ID(), as, line.getAmtSourceDr());
					
					fl = fact.createLine (line, acct,
							getC_Currency_ID(), line.getAmtSourceDr(), null);//
					if (fl != null && payment != null)
						fl.setAD_Org_ID(payment.getAD_Org_ID());
					...
					|| getC_Currency_ID() != line.getInvoiceC_Currency_ID()))	//	allocation <> invoice currency
			{
				p_Error = createRealizedGainLoss (as, fact, bpAcct, invoice, 
					allocationSource, allocationAccounted, line);
				if (p_Error != null)
					return null;
			}
	...
	private MAccount getPaymentAcct(MAcctSchema as, int C_Payment_ID) {
		...
		sql.append("SELECT p.C_BankAccount_ID, d.DocBaseType, p.IsReceipt, p.IsPrepayment ");
		...
		try {
			...
			if (rs.next()) {
				...
				if ("Y".equals(rs.getString(4))) { // Prepayment
					...
					// Lama - anticipos cliente RQM #5325
					final MPayment payment = new MPayment(getCtx(), C_Payment_ID, getTrxName());
					if (Doc_Payment.isClient(payment)) {
						accountType = Doc.ACCTTYPE_C_Prepayment;
					}
				}
			}
		...
		DocLine_Allocation line)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				invoiceSource = rs.getBigDecimal(1);//1160
				invoiceAccounted = rs.getBigDecimal(2);//14500
			}
			rs.close();
			pstmt.close();
			pstmt = null;
		...
		//Invoice=(100)1160.00/14500.00 - Allocation=(130)14500.0000000/14500.00
		...
		if (getC_Currency_ID() != invoice.getC_Currency_ID())
		{
			...
			log.fine(d2);//Allocation=(130)14500.0000000->(100)1112.50
			description += " - " + d2;//Invoice=(100)1160.00/14500.00 - Allocation=(130)14500.0000000/14500.00 - Allocation=(130)14500.0000000->(100)1112.50
			allocationSource = allocationSourceNew;
		}

		BigDecimal amt = utilidadPerdidaCambiaria(line, invoice);
...
class Doc_AllocationTax
{
	...
	public boolean createEntries (MAcctSchema as, Fact fact, DocLine line)
	{
		...
		for (int i = 0; i < m_facts.size(); i++)
		{
			...
			// TODO Revisar este punto con el cargo
			//	WriteOff Amount	
