 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: BankStatementMatcher.java,v 1.2 2006/07/30 00:51:01 jjanke Exp $
 */
public class BankStatementMatcher extends SvrProcess
{
	 ...
	 *  @throws Exception if not successful
	 */
	public String doItWeb(Properties Ctx, int Table_ID, int Record_ID, String TrxName) throws Exception
	{
		m_matchers = MBankStatementMatcher.getMatchers(Ctx, TrxName);
		...
		if (Table_ID == X_I_BankStatement.Table_ID)
			return match (new X_I_BankStatement(Ctx, Record_ID, TrxName));
		else if (Table_ID == MBankStatement.Table_ID)
			return match (new MBankStatement(Ctx, Record_ID, TrxName));
		else if (Table_ID == MBankStatementLine.Table_ID)
			return match (new MBankStatementLine(Ctx, Record_ID, TrxName));
