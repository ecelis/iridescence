...
public class Doc_Inventory extends Doc
{
	...
	protected String loadDocumentDetails()
	{
		...
		if(inventory.getPost()==null){
			post = MEXMEMejoras.isControlaExistencias(inventory.getAD_Client_ID(), inventory.getAD_Org_ID(), null);
			
			// Evitar bloqueo
			final MInventory inventorySub = new MInventory(getCtx(), inventory.get_ID(), null);
			inventorySub.set_TrxName(null);
			inventorySub.setPost(post?"Y":"N");
			inventorySub.save();
			
		} else {
			post = inventory.getPost().equals("Y");
			p_PostMovement = inventory.getPost().equals(Doc.STATUS_Unposted)?false:true;
		}
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if(post || !line.getProduct().isStocked()){
				...
				if (costs == null || costs.signum() == 0)
				{
					...
					costs = ((MInventoryLine)line.getPOLine()).getCostTransaction(as
