...
public class MCostDetail extends X_M_CostDetail {
	/** serialVersionUID */
	private static final long serialVersionUID = -2346240921183049996L;
	...
	public static boolean createOrder(MAcctSchema as, int AD_Org_ID,
			...
			int C_OrderLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
		...
		MCostDetail cd = get(as.getCtx(),
				"C_OrderLine_ID=? ",
				C_OrderLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID,
					M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty,
					Description, trxName);
	...
	public static boolean createInvoice(MAcctSchema as, int AD_Org_ID,
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int C_InvoiceLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
				+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
		...
		MCostDetail cd = get(as.getCtx(),
				"C_InvoiceLine_ID=? ",
				C_InvoiceLine_ID, as.getC_AcctSchema_ID(), M_AttributeSetInstance_ID, trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID,
	...
	public static boolean createShipment(MAcctSchema as, int AD_Org_ID,
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_InOutLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, boolean IsSOTrx, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
				+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
	...
	public static boolean createInventory(MAcctSchema as, int AD_Org_ID,
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_InventoryLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
				+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
	...
	public static boolean createMovement(MAcctSchema as, int AD_Org_ID,
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_MovementLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, boolean from, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
				+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
		...
		MCostDetail cd = get(as.getCtx(),
				"M_MovementLine_ID=? AND IsSOTrx="
						+ (from ? "'Y'" : "'N'"), M_MovementLine_ID,
	...
	public static boolean createProduction(MAcctSchema as, int AD_Org_ID,
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_ProductionLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		final String sql = "DELETE M_CostDetail "
				+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
...
//	 * @return cost detail
//	 */
//	private static MCostDetail get(Properties ctx, String whereClause, int ID,
//			int ID2, String trxName) {
...
//	} // get
			...
			, String trxName)
	{
		final String localWhereClause = whereClause
			...
			+ " AND C_AcctSchema_ID=?";
		final MCostDetail retValue = new Query(ctx,I_M_CostDetail.Table_Name,localWhereClause,trxName)
...
//		return counterError == 0;
		...
		.append(" AD_Client_ID =? AND ")
		.append(I_M_CostDetail.COLUMNNAME_M_Product_ID).append(" = ? AND ")
		.append(I_M_CostDetail.COLUMNNAME_Processed).append(" = ? ");

		int counterOK = 0;
		int counterError = 0;
		
		final List<MCostDetail> list = new Query(product.getCtx(),I_M_CostDetail.Table_Name,whereClause.toString(),trxName)
		.setParameters(Env.getAD_Client_ID(product.getCtx()), product.getM_Product_ID(), false)
	...
	public String toString() {
		...
		if (isDelta())
			sb.append(",DeltaAmt=").append(getDeltaAmt()).append(",DeltaQty=")
					.append(getDeltaQty());
		sb.append("]");
		return sb.toString();
	} // toString
...
//			CostingLevel = pca.getCostingLevelDefault();
	 ...
	 *            cost element
	 * @param Org_ID
	 *            org - corrected for costing level
	 * @param M_ASI_ID
	 *            - asi corrected for costing level
	 * @return true if cost ok
	 */
	private boolean process(MAcctSchema as, MProduct product, MCostElement ce,
			int Org_ID, int M_ASI_ID) {
		
		log.finer(" Se esta CREANDO/ACTUALIZANDO el costo del producto: "+product.getValue()+" con el Metodo: "+ce.toString());
		
		MCost cost = MCost.get(product, M_ASI_ID, as, Org_ID,
		...
		if(product!=null && product.getM_Product_ID()>0){
			log.finer(" Esquema contable: " + as.toString());
		}
...
//		BigDecimal amt = getAmt();
		...
		{  log.finer(" CostDetail Qty:" + getQty() + " Amt:" + getAmt());//10-33.00
			qty = getQty();//  2
			amt = getAmt();//-14.1666
		}
		// end MZ
		...
		if (qty.signum() != 0){//-1 (negative), 0 (zero), or 1 (positive)
			price = amt.divide(qty, precision, BigDecimal.ROUND_HALF_UP);
			log.finer(" CostDetail Price:" + price);
		}
		/**
		 * All Costing Methods if (ce.isAverageInvoice()) else if
		 * (ce.isAveragePO()) else if (ce.isFifo()) else if (ce.isLifo()) else
		 * if (ce.isLastInvoice()) else if (ce.isLastPOPrice()) else if
		 * (ce.isStandardCosting()) else if (ce.isUserDefined()) else if
		 * (!ce.isCostingMethod())
		 **/
		
		// Embarque y devolución de cliente
		boolean isSOTrxMMR = false;
		if(isSOTrx() && getM_InOutLine_ID()>0 && getC_OrderLine_ID()<=0){
			final MInOutLine mLine = new MInOutLine(getCtx(), getM_InOutLine_ID(), get_TrxName());
			isSOTrxMMR = X_M_InOut.MOVEMENTTYPE_CustomerReturns.equals(mLine.getParent().getMovementType().trim());
		} 
		
		// Recepción y Devolución a proveedor
		BigDecimal costStd = null;
		if(!isSOTrx() && getM_InOutLine_ID()<=0 && getC_OrderLine_ID()>0){
			isSOTrxMMR = true;
			
			// Obtener el costo estandar cuando no existe previamente el costo promedio
			if(cost!=null && cost.getCurrentCostPrice().compareTo(Env.ZERO)==0 
					&& X_M_Cost.COSTINGMETHOD_AveragePO.equals(ce.getCostingMethod())){
				final MCostElement ces = MCostElement.getStandardCosting(product.getCtx());
				if(ces!=null){
					final MCost mCostStd = MCost.get(product, M_ASI_ID, as, Org_ID, ces.getM_CostElement_ID(), null);
					log.finer("Costo estandar: - " + mCostStd.toString());
					if(mCostStd!=null && mCostStd.getCurrentCostPrice().compareTo(Env.ZERO)>0) {
						costStd = mCostStd.getCurrentCostPrice();
						log.finer("Monto Costo estandar: - " + costStd);
					}
				}
			}
		} 
		...
		// Cantidad
		final BigDecimal qtyOnHand = getQtyOnHandStorage(cost, product, as, M_ASI_ID).subtract(qty);
		
		// *** Purchase Order Detail Record ***
		if (getC_OrderLine_ID() != 0) {
			log.finer(" CostDetail Por la linea de la orden:" + getC_OrderLine_ID());
			boolean isReturnTrx = qty.signum() < 0;
			
			if (ce.isAveragePO()) {
				cost.setWeightedAverage(costStd, amt, qty, isSOTrxMMR, qtyOnHand);
				log.finer("PO - AveragePO - " + cost);
				
				cost.setCumulatedQty(cost.getCurrentQty());
				cost.setCumulatedAmt(cost.getCurrentQty().multiply(cost.getCurrentCostPrice()));
				log.finer("PO - AveragePO - cost.setCumulatedQty(cost.getCurrentQty()) " + cost.getCumulatedQty());
		...
		// *** AP Invoice Detail Record ***
			...
			if (ce.isAverageInvoice()) {
				cost.setWeightedAverage(costStd, amt, qty, true, null);
				log.finer("Inv - AverageInv - " + cost);
				
			} else if (ce.isFifo() || ce.isLifo()) {
				// Real ASI - costing level Org
				MCostQueue cq = MCostQueue.get(product,
						getM_AttributeSetInstance_ID(), as, Org_ID,
						ce.getM_CostElement_ID(), get_TrxName());
				...
				if (cost.is_new()
					...
					if (cost.getCurrentCostPrice().signum() == 0
						...
						cost.setCurrentCostPrice(MCost.getSeedCosts(product,
								M_ASI_ID, as, Org_ID, ce.getCostingMethod(),
								getC_OrderLine_ID()));
				}
				cost.add(amt, qty);
		...
		// *** Qty Adjustment Detail Record ***
			...
			if (ce.isAverageInvoice()) {
				...
				if (addition ) {//INVENTARIO FISICO
					cost.setWeightedAverage(costStd, amt, qty, isSOTrxMMR, qtyOnHand);
					
				} else {// CARGO Y CANCELACION DE CARGOS Y SALIDA AL GASTO
					cost.setCurrentQty(cost.getCurrentQty().add(qty));
				}
				...
				cost.setCumulatedQty(cost.getCurrentQty());
				cost.setCumulatedAmt(cost.getCurrentQty().multiply(cost.getCurrentCostPrice()));
				log.finer("QtyAdjust - AveragePO - cost.setCumulatedQty(cost.getCurrentQty()) " + cost.getCumulatedQty());
				
			} else if (ce.isFifo() || ce.isLifo()) {
				if (addition) {
					// Real ASI - costing level Org
					MCostQueue cq = MCostQueue.get(product,
							getM_AttributeSetInstance_ID(), as, 
							Org_ID,
							ce.getM_CostElement_ID(), get_TrxName());
					cq.setCosts(amt, qty, precision);
//					cq.save();
					...
					MCostQueue.adjustQty(product, M_ASI_ID, as, Org_ID, ce,
							qty.negate(), get_TrxName());
				}
				// Get Costs - costing level Org/ASI
				MCostQueue[] cQueue = MCostQueue.getQueue(product, M_ASI_ID,
						as, Org_ID, ce, get_TrxName());
				...
				{					
					...
					for (int i = 0 ; i < lce.length ; i++)
					{
						MCost lCost = MCost.get(getCtx(), cost.getAD_Client_ID(), cost.getAD_Org_ID(), 
							...
							lce[i].getM_CostElement_ID(), cost.getM_AttributeSetInstance_ID(), get_TrxName(),true);
						if (lCost != null)
						{
							if (qtyOnhand.signum() != 0)
							{
...
//		setCumulatedQty(cost.getCumulatedQty());
	 ...
	 * @return
	 */
	private BigDecimal getQtyOnHandStorage(MCost cost, MProduct product, MAcctSchema as, int M_ASI_ID) {

		StringBuilder sql = new StringBuilder("SELECT COALESCE(SUM(QtyOnHand),0) ")
		...
		if (MAcctSchema.COSTINGLEVEL_Organization.equals(CostingLevel)) {
			...
			qtyOnhand = DB.getSQLValueBD(get_TrxName(), sql.toString(), cost.getAD_Client_ID(), cost.getM_Product_ID(), cost.getAD_Org_ID());
		} else if (MAcctSchema.COSTINGLEVEL_BatchLot.equals(CostingLevel)) {
			sql.append(" AND M_AttributeSetInstance_ID = ? ");
			qtyOnhand = DB.getSQLValueBD(get_TrxName(), sql.toString(), cost.getAD_Client_ID(), cost.getM_Product_ID(), M_ASI_ID);
		} else {
			qtyOnhand = DB.getSQLValueBD(get_TrxName(), sql.toString(), cost.getAD_Client_ID(), cost.getM_Product_ID());
		}
