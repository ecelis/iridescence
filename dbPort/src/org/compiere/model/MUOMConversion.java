...
{
	/** serialVersionUID */
	private static final long serialVersionUID = 417902607808983994L;
...
//	 *  @return converted qty (std precision)
//	 */
//	static public BigDecimal convertDiv (Properties ctx,
//		int C_UOM_ID, int C_UOM_To_ID, BigDecimal qty)
//	{
//		if (qty == null || qty.equals(Env.ZERO) || C_UOM_ID == C_UOM_To_ID)
//			return qty;
...
//			MUOM uom = MUOM.get (ctx, C_UOM_To_ID);
//			if (uom != null)
//				return uom.round(retValue.divide(qty), true);
//			return retValue.divide(qty);
//		}
//		return null;
//	}	//	convert
	...
	{
		GregorianCalendar endDate = new GregorianCalendar(TimeZone.getTimeZone(Env.getTimezone(Env.getCtx())));
	...
	public static BigDecimal convert(int C_UOM_From_ID, int C_UOM_To_ID, BigDecimal qty, boolean StdPrecision) {
		...
		if (qty == null || qty.equals(Env.ZERO)) {
			s_log.severe("Qty is Mandatory");
			return qty;
		} else if(C_UOM_From_ID == C_UOM_To_ID){
			s_log.severe("Same UOM - FromUOM=" + C_UOM_From_ID + ", ToUOM=" + C_UOM_To_ID);
			return qty;
		}		
		//
		BigDecimal retValue = null;
		...
		sql.append("SELECT c.MultiplyRate, uomTo.StdPrecision, uomTo.CostingPrecision ");
		sql.append(" FROM	C_UOM_Conversion c");
		sql.append(" INNER JOIN C_UOM uomTo ON (c.C_UOM_TO_ID=uomTo.C_UOM_ID) ");
		sql.append(" WHERE c.IsActive='Y' AND c.C_UOM_ID=? AND c.C_UOM_TO_ID=? "); // #1/2
		sql.append(" AND c.M_Product_ID IS NULL ");
		sql.append(" ORDER BY c.AD_Client_ID DESC, c.AD_Org_ID DESC");
		...
		if (retValue == null) {
			s_log.severe("NOT found - FromUOM=" + C_UOM_From_ID + ", ToUOM=" + C_UOM_To_ID);
			return null;
		}
	...
	/**
	 * rsolorzano
	 * obtiene el id de unidad de medida equivalente de acuerdo al sistema de medici�n
	 * @param cuomID
	 * @param sistemaMedicionTo
	 * @return int
	 */
	public static int getCUOMToID(Properties ctx, int cuomID, String sistemaMedicion){
		int cuomToID = 0;
		
		StringBuilder sql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY);
		sql.append("SELECT ");
		sql.append("  u.c_uom_id as ID ");
		sql.append("FROM ");
		sql.append("  c_uom_conversion uc ");
		sql.append("  INNER JOIN c_uom u ");
		sql.append("  ON uc.c_uom_to_id = u.c_uom_id ");
		sql.append("WHERE ");
		sql.append("  uc.c_uom_id = ? AND ");
		sql.append("  u.isdefault = 'Y' AND ");
		sql.append("  u.sistemamedicion = ? AND ");
		sql.append(" uc.isActive = 'Y' ");
		if (Env.getUserPatientID(ctx) < 0) // Si es un usuario paciente no agrega el accessLevel
			sql.append(MEXMELookupInfo.addAccessLevelSQL(ctx, " ", MUOMConversion.Table_Name, "uc"));
		...
		try {
			pstmt = DB.prepareStatement(sql.toString(), null);
			pstmt.setString(2, sistemaMedicion);
			pstmt.setInt(1, cuomID);
			rs = pstmt.executeQuery();

			if (rs.next()) {
				cuomToID = rs.getInt("ID");
			}
		} catch (SQLException e) {
			s_log.log(Level.SEVERE, "getCUOMToID", e);
		} finally {
			DB.close(rs,pstmt);
		}
	...
	/**
	 * Realiza la conversion de unidades verificando ambo sentidos de la conversion
	 * @param C_UOM_From_ID
	 * @param C_UOM_To_ID
	 * @param qty
	 * @param StdPrecision
	 * @return
	 */
	public static BigDecimal convertUOMs(int C_UOM_From_ID, int C_UOM_To_ID, BigDecimal qty, boolean StdPrecision) {
		...
		sql.append("SELECT c.MultiplyRate, uomTo.StdPrecision, uomTo.CostingPrecision, c.dividerate ");
		sql.append(" FROM	C_UOM_Conversion c");
		sql.append(" INNER JOIN C_UOM uomTo ON (c.C_UOM_TO_ID=uomTo.C_UOM_ID) ");
		sql.append(" WHERE c.IsActive='Y' AND c.C_UOM_ID=? AND c.C_UOM_TO_ID=? "); // #1/2
		sql.append(" AND c.M_Product_ID IS NULL ");
	...
	/**
	 * Indica si existe la conversion en el sentido de los parametros
	 * @param C_UOM_From_ID
	 * @param C_UOM_To_ID
	 * @return
	 */
	public static boolean getModToDo(int C_UOM_From_ID, int C_UOM_To_ID) {
	...
	/**
	 * Obtiene el valor de multiplicacion o division
	 * @param C_UOM_From_ID
	 * @param C_UOM_To_ID
	 * @param column num de columna a regresar
	 * @return
	 */
	public static BigDecimal getRateUOMs(int C_UOM_From_ID, int C_UOM_To_ID, int column) {
		...
		try {
			...
			if (rs.next()) {
				retValue = rs.getBigDecimal(column);
	...
	/**
	 * Regresa el valor del residuo de la division de una conversion de UDM
	 *  [9/3=0(Cociente=3,Residuo=0), 57/28=1(Cociente=2,Residuo=1)]
	 * @param product producto para las udm
	 * @param value valor que se requiere su residuo
	 * @return residuo de la division 
	 */
	public static BigDecimal getConversionDivisor(final MProduct product, final BigDecimal value){
		BigDecimal divisor;
		BigDecimal Qtyuom = BigDecimal.ZERO;
		// 4 es el factor de division, 1 el de multiplicacion, no usar 2 y 3
		if (MUOMConversion.getModToDo(product.getC_UOM_ID(),
				product.getC_UOMVolume_ID())) {			
			divisor = MUOMConversion.getRateUOMs(product.getC_UOM_ID(),
					product.getC_UOMVolume_ID(), 4);
		} else {
			divisor = MUOMConversion.getRateUOMs(product.getC_UOMVolume_ID(),
					product.getC_UOM_ID(), 1);
		}
		if (divisor != null && value.compareTo(BigDecimal.ZERO) > 0) {
			Qtyuom = value.remainder(divisor);
		}
		return Qtyuom;
	}
	
	/**
	 * Obtiene todas las conversiones de una unidad de medida
	 * @param ctx
	 * @param where
	 * @param trxName
	 * @return
	 */
	public static List<KeyNamePair> getAllUomConvertion(Properties ctx, int cUom, String trxName) {
		final StringBuilder sql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY);
		final boolean base = Language.isBaseLanguage(Env.getAD_Language(ctx));
		sql.append("SELECT distinct CON.C_UOM_ID, ").append(base ? "c1" : "ct").append(".Name as name ")
		.append("FROM C_UOM_CONVERSION CON ")
		.append("INNER JOIN C_UOM c1 on CON.C_UOM_ID = c1.C_UOM_ID AND c1.IsActive = 'Y' ")
		.append("INNER JOIN C_UOM c2 on CON.C_UOM_ID = c2.C_UOM_ID AND c2.IsActive = 'Y' ");
		if (!base) {
			sql.append("INNER JOIN C_UOM_Trl ct ON (CON.C_UOM_ID = ct.C_UOM_ID AND ct.AD_Language=?) ");
		}
		sql.append(" WHERE CON.IsActive = 'Y' ")
		.append(" AND (CON.C_UOM_ID= ? OR CON.C_UOM_TO_ID=?) ");
		sql.append(MEXMELookupInfo.addAccessLevelSQL(ctx, " ", Table_Name, "CON"));
		...
		try {
			...
			int position=1;
			if(!base){
				pstmt.setString(position++, Env.getAD_Language(ctx));
			}
			pstmt.setInt(position++, cUom);
			pstmt.setInt(position++, cUom);
			rs = pstmt.executeQuery();
			while (rs.next()) {
