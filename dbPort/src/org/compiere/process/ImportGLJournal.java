...
public class ImportGLJournal extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuffer ("UPDATE I_GLJournal i "
			+ "SET DateAcct="+ DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + " "
	   ...
	   //	ORDER BY ValidFrom DESC
	        sql.append(" WHERE CurrencyRate IS NULL OR CurrencyRate=0 AND C_Currency_ID>0"
		   + " AND I_IsImported<>'Y'").append (clientCheck);
		...
		sql = new StringBuffer ("UPDATE I_GLJournal i "
			...
			+ " AND r.C_ConversionType_ID=i.C_ConversionType_ID"
			+ " AND i.DateAcct BETWEEN r.ValidFrom AND r.ValidTo AND ");
		...
		sql.append(" WHERE C_Period_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append (clientCheck);
		...
		sql.append(" WHERE Account_ID IS NULL AND AccountValue IS NOT NULL"
			+ " AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql.append(" WHERE M_Product_ID IS NULL AND (ProductValue IS NOT NULL OR UPC IS NOT NULL OR SKU IS NOT NULL)"
			+ " AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql = new StringBuffer(Constantes.INIT_CAPACITY_ARRAY);
			sql.append("SELECT * FROM I_GLJournal ");
			sql.append(" WHERE I_IsImported='N'").append(clientCheck);
			sql.append(" ORDER BY COALESCE(BatchDocumentNo, TO_NCHAR(I_GLJournal_ID)),	COALESCE(JournalDocumentNo, ");
			sql.append(" TO_NCHAR(I_GLJournal_ID)), C_AcctSchema_ID, PostingType, ");
			sql.append(" C_DocType_ID, GL_Category_ID, C_Currency_ID, ");
		if (DB.isOracle()) {
			sql.append(" TRUNC(DateAcct), Line, I_GLJournal_ID");
		} else if (DB.isPostgreSQL()) {
			sql.append(" DATE_TRUNC('day', DateAcct), Line, I_GLJournal_ID");
		}
		try
		{
			pstmt = DB.prepareStatement (sql.toString (), get_TrxName());
			...
			while (rs.next())
			{
					...
					|| !BatchDocumentNo.equals(impBatchDocumentNo))
				{
					...
					if (description == null || description.length() == 0)
						...
						description += " *Import-";
					description += DB.getTimestampForOrg(Env.getCtx());
