...
public class Doc_GLJournal extends Doc
{
	...
	private DocLine[] loadLines(MJournal journal)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			//  --  Converted Amounts
			docLine.setC_Currency_ID(line.getC_Currency_ID());//card 1195 Es el monto en moneda del cliente
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				if (p_lines[i].getC_AcctSchema_ID () == as.getC_AcctSchema_ID ())
				{
					FactLine line = fact.createLine (p_lines[i],
									...
									p_lines[i].getC_Currency_ID(),//card 1195 Es el monto en moneda del cliente
	...
	public int getGL_Category_ID() {
		MJournal journal = (MJournal)getPO();
		return journal.getGL_Category_ID();
	}
