...
//		String variable = "";
	...
	public static int getProduct (Properties ctx, int M_Product_ID,
		...
		boolean IsSOTrx)
	{
//		if(!IsSOTrx && M_Product_ID>0){
//			MProduct m_Product = new MProduct(ctx,M_Product_ID,null);
//			if()
//		} else {
//		
		...
		try
		{
			...
			// Get all at once
			//El impuesto ahora saldra del charge master [p.C_TaxCategory_ID(antes)]
			sql.append(" SELECT NVL(cte.C_TaxCategory_ID,porg.C_TaxCategory_ID), o.C_Location_ID ");
			sql.append(billC_BPartner_Location_ID>0?", il.C_Location_ID":", 0 AS C_Location_ID ");
			sql.append(billC_BPartner_Location_ID>0?", b.IsTaxExempt ":", 'N' AS IsTaxExempt ");
			sql.append(", w.C_Location_ID ");
			sql.append(shipC_BPartner_Location_ID>0?" , sl.C_Location_ID ":", 0 AS C_Location_ID ");
			sql.append("FROM M_Product p ");
			//Cambio para chargeMaster en impuestos
			sql.append(" LEFT JOIN  exme_productoorg porg ON (p.M_Product_ID = porg.M_Product_ID AND porg.AD_Client_ID = ?  and porg.AD_Org_ID = ?) ");// 1-2
			// Lama .- Product_Cte
			sql.append(" LEFT JOIN  M_Product_Cte cte ON (p.M_Product_ID = cte.M_Product_ID AND cte.AD_Client_ID = ? ), ");// 3
			sql.append(" AD_OrgInfo o,");
			sql.append(billC_BPartner_Location_ID>0?" C_BPartner_Location il INNER JOIN C_BPartner b ON (il.C_BPartner_ID=b.C_BPartner_ID),":"");
			sql.append(" M_Warehouse w ");
			sql.append(shipC_BPartner_Location_ID>0?", C_BPartner_Location sl ":"");
			sql.append("WHERE p.M_Product_ID=?");
			sql.append(" AND o.AD_Org_ID=?");
			sql.append(billC_BPartner_Location_ID>0?" AND il.C_BPartner_Location_ID=?":" AND 0=?");
			sql.append(" AND w.M_Warehouse_ID=?");
			sql.append(shipC_BPartner_Location_ID>0?" AND sl.C_BPartner_Location_ID=?":" AND 0=?");
				
			PreparedStatement  pstmt = DB.prepareStatement(sql.toString(), null);
			//Cambio para chargeMaster en impuestos
			pstmt.setInt(1, Env.getAD_Client_ID(ctx));//0
			pstmt.setInt(2, Env.getAD_Org_ID(ctx));//0
			//Cambio para chargeMaster en impuestos
			pstmt.setInt(3, Env.getAD_Client_ID(ctx));//10001000
			pstmt.setInt(4, M_Product_ID);//1395724
			pstmt.setInt(5, AD_Org_ID);//10001034
			pstmt.setInt(6, billC_BPartner_Location_ID);//0
			pstmt.setInt(7, M_Warehouse_ID);//10001044
			pstmt.setInt(8, shipC_BPartner_Location_ID);//0
		...
		//	M_Product_ID				->	C_TaxCategory_ID
			//Se cambio de m_product a productoorg
			sql = new StringBuilder( "SELECT C_TaxCategory_ID FROM Exme_ProductoOrg "
				+ "WHERE M_Product_ID=? and AD_client_ID= ? and AD_Org_ID = ?");
		...
		//	AD_Org_ID					->	billFromC_Location_ID
			sql = new StringBuilder("SELECT C_Location_ID FROM AD_OrgInfo "
				+ "WHERE AD_Org_ID=?");
			variable = "AD_Org_ID";
			pstmt = DB.prepareStatement(sql.toString(), null);
			...
			if (rs.next())
			{
				billFromC_Location_ID = rs.getInt (1);//2687288
				found = true;
			}
			rs.close();
			pstmt.close();
		...
		//	billC_BPartner_Location_ID  ->	billToC_Location_ID
			sql = new StringBuilder( "SELECT l.C_Location_ID, b.IsTaxExempt "
		...
		//	M_Warehouse_ID				->	shipFromC_Location_ID
			sql = new StringBuilder("SELECT C_Location_ID FROM M_Warehouse "
		...
		//	shipC_BPartner_Location_ID 	->	shipToC_Location_ID
			sql =new StringBuilder( "SELECT C_Location_ID FROM C_BPartner_Location "
				+ "WHERE C_BPartner_Location_ID=?");
		...
		Timestamp billDate, int billFromC_Location_ID, int billToC_Location_ID)
	{
		...
		for (int i = 0; i < taxes.length; i++)
		{
			...
			if (tax.getC_TaxCategory_ID() != C_TaxCategory_ID
				...
				continue;
			if (IsSOTrx && MTax.SOPOTYPE_PurchaseAndDirectSalesTax.equals(tax.getSOPOType()))
				continue;
			if (!IsSOTrx && MTax.SOPOTYPE_SalesByExtensionTax.equals(tax.getSOPOType()))
				continue;
