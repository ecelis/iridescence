...
public class GridWindow implements Serializable
{
	...
	public static GridWindow get (Properties ctx, int WindowNo, int AD_Window_ID)
	{
		...
		if (mWindowVO == null)
			return null;
		return new GridWindow(mWindowVO, null);
	}	//	get
	 ...
	 */
	public GridWindow (GridWindowVO vo, POConfirmPanel confirmPanel)
	{
		m_vo = vo;
		
		if (loadTabData(confirmPanel))
			enableEvents();
	}	//	MWindow
	 ...
	 */
	private boolean loadTabData(POConfirmPanel confirmPanel)
	{
		log.config("");
		...
		for (int t = 0; t < m_vo.Tabs.size(); t++)
		{
			...
			if (mTabVO != null)
			{
				GridTab mTab = new GridTab(mTabVO, confirmPanel);
				m_tabs.add(mTab);
			}
		}	//  for all tabs
		return true;
	...
	public Image getImage()
	{
		if (m_vo.AD_Image_ID == 0)
			return null;
		//
		MImage mImage = MImage.get(getCtx(), m_vo.AD_Image_ID);
	...
	{
		String title = Msg.getMsg(getCtx(), "Window") + ": " + getName();
		WebDoc doc = null;
		if (javaClient)
		{
			doc = WebDoc.create (getCtx(), false, title, javaClient);
		}
		else	//	HTML
		{
			doc = WebDoc.createPopup (getCtx(), title);
			doc.addPopupClose(getCtx());
		}
		...
		for (int i = 0; i < size; i++)
		{
			...
			tabHeader.addElement(new tr()
				.addElement(new td().addElement(new a().setName("Tab" + i)
						.addElement(new h2(Msg.getMsg(getCtx(), "Tab") + ": " + tab.getName()))))
			...
			for (int j = 0; j < tab.getFieldCount(); j++)
			{
				...
				if (hdr != null && hdr.length() > 0)
				{
					...
					.addElement(new td().addElement(new a().setName("Field" + i + j)
							.addElement(new h3(Msg.getMsg(getCtx(), "Field") + ": " + hdr)))
					.addElement(new td().addElement(WebDoc.NBSP).addElement(WebDoc.NBSP))
