...
{
	/**	Logger			*/
	private static CLogger s_log = CLogger.getCLogger(MProject.class);
	...
	/**
	 * Get Project according to Business Partner
	 * */
	public List<MProject> getBusinessPartnerProjects(Integer cBPartnerID){
		List<MProject> projects = null;
		StringBuilder sb = new StringBuilder(" SELECT * FROM C_PROJECT ")
			.append(" WHERE C_PROJECT.ISSUMMARY='N' ")
			.append(" AND (C_Project.C_BPartner_ID IS NULL OR C_Project.C_BPartner_ID=? ")
			.append(" OR C_Project.C_BPartnerSR_ID=?) ");
		...
		try{
			...
			projects = new ArrayList<MProject>();
			while (rs.next())
				projects.add(new MProject(getCtx(),rs,null));
			
		}catch(SQLException e){
			s_log.log(Level.SEVERE, "getBusinessPartnerProjects", e);
		}finally{
			try{
				if (pstmt != null)
					pstmt.close();
	...
	/**
	 *  Method to obtain the list of projects from the business partner selected
	 *  @param int cBpartner_id
	 *  @return Vector<OptionItem>	 
	 */
	public Vector<OptionItem> getProjects(int cBpartner_id){
		
		Vector<OptionItem> projects=null;
							
		StringBuilder sb = new StringBuilder("SELECT C_Project_id,Name,Value ");
				sb.append("FROM C_Project ");
				sb.append("WHERE C_Project.IsSummary='N' AND (C_Project.C_BPartner_ID IS NULL OR C_Project.C_BPartner_ID=? OR C_Project.C_BPartnerSR_ID=?)");
		StringBuilder sql = new StringBuilder(MEXMELookupInfo.addAccessLevelSQL(getCtx(), sb.toString(), this.get_TableName()));
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try{
			...
			pstmt.setInt(2, cBpartner_id);
			
			
		rs = pstmt.executeQuery();
		projects = new Vector<OptionItem>();	
		projects.add(new OptionItem("0"," "));
		while (rs.next()){
			
			projects.add(new OptionItem(rs.getString(1),rs.getString(2)+"_"+rs.getString(3)));
				}
			}catch(SQLException e){
					e.printStackTrace();
				}
				...
				try{
					...
					log.log(Level.SEVERE, "getProjects", e);
				}
			}
