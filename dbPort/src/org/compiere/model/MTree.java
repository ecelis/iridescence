 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: MTree.java,v 1.6 2006/08/11 02:26:22 mrojas Exp $
 */
public class MTree extends MTree_Base
{
	...
	{
		this (ctx, AD_Tree_ID, editable, clientTree, false, false, trxName);
	}   //  MTree

	public MTree (Properties ctx, int AD_Tree_ID, 
			boolean editable, boolean clientTree, 
			boolean allNodes, boolean inactive,
			String trxName)
	{
	...
	public static int getDefaultAD_Tree_ID(int AD_Client_ID, String keyColumnName) {
		return getDefaultAD_Tree_ID(AD_Client_ID, keyColumnName, 0);
	}
	 ...
	 *  @return AD_Tree_ID
	 */
	public static int getDefaultAD_Tree_ID(int AD_Client_ID, String keyColumnName, int C_Element_ID) {
		...
		sql.append("SELECT t.AD_Tree_ID, t.Name FROM AD_Tree t ");
		sql.append("WHERE t.AD_Client_ID=?  AND t.IsActive='Y' ");
		if(C_Element_ID > 0){
			sql.append(" AND EXISTS (SELECT * FROM C_Element e WHERE e.C_Element_ID=? ");
			sql.append(" AND e.IsActive='Y' AND t.AD_Tree_ID = e.AD_Tree_ID) ");
		} else{
			sql.append(" AND t.TreeType=? AND t.IsAllNodes='Y' ");
		}
		sql.append("ORDER BY t.IsDefault DESC, t.AD_Tree_ID");
		
		AD_Tree_ID = DB.getSQLValue(null, sql.toString(), AD_Client_ID, C_Element_ID > 0 ? C_Element_ID : TreeType);
...
//		}
	 ...
	 */
	private void loadNodes (int AD_User_ID, boolean inactive)
	{
		//  SQL for TreeNodes
...
//		diagPrintTree();
	 ...
	 */
	private void getNodeDetails (boolean inactive)
	{
		//  SQL for Node Info
		...
		if (getTreeType().equals(TREETYPE_Menu))
		{
			...
			if (base)
				sqlNode.append("SELECT m.AD_Menu_ID, m.Name,m.Description,m.IsSummary,m.Action, "
					+ "m.AD_Window_ID, m.AD_Process_ID, m.AD_Form_ID, m.AD_Workflow_ID, m.AD_Task_ID, m.AD_Workbench_ID "//, m.EXME_Struts_ID " //EXPERT: mrojas
					+ "FROM AD_Menu m");
			else
		...
		} else if (columnNameX == null) {
			throw new IllegalArgumentException("Unknown TreeType=" + getTreeType());
		} else {
			sqlNode.append("SELECT t.").append(columnNameX).append("_ID,");
			if (TREETYPE_ElementValue.equals(getTreeType())) {
				sqlNode.append("t.value||' - '||t.Name,");//Lama: cuentas
			} else {
				sqlNode.append("t.Name,");
			}
			sqlNode.append("t.Description,t.IsSummary,");
			sqlNode.append(color).append(" FROM ");
			sqlNode.append(fromClause);
...
//					int EXME_Struts_ID = m_nodeRowSet.getInt(index++); //EXPERT: mrojas
					//
...
////					/* EXPERT: mrojas : lama*/
//					else if (X_AD_Menu.ACTION_Struts.equals(actionColor))
//						access = null;
//						access = role.getStrutsAccess(EXME_Struts_ID);
					/* EXPERT: mrojas */
				...
				//	log.fine("getNodeDetail - " + name + " - " + actionColor + " - " + access);
					//
					if ((access != null && access)		//	rw or ro for Role 
	...
	private void dumpTree()
	{
		...
		s_log.log(Level.FINEST,"Count=" + count);
