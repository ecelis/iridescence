...
public class X_Fact_Acct extends PO implements I_Fact_Acct, I_Persistent 
{
	...
	public I_EXME_PolicyInfo getEXME_PolicyInfo() throws RuntimeException 
    {
        Class<?> clazz = MTable.getClass(I_EXME_PolicyInfo.Table_Name);
        I_EXME_PolicyInfo result = null;
        try	{
	        Constructor<?> constructor = null;
	    	constructor = clazz.getDeclaredConstructor(new Class[]{Properties.class, int.class, String.class});
    	    result = (I_EXME_PolicyInfo)constructor.newInstance(new Object[] {getCtx(), new Integer(getEXME_PolicyInfo_ID()), get_TrxName()});
        } catch (Exception e) {
	        log.log(Level.SEVERE, "(id) - Table=" + Table_Name + ",Class=" + clazz, e);
	        log.saveError("Error", "Table=" + Table_Name + ",Class=" + clazz);
           throw new RuntimeException( e );
	...
	public void setEXME_PolicyInfo_ID (int EXME_PolicyInfo_ID)
	{
		if (EXME_PolicyInfo_ID < 1) 
			set_Value (COLUMNNAME_EXME_PolicyInfo_ID, null);
		else 
			set_Value (COLUMNNAME_EXME_PolicyInfo_ID, Integer.valueOf(EXME_PolicyInfo_ID));
	}
	...
	{
		set_Value (COLUMNNAME_Interfase, Boolean.valueOf(Interfase));
	}
	...
	{
		Object oo = get_Value(COLUMNNAME_Interfase);
		if (oo != null) 
		{
	...
	/** Get Line.
		@return Transaction line (internal)
	  */
