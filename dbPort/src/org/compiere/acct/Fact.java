...
public final class Fact
{
	...
	//	log.fine("createLine - " + account	+ " - Dr=" + debitAmt + ", Cr=" + creditAmt);
		...
		// Card 947 Distribuir la contabilidad por los cargos de la factura
		if (account != null && account.getAD_OrgTrx_ID() > 0
				&& m_doc.get_Table_ID() == X_C_Invoice.Table_ID // Es factura ?
				&& !m_doc.isSOTrx() // Es una factura de compra (Proveedor)
				&& docLine.getC_Charge_ID() > 0)// Que sea un cargo
		{
			line.setAD_OrgTrx_ID(account.getAD_OrgTrx_ID());
		}
	...
	public boolean isSourceBalanced()
	{
		...
		if (retValue)
			...
			log.warning ("NO (isSourceBalanced) - Diff=" + balance + " - " + toString());
		return retValue;
	}	//	isSourceBalanced
	...
	public FactLine balanceSource()
	{
		...
		if (diff.signum() < 0)  { //  negative balance => DR
			line.setAmtSource(m_acctSchema.getC_Currency_ID(), diff.abs(), Env.ZERO);// Se cambio de la moneda del documento a la moneda del esquema ticket #06894
		} else {                               //  positive balance => CR
			line.setAmtSource(m_acctSchema.getC_Currency_ID(), Env.ZERO, diff);// Se cambio de la moneda del documento a la moneda del esquema ticket #06894
		}
		//  Convert
		line.convert();
		//
	...
	public boolean isAcctBalanced()
	{
		...
		if (retValue)
			...
			log.warning("NO (isAcctBalanced) - Diff=" + balance + " - " + toString());
		return retValue;
	}	//	isAcctBalanced
