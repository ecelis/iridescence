...
public class MRequest extends X_R_Request
{
	...
	public void setDueType()
	{
		...
		Timestamp now = Env.getCurrentDate();
		//
	...
	{
		super.setDateLastAlert (Env.getCurrentDate()); 
	}	//	setDateLastAlert
	...
	private void setPriority()
	{
		...
		if (getBPartner() != null)
		{
			MBPGroup bpg = MBPGroup.get(getCtx(), getBPartner().getC_BP_Group_ID(), get_TrxName());
		...
		if (newRecord || is_ValueChanged("R_RequestType_ID"))
		{
			if (m_requestType != null)
			{
				...
				if (getDateNextAction() == null && m_requestType.getAutoDueDateDays() > 0)
					setDateNextAction(TimeUtil.addDays(Env.getCurrentDate(), 
		...
		if (status != null)
		{
			if (status.isOpen())
			{
				if (getStartDate() == null)
					setStartDate (Env.getCurrentDate());
				if (getCloseDate() != null)
					setCloseDate(null);
			}
			if (status.isClosed() 
				&& getCloseDate() == null)
				setCloseDate(Env.getCurrentDate());
...
//		setLastResult(msg);
	 ...
	 * @return
	 */
	public static List<MRequest> getRequestByCtaPac(int tableId, int ctaPacId){
		List<MRequest> list = new ArrayList<MRequest>();
		
		StringBuilder sql = new StringBuilder();
		sql.append("select * from R_Request where ad_table_id = ? ");
		...
		try {
			...
			while(rs.next()){
				list.add(new MRequest(Env.getCtx(), rs, null));
			}
			
		} catch (Exception e) {
			s_log.log(Level.SEVERE, "MRequest- getRequestByCtaPac() ", e);
		}finally{
			DB.close(rs, pstmt);
		}
		return list;
	...
	/**
	 * Creacion de request USA
	 * @param cargo
	 * @param trxName
	 */
	public static MRequest createRequest(final MCtaPacDet cargo, String trxName){
		MRequest req = null;
		if (cargo.getPrecioAnterior().compareTo(cargo.getPriceActual()) != 0) {
			req = new MRequest(cargo.getCtx(), 0, trxName);
			req.createRequest(cargo);
		}
		return req;//Lama: cargos 2014
	}
	 ...
	 * @param cargo
	 */
	public void createRequest(final MCtaPacDet cargo){
		final String nombreUsuario = MUser.getFromID(MCtaPacDet.COLUMNNAME_Description, Env.getAD_User_ID(getCtx()));
		setClientOrg(Env.getAD_Client_ID(getCtx()), Env.getAD_Org_ID(getCtx()));
		//The price of the Charge {0} was modified by User {1} at the moment of entered.
		setSummary(Utilerias.getLabel("newChargeNote", String.valueOf(cargo.getProduct().getValue()), String.valueOf(cargo.getProduct().getName()), String.valueOf(cargo.getPrecioAnterior()), String.valueOf(cargo.getPriceActual()), String.valueOf(nombreUsuario)));
		setNoteType(MRequest.NOTETYPE_GeneralNote);
		setStartDate(Env.getCurrentDate());
		setInitials(nombreUsuario);
		setDueType(MRequest.DUETYPE_Overdue);// TODO pendiente saber si es la tabla correspondiente  
		setAD_Table_ID(MEXMECtaPac.Table_ID);// TODO pendiente saber si es la tabla correspondiente  
		setRecord_ID(cargo.getEXME_CtaPac_ID());// TODO pendiente saber si es la tabla correspondiente  
		if(save(get_TrxName())){
			log.log(Level.INFO, "MRequest : Ok save ");
		}else {
			log.log(Level.SEVERE, "MRequest : Error save");
		}
	}
