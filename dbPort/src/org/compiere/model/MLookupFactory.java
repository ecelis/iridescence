...
public class MLookupFactory
{
		...
		throws Exception
	{
		...
		if (info == null){//Expert: Lama - Validate null
			s_log.log(Level.SEVERE, "(" + Column_ID + ") - No LookupInfo");
	...
	public static MLookup get (Properties ctx, int WindowNo, int TabNo, int Column_ID, int AD_Reference_ID)
	{
		//
		MLookupInfo info = getLookupInfo (ctx, WindowNo, Column_ID, AD_Reference_ID);
		if(info == null){//Lama - Validate null
			s_log.log(Level.SEVERE, "(" + Column_ID + ") - No LookupInfo");
			return null;
		}
		...
		boolean IsParent, String ValidationCode)
	{
		...
		// Expert: Lama - Location with different columnname of C_Location_ID - 26/03/2010
		else if(AD_Reference_ID == DisplayType.Location && !ColumnName.equals("C_Location_ID")){
			info = getLookup_TableDir(ctx, language, WindowNo, ColumnName, "C_Location");
		}
		//	TableDir, Search, ID, ...
		else
		{
		...
		//	Add Security // Raul si tiene un perfil de paciente no agrega el accesslevel
		if (needToAddSecurity && !new MRole(ctx, Env.getAD_Role_ID(ctx), null).isForPatientAccess())
	...
	static private MLookupInfo getLookup_Table(Properties ctx, Language language, int WindowNo, int AD_Reference_Value_ID) {
		...
		MLookupInfo retValue = null;// (MLookupInfo)s_cacheRefTable.get(key);
		// if (retValue != null)
		// {
	...
	static private MLookupInfo getLookup_TableDir (Properties ctx, Language language,
		 ...
		 return getLookup_TableDir(ctx, language, WindowNo, ColumnName, null);
	}
	 ...
	 * @param ColumnName column name
	 * @param WindowNo Window (for SOTrx)
	 * @param TableName - not for TableDirect columns - Expert: Lama 26/03/2010
	 * @return SELECT Key, NULL, Name, IsActive from Table (fully qualified)
	 */
	static private MLookupInfo getLookup_TableDir (Properties ctx, Language language,
		int WindowNo, String ColumnName, String tableName)
	{
		...
		String TableName = tableName == null ? MQuery.getZoomTableName(ColumnName) : tableName;//Expert:Lama
		//boolean isSOTrx = !"N".equals(Env.getContext(ctx, WindowNo, "IsSOTrx"));
		int ZoomWindow = 0;
		int ZoomWindowPO = 0;
		...
		try
		{
			...
			while (rs.next())
			{
				LookupDisplayColumn ldc = new LookupDisplayColumn (rs.getString(1),
					"Y".equals(rs.getString(2)), rs.getInt(3), rs.getInt(4), rs.getString("ColumnSQL"));
				list.add (ldc);
		...
		if (list.size() == 0)
		{
			s_log.log(Level.SEVERE, "No Identifier records found: " + cacheKey + " ColumnName--> " + ColumnName);//Expert: Lama
			return null;
		}

		StringBuffer realSQL = new StringBuffer("SELECT ");
		realSQL.append(TableName).append(".");
		realSQL.append(tableName == null ? KeyColumn : (tableName+"_ID"))//Expert:Lama
				.append(",NULL,");
		...
		for (int i = 0; i < size; i++)
		{
			...
			// virtualcolumn //Expert:Lama
			else if(!StringUtils.isBlank(ldc.ColumnSQL)){
				displayColumn.append("(").append(ldc.ColumnSQL).append(")");
			}
			//  String
			else {
		...
		MLookupInfo lInfo = new MLookupInfo(realSQL.toString(), TableName,
			TableName + "." + //
			(tableName != null ? (tableName+"_ID"): KeyColumn), //Expert: Lama
	...
	static public String getLookup_TableDirEmbed (Language language,
		...
		return getLookup_TableDirEmbed ( language,
				ColumnName,  BaseTable,  BaseColumn, null);
	}

	static public String getLookup_TableDirEmbed (Language language,
		String ColumnName, String BaseTable, String BaseColumn, String columnOrig)
	{
		String KeyColumn = MQuery.getZoomColumnName(ColumnName);
		...
		if (list.size() == 0)
		{
			s_log.log(Level.SEVERE, "No Identifier records found: " + KeyColumn + " ColumnName--> " + ColumnName);//Expert: Lama
			return "";
		}
		...
		if (! BaseColumn.trim().startsWith("(")) {
			embedSQL.append(BaseTable).append(".").append(columnOrig==null?BaseColumn:columnOrig);
		}
		else {
