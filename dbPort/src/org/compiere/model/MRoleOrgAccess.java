...
public class MRoleOrgAccess extends X_AD_Role_OrgAccess
{
	...
	public static boolean createForOrg (MOrg org)
	{
		...
		for (int i = 0; i < roles.length; i++)
		{
			if (!roles[i].isManual())
			{
				...
				if (orgAccess.save(org.get_TrxName()))
					counter++;
			}
		}
	...
	/**
	 * Get the organization that the current role has access to. The role and client
	 * are taken from the application context.
	 * @param ctx The application context.
	 * @return An array with the organization ids.
	 */
	public static Integer[] getForRoleClient(Properties ctx) {
		List<Integer> list = new ArrayList<Integer>();
		Integer[] retVal = null;
		String sql =
				"SELECT AD_Org_ID FROM AD_Role_OrgAccess WHERE AD_Client_ID = ? AND AD_Role_ID = ?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			pstmt.setInt(2, Env.getAD_Role_ID(ctx));
