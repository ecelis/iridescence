...
public class GridFieldVO implements Serializable
{
		...
		int AD_Window_ID, int AD_Tab_ID, boolean readOnly, ResultSet rs)
	{
		...
		try
		{
			...
			// Expert: Lama - Mask
			String replaceMaskChars = null;
			String definitionMask = null;
			...
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				...
				// Expert: Lama
				else if (columnName.equalsIgnoreCase("IsSearchColumn"))
					vo.IsSearchColumn = "Y".equals(rs.getString (i));
				...
				// Expert: Lama - Mask
				else if (columnName.equalsIgnoreCase("ReplaceMaskChars"))
					replaceMaskChars = rs.getString(i);
				else if (columnName.equalsIgnoreCase("DefinitionMask"))
					definitionMask = rs.getString(i);
				// fin
...
//				Feature Request FR [ 2003044 ]
			...
			// Expert: Lama - Mask
			if (vo.VFormat != null) {
				vo.format = new Mask(vo.VFormat, definitionMask, replaceMaskChars);
			}
	...
	public static GridFieldVO createParameter (Properties ctx, int WindowNo, ResultSet rs)
	{
		...
		try
		{
			...
			// Expert: Lama - Mask
			if (vo.VFormat != null) {
				vo.format = new Mask(vo.VFormat, rs.getString("ReplaceMaskChars"), rs.getString("DefinitionMask"));
			}
	...
	public static GridFieldVO createParameter (GridFieldVO voF)
	{
		...
		//voT.AD_Field_ID = voF.AD_Field_ID;
	...
	/** Expert: Lama - 	Search columns		*/
	public boolean		IsSearchColumn = false;
	/** Expert: Lama - Mask */
	public Mask format = null;
	...
	protected void initFinish()
	{
		...
		if (DisplayType.isLookup(displayType) && IsDisplayed) {
			try {
				lookupInfo = 
					MLookupFactory.getLookupInfo (ctx, WindowNo, AD_Column_ID, displayType,
		...
		boolean TabReadOnly)
	{
		...
		clone.IsSearchColumn = IsSearchColumn; // Expert: Lama
