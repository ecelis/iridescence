...
public class MWorkflowAccess extends X_AD_Workflow_Access
{
	...
	private static CLogger s_log = CLogger.getCLogger(MWorkflowAccess.class);
	...
	public static List<MWorkflowAccess> getAccesFromRole(Properties ctx, int AD_Role_ID, String trxName) {
		StringBuilder st = new StringBuilder("select * from AD_Workflow_Access where AD_Role_ID = ? and isActive = 'Y'");
		List<MWorkflowAccess> lst = new ArrayList<MWorkflowAccess>();
		...
		try {
			...
			while (rs.next()) {
				lst.add(new MWorkflowAccess(ctx, rs, trxName));
			}
			rs.close();
			pstmt.close();
			pstmt = null;
