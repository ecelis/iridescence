...
public class MAttachmentEntry {
	 ...
	 *            optional index
	 */
	public MAttachmentEntry(String name, byte[] data, int index, String description) {
		super();
		setName(name);
		setData(data);
	 ...
	 *            binary data
	 */
	public MAttachmentEntry(String name, byte[] data, String description) {
		this(name, data, 0, description);
	} // MAttachmentItem
	...
	public void dump() {
		...
		/**
		 * // Dump w/o nulls if (nullCount > 0) { for (int i = 0; i < m_data.length-nullCount; i++) System.out.print((char)m_data[i]);
		 * System.out.println (); System.out.println (hdr); } /**
		 **/
	} // dump
	...
	public File getFile(File file) {
		...
		try {
			...
			StringBuilder st = new StringBuilder(getTmpDirectory());
			st.append(file.getName());
			file = new File(st.toString());
	...
	public static String getTmpDirectory() {
		if (StringUtils.isBlank(tmpDirectory)) {
			StringBuilder st = new StringBuilder();
			st.append(Ini.getCompiereHome()).append(File.separatorChar);
			st.append("utils").append(File.separatorChar);
			st.append("tmp").append(File.separatorChar);
			tmpDirectory = st.toString();
		}
		return tmpDirectory;
	}
