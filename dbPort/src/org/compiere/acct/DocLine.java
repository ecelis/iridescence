...
{	
	...
	/** Element user */
	private int                 m_UserElement1_ID=-1;
	/** Element user */
	private int                 m_UserElement2_ID=-1;
	/** Price actual *///Card# 1399
	private BigDecimal      	m_priceActual = null;
...
//		String costingMethod = null;
		final BigDecimal costs = getProductCost().getProductCosts(as
				, AD_Org_ID
				, null
				, getC_OrderLine_ID()
				, zeroCostsOK
...
//		}
	...
	public BigDecimal getProductCosts (MAcctSchema as, int AD_Org_ID, boolean zeroCostsOK, final boolean createUpdateCosts)
	{
//		ProductCost pc = getProductCost();
//		int C_OrderLine_ID = getC_OrderLine_ID();
//		String costingMethod = null;
...
//			return m_productCost.getProduct();
		return m_productCost == null?null:m_productCost.getProduct();
	}	//	getProduct
...
//			return product.getC_RevenueRecognition_ID();
		return product == null?0:product.getC_RevenueRecognition_ID();
...
//			return (String)p_po.get_Value(index);
		return index != -1?(String)p_po.get_Value(index):null;
	}	//	getDescription
	 ...
	 * @return
	 */
	public int getUserElement1_ID(final int C_AcctSchema_ID) {
		try {
			if(m_doc!=null){
				String columnName = m_doc.getBeanUserElement1(C_AcctSchema_ID);
	 ...
	 * @return
	 */
	public int getUserElement2_ID(final int C_AcctSchema_ID) {
		try {
			if(m_doc!=null){
				String columnName = m_doc.getBeanUserElement2(C_AcctSchema_ID);
		...
		return m_UserElement2_ID;
	}
	
	//card 1195 Es el monto en moneda del cliente
	public void setC_Currency_ID(final int moneda){
		m_C_Currency_ID = moneda;
	}
	...
	public BigDecimal getPriceActual () {
		if (m_priceActual == null || m_priceActual.compareTo(Env.ZERO)==0){
			int index = p_po.get_ColumnIndex("PriceActual");
			if (index != -1) {
				m_priceActual = (BigDecimal)p_po.get_Value(index);
			}
			
			if (m_priceActual == null){
				m_priceActual = Env.ZERO;
			}
		}
		return m_priceActual;
	}   //  getPriceActual
	
	/** Objeto original card 1444*/
	public PO getPOLine(){
		return p_po;
	}
}	//	DocumentLine
