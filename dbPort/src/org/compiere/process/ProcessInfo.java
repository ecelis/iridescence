...
public class ProcessInfo implements Serializable
{
	...
	//windowNo - Usada para mostrar ADialog en un Process
	private int m_windowNo = 0;
	
	//Flags of dialog window
	private boolean flagWarning = false;
	private boolean flagInfo = false;
	private boolean flagError = false;
	private String msgDialog = null;
	...
	@SuppressWarnings("unchecked")
	public void setSummary (String translatedSummary, boolean error, List result)
	{
		setSummary (translatedSummary);
		setError(error);
	...
	/*
	 * Set windowNo for ADialog in SvrProcess
	 * @param wNo
	 */
	public void setWindowNo(int wNo)
	{
		m_windowNo = wNo;
	}
	
	/*
	 * Get windowNo for ADialog in SvrProcess
	 */
	...
	@SuppressWarnings("unchecked")
	public void setResultObject(List mResult) {
		m_Result = mResult;
	}
	
	private StateEngine stateEngine;
	public void setStateEngineVal(StateEngine stateEngine) {
		this.stateEngine = stateEngine;
	}
	public StateEngine getStateEngine() {
		return stateEngine;
	}
	/** Regresa el mensaje traducido .- Lama */
	public String getSummary(boolean translated, Properties ctx) {
		if(translated) {
			String summary = getSummary();
			if(StringUtils.isNotEmpty(summary)) {
				if(stateEngine != null && StringUtils.equals(summary, stateEngine.toString())) {
//					int AD_Reference_ID = 305;
					String st = MRefList.getListName(ctx, X_AD_WF_Activity.WFSTATE_AD_Reference_ID, stateEngine.getState());
					if (StringUtils.isNotEmpty(st) && !StringUtils.equals(st, summary)) {
						return st;
					}
				} else {
					return Msg.parseTranslation(ctx, summary);
				}
			}
			return summary;
		} else {
