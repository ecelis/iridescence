...
public class MWFProcess extends X_AD_WF_Process
{
	...
	private static final long serialVersionUID = -8992222567597358696L;
	 ...
	 /** Static Logger               */
    private static CLogger      s_log = CLogger.getCLogger (MWFProcess.class);
	...
	{
		this(wf, pi.getRecord_ID(), wf.getAD_Table_ID(), pi.getAD_User_ID(), trxName);
	}
	 ...
	 * @throws Exception
	 */
	public MWFProcess (MWorkflow wf, int recordID, int tableID, int userID, String trxName) throws Exception
	{
...
//TODO  m_pi = pi; red1 - never used  -check later	
		...
		setUser_ID(userID);		//	user starting
		//
		m_state = new StateEngine (getWFState());
		setProcessed (false);
		//	Lock Entity
 ...
 */
	...
	private boolean startNext (MWFActivity last, MWFActivity[] activities, PO lastPO, String trxName)
	{
		...
		// 05062012 - rsolorzano.  se quita el cliente de PO (last.getPO_AD_Client_ID()) dado que hay tablas que tiene nivel de 
		// acceso system y el ad_client siempre sera 0, nunca regresara los nodos siguientes del wf. 
		MWFNodeNext[] transitions = getWorkflow().getNodeNexts(
			last.getAD_WF_Node_ID(), Env.getAD_Client_ID(Env.getCtx())); 
	...
	public static MWFProcess getActualProcess(MWorkflow wf, int tableID, int recordID){
		...
		try {
			...
			sql.append("SELECT AD_WF_PROCESS.*  ");
			sql.append("FROM AD_WF_PROCESS ");
			sql.append("WHERE AD_WF_PROCESS.AD_WORKFLOW_ID = ? ");
			sql.append("AND AD_WF_PROCESS.AD_TABLE_ID = ? ");
			sql.append("AND AD_WF_PROCESS.RECORD_ID = ? ");
			sql.append("AND AD_WF_PROCESS.PROCESSED = 'N' ");
			sql.append("AND AD_WF_PROCESS.WFSTATE = '");
			sql.append(MWFProcess.WFSTATE_Suspended);
			sql.append(" ' AND AD_WF_PROCESS.ISACTIVE = 'Y' ");
			sql.append(MEXMELookupInfo.addAccessLevelSQL(Env.getCtx(), " ", MWFProcess.Table_Name, "AD_WF_PROCESS"));
			...
			if (rs.next()) {
				process = new MWFProcess(Env.getCtx(), rs, null);
			}
