...
public class Doc_Cash extends Doc
{
	...
	protected String loadDocumentDetails ()
	{
		...
		// La fecha del cierre de caja será la fecha de la poliza contable ticket GC 26/11/2014 por monclova
		// que cierra sus cajas del día por la tarde hace la conciliación de bancos pero si habre caja ese mismo día
		// y cierra al otro el asiento se muestra el dia que abrio caja y eso a monclova no le sirve #8814
		setDateDoc(cash.getEndDate()==null?cash.getStatementDate():cash.getEndDate());
	...
	private DocLine[] loadLines(MCash cash, MCashBook cb)
	{
		...
		MCashLine[] lines = cash.getLines(false, false, null);
		for (int i = 0; i < lines.length; i++)
		{
			MCashLine line = lines[i];
			DocLine_Cash docLine = new DocLine_Cash (line, this);
			
			if(line.getM_parent()!=null){
//				docLine.setDateAcct(line.getM_parent().getDateAcct()); // TODO En la misma fecha del encabezado GC
				docLine.setDateAcct(cash.getEndDate()==null?cash.getStatementDate():cash.getEndDate());
			}
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			//		p_vo.C_Currency_ID, null, line.getAmount().negate());
				...
				fact.createLine(null, getAccount(Doc.ACCTTYPE_CashAsset, as),
						getC_Currency_ID(), line.getAmount());//FIXME: Negativo ?
			}
			...
			//		p_vo.C_Currency_ID, line.getAmount(), null);
				...
				fact.createLine(null, getAccount(Doc.ACCTTYPE_CashAsset, as),
						getC_Currency_ID(), line.getAmount());// Nuevo
			}
...
//			getC_Currency_ID(), assetAmt);
		log.log(Level.SEVERE, "Cash Asset > " + assetAmt);
