...
public class Doc_MatchInv extends Doc
{
	...
	protected String loadDocumentDetails ()
	{
		...
		if(matchInv.getPost()==null){
			post = MEXMEMejoras.isControlaExistencias(matchInv.getAD_Client_ID(), matchInv.getAD_Org_ID(), null);
			
			final MMatchInv matchInvSub = new MMatchInv(getCtx(), matchInv.get_ID(), null);
			matchInvSub.set_TrxName(null);
			matchInvSub.setPost(post?"Y":"N");
			matchInvSub.save();
			
		} else {
			post = matchInv.getPost().equals("Y");
			p_PostMovement = matchInv.getPost().equals(Doc.STATUS_Unposted)?false:true;
		}
		...
		if(m_receiptLine.getParent().isSetpoint()){
			post = false;
		}
		return null;
	}   //  loadDocumentDetails
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		MProduct mProduct = new MProduct(as.getCtx(), getM_Product_ID(), null);
		...
		if (m_pc.isService())
		{
			...
			if (!dr.updateReverseLine (MInOut.Table_ID, 		//	Amt updated
					...
					multiplier))
			{
				p_Error = Utilerias.getLabel("msj.recMatNoPosteada");
				return null;
			}
...
//				}
				...
				cr.setmConversionTypeID(invoice.getC_ConversionType_ID());
				...
				{
					p_Error = Utilerias.getLabel("msj.facturaNoPosteada");
					return null;
				}
