	...
	implements FormPanel, ActionListener, VetoableChangeListener
{
	...
	private void dynInit()
	{
		...
		if (wVO == null)
			return;
		m_mWindow = new GridWindow (wVO, null);
	...
	private void refresh()
	{
		...
		if (ts != null) {
			if (DB.isOracle()) {
				...
				query.addRestriction("DATE_TRUNC('day', MovementDate)", MQuery.GREATER_EQUAL, ts);
			}
		}
		//  DateTO
		...
		if (ts != null) {
			if (DB.isOracle()) {
				...
				query.addRestriction("DATE_TRUNC('day', MovementDate)", MQuery.LESS_EQUAL, ts);
			}
		}
