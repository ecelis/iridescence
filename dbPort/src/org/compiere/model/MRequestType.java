...
public class MRequestType extends X_R_RequestType
{
	...
	private synchronized void updateStatistics()
	{
		...
		m_nextStats = DB.getTimestampForOrg(getCtx()).getTime() + 3600000;		//	every hour
	}	//	updateStatistics
		...
		Timestamp startDate, MRole role)
	{
		...
		if (MMeasure.MEASUREDATATYPE_QtyAmountInTime.equals(MeasureDataType)
			...
			//		;
				...
				orderBy = "DATE_TRUNC('" + trunc + "', " + dateColumn + ")";
			}
			
			groupBy = orderBy + ", 0 ";
			sb.append(groupBy).append("FROM R_Request ");
		} else {
			orderBy = "s.SeqNo"; 
		...
		String MeasureDisplay, Timestamp date, int R_Status_ID, MRole role)
	{
		...
		if (R_Status_ID != 0) {
			...
			//	else if (MGoal.MEASUREDISPLAY_Day.equals(MeasureDisplay)) 
			//		trunc = "D";
				where = "DATE_TRUNC('" + trunc + "', " + dateColumn + ") " +
					 "= DATE_TRUNC('" + trunc + "', " + DB.TO_DATE(date) + ")";
			}
		}
