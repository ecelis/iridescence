...
import org.compiere.model.I_DD_Order;
import org.compiere.model.I_HR_Process;
import org.compiere.model.I_PP_Cost_Collector;
import org.compiere.model.I_PP_Order;
		...
		if (ACTION_Complete.equals(m_action) || ACTION_WaitComplete.equals(m_action))
		{
			...
			*/
			...
			if (m_document != null && ok)
			{
				...
				if (m_document != null && !Ini.isClient())		//	Post Immediate if on Server
				{
					MClient client = MClient.get(m_document.getCtx(),
			...
			return ok; // fin Merge ADempiere trunk_rev14653
		}
...
//		Locked
		if (processing != null)
		{
			...
			if (!locked && processing instanceof Boolean){
				...
				opts.add(DocumentEngine.ACTION_Unlock);
			}
		}
		...
		{
			opts.add(DocumentEngine.ACTION_Prepare);
			opts.add(DocumentEngine.ACTION_Void);
		}
		//	Draft/Invalid				..  DR/IN
		else if (docStatus.equals(DocumentEngine.STATUS_Drafted)
			|| docStatus.equals(DocumentEngine.STATUS_Invalid))
		{
			opts.add(DocumentEngine.ACTION_Complete);
			opts.add(DocumentEngine.ACTION_Prepare); //rocio
			opts.add(DocumentEngine.ACTION_Void);
		}
		//	In Process                  ..  IP
		...
		{
			opts.add(DocumentEngine.ACTION_Close);
		}
		//	Waiting Payment
		...
		{
			//	Draft                       ..  DR/IP/IN
			if (docStatus.equals(DocumentEngine.ACTION_Prepare))
				{
				opts.add(DocumentEngine.ACTION_Complete);
				}else
			if (docStatus.equals(DocumentEngine.STATUS_Drafted)
				...
				|| docStatus.equals(DocumentEngine.STATUS_Invalid))
			{
				opts.remove(DocumentEngine.ACTION_Prepare);
//				opts.add(DocumentEngine.ACTION_Close);
...
//			Card 1124: GC solicito que las ordenes de compras completadas solo se pueden
//			mandar a cerradas
//			//	Complete                    ..  CO
//			else if (docStatus.equals(DocumentEngine.STATUS_Completed))
//			{
//				opts.add(DocumentEngine.ACTION_Void;
//				opts.add(DocumentEngine.ACTION_ReActivate;
//			}
			else if (docStatus.equals(DocumentEngine.STATUS_WaitingPayment))
			{
				opts.add(DocumentEngine.ACTION_ReActivate);
		...
		else if (AD_Table_ID == MInOut.Table_ID)
		{
			...
			{
				opts.add(DocumentEngine.ACTION_Void);
//				opts.add(DocumentEngine.ACTION_Reverse_Correct); Ticket#08530 Jonathan
			}
		}
		/********************
		 *  Invoice
		...
		else if (AD_Table_ID == MInvoice.Table_ID)
		{
			...
			{
				opts.add(DocumentEngine.ACTION_Void);
				opts.add(DocumentEngine.ACTION_Reverse_Correct);
			}
		}
		/********************
		 *  Payment
		...
		else if (AD_Table_ID == MJournal.Table_ID || AD_Table_ID == MJournalBatch.Table_ID)
		{
			...
			{
				opts.add(DocumentEngine.ACTION_Reverse_Correct);
				// GC dijo que no se usa esta acción 04/03/15
				// opts.add(DocumentEngine.ACTION_Reverse_Accrual);
				opts.add(DocumentEngine.ACTION_ReActivate);
			}
		}
		/********************
		 ...
		 */
			...
			if (DocumentEngine.STATUS_Completed.equals(docStatus)) {
				// opts.add(DocumentEngine.ACTION_Void);
				// opts.add(DocumentEngine.ACTION_Reverse_Correct);
			} else if (DocumentEngine.STATUS_Drafted.equals(docStatus)) {
				opts.remove(DocumentEngine.ACTION_Prepare);
			}
		}
		/********************
		...
		log.finest("*****ACCIONES VALIDAS *******");
		int index = 0;
		for(String opt : opts){
			log.finest("accion " + index + ":" + opt);
			options[index++] = opt;
		}
			...
			ArrayList<String> v_description)
	{
		...
		if (Env.isBaseLanguage(Env.getCtx(), "AD_Ref_List")){
			sql.append("SELECT Value, Name, Description FROM AD_Ref_List ");
			sql.append("WHERE AD_Reference_ID=? ORDER BY Name");
		} else {
			sql.append("SELECT l.Value, t.Name, t.Description ");
			sql.append("FROM AD_Ref_List l, AD_Ref_List_Trl t ");
			sql.append("WHERE l.AD_Ref_List_ID=t.AD_Ref_List_ID");
			sql.append(" AND l.AD_Reference_ID=? ");
...
//
//		String error = null;
//		String ca = MSysConfig.getValue("CLIENT_ACCOUNTING", "D", // default
//				Env.getAD_Client_ID(Env.getCtx()));
//		boolean isClientAccounting = (ca.equalsIgnoreCase("I") || ca.equalsIgnoreCase("Q"));
//		// Adempiere FIXME: revisar
//		// if (MClient.isClientAccounting()) {
//		if (isClientAccounting) {
//			log.info("Table=" + AD_Table_ID + ", Record=" + Record_ID);
//			MAcctSchema[] ass = MAcctSchema.getClientAcctSchema(ctx, AD_Client_ID);
//			// Adempiere FIXME: revisar ? DOC
//			//error = org.compiere.acct.Doc.postImmediate(ass, AD_Table_ID, Record_ID, force, trxName);
//			return error;
//		}
//
