...
{
	/** serialVersionUID */
	private static final long serialVersionUID = -3868326827258110298L;
	...
	/**
	* Obtenemos los elementos de un esquema contable
	* @param esquema Esquema contable a obtener sus elementos
	* @return Un ResultSet con los elementos del esquema contable
	*/
	public static List<String> getAcctSchemaElement(long esquema) throws Exception {
		...
		//buscamos los precios del producto
		sql.append("SELECT DISTINCT C_AcctSchema_Element.ElementType ")//
				.append("FROM C_AcctSchema_Element ")//
				.append("WHERE C_AcctSchema_Element.C_AcctSchema_ID = ? AND ")//
				.append("C_AcctSchema_Element.ElementType IN (")//	
				.append(DB.TO_STRING(ELEMENTTYPE_Activity)).append(", ")// AY
				.append(DB.TO_STRING(ELEMENTTYPE_Campaign)).append(", ")// MC
				.append(DB.TO_STRING(ELEMENTTYPE_OrgTrx)).append(", ")// OT
				.append(DB.TO_STRING(ELEMENTTYPE_Project)).append(", ")// PJ
				.append(DB.TO_STRING(ELEMENTTYPE_UserList1)).append(", ")// U1
				.append(DB.TO_STRING(ELEMENTTYPE_UserList2)).append(") ")// U2"
				.append("AND IsActive = 'Y' ");

		try {

			pstmt = DB.prepareStatement(sql.toString(), null);
			pstmt.setLong(1, esquema);
			rs = pstmt.executeQuery();
			
			while (rs.next()) {
				lst.add(rs.getString("ElementType"));
			}


		} catch (SQLException sqle) {
			s_log.log(Level.SEVERE, sqle.getMessage(), sqle);
			throw new Exception("error.getAcctSchemaElement");
		} finally {
			try {
                if (pstmt != null)
                    pstmt.close ();
	...
	* @return Un ResultSet con los elementos del esquema contable
	*/
	public static MAcctSchemaElement getAcctSchemaElementUserElement(
			final Properties ctx, final int C_AcctSchema_ID, final String value, final String trxName) throws Exception {

		StringBuilder sql = new StringBuilder(100);
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		MAcctSchemaElement lst = null;
		
		sql.append(" SELECT C_AcctSchema_Element.* ")//
				.append(" FROM C_AcctSchema_Element ")//
				.append(" WHERE C_AcctSchema_Element.C_AcctSchema_ID = ? ")//
				.append(" AND C_AcctSchema_Element.ElementType IN (")//	
				.append(DB.TO_STRING(value)).append(") ")// U2"
				.append(" AND IsActive = 'Y' ");
		...
		try {
			...
			while (rs.next()) {
				lst = new MAcctSchemaElement(ctx, rs,trxName);
			}
