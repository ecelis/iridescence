...
public class InfoPayment extends Info
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3466556225213689080L;
	...
	String getSQLWhere()
	{
		...
		if (fDateFrom.getValue() != null || fDateTo.getValue() != null)
		{
			...
			if (DB.isOracle()) {
				...
				if (from == null && to != null)
					sql.append(" AND DATE_TRUNC('day', p.DateTrx) <= ?");
				else if (from != null && to == null)
					sql.append(" AND DATE_TRUNC('day', p.DateTrx) >= ?");
				else if (from != null && to != null)
					sql.append(" AND DATE_TRUNC('day', p.DateTrx) BETWEEN ? AND ?");
			}
		}
		//
