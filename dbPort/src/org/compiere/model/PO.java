...
import org.compiere.model.accessdb.MetaData;
...
import com.ecaresoft.log.Column;
import com.ecaresoft.log.Data;
import com.ecaresoft.log.LogEntry;
import com.ecaresoft.log.Table;
import com.ecaresoft.log.User;
...
implements Serializable, Comparator, Evaluatee, Cloneable, OnCompare
{
	/**
	 *
	 */
	...
	private POConfirmPanel confirmPanel = null;

	public static final String	COLUMN_AD_Client_ID		= "AD_Client_ID";

	public static final String	COLUMN_AD_Org_ID		= "AD_Org_ID";
	...
	public static com.mongodb.DB mongoDB = null;

	public static com.mongodb.DB getMongoDB() {
		return mongoDB;
	}

	public static void setMongoDB(com.mongodb.DB mongoDB) {
		PO.mongoDB = mongoDB;
	}
	...
	public void setConfirmPanel(POConfirmPanel confirmationPanel) {
		this.confirmPanel = confirmationPanel;
	}
	...
	/*INICIO EXPERT - ASAEL*/
	/**Store the database error message		*/
	...
	public PO (Properties ctx, int ID, String trxName, ResultSet rs)
	{
		...
		getNickName();// El Seteo del nickname debe ser manual no por medio de el getter
	...
	//INPROCESS
	/** Access Level Client to System  saves from a client into system */
	public static final int ACCESSLEVEL_CLIENTSYSTEM = 9;
	...
	protected final boolean set_Value (int index, Object value)
	{
		...
		if (value == null) {
			...
			final Class<?> columnClazz = p_info.getColumnClass(index);
			// Lama: forzar vacio para el caso de los Strings
			if (columnClazz == String.class && value instanceof String && StringUtils.isEmpty((String) value)) {
				m_newValues[index] = Null.NULL; // correct
				log.finer(ColumnName + " = null");
			} else
			// matching class or generic object
			if (value.getClass().equals(columnClazz) || columnClazz == Object.class) {
				m_newValues[index] = value; // correct
				// Integer can be set as BigDecimal
			} else if (value.getClass() == BigDecimal.class && columnClazz == Integer.class) {
				m_newValues[index] = new Integer(((BigDecimal) value).intValue());
				// Set Boolean
			} else if (columnClazz == Boolean.class && ("Y".equals(value) || "N".equals(value))) {
			...
			} else if (value.getClass() == Integer.class && columnClazz == String.class) {
				m_newValues[index] = value;
			} else if (value.getClass() == String.class && columnClazz == Integer.class) {
				try {
					m_newValues[index] = new Integer((String) value);
				} catch (NumberFormatException e) {
					log.log(Level.SEVERE, ColumnName
							+ " - Class invalid: " + value.getClass().toString()
							+ ", Should be " + columnClazz.toString() + ": " + value
							+ e.toString());
					return false;
				}
			} else {
			...
			if (p_info.getColumnClass(index) == String.class) {
				...
				//Lama: valida el tipo de dato: TextLong (ya que AD_Column.FieldLength es de cero)
				if (stringValue.length() > length && length > 0) {
				     if (DisplayType.TextLong == p_info.getColumnDisplayType(index)) {
				...
				} else if (DisplayType.TextLong == p_info.getColumnDisplayType(index)) {
				     m_newValues[index] = stringValue;
				}
			}
	...
	public void setProcessedOn(String ColumnName, Object value, Object oldValue) {
		if ("Processed".equals(ColumnName)
			...
			if (get_ColumnIndex("ProcessedOn") > 0) {
				...
				Timestamp ts = DB.getDbTimestamp();
	...
	/**
	 * @see PO#copyValues(PO, PO, boolean, boolean)
	...
	public static void copyValues (PO from, PO to){
		copyValues (from, to, false, false);
	}
	/**
	 *  @see PO#copyValues(PO, PO, boolean, boolean)
	 *  @param from old, existing & unchanged PO
	 *  @param to new, not saved PO
	 *  @param ignoreSearchKey - true: ignore value/documentNo columns
	 *  @author Expert:Lama
	 */
	public static void copyValues (PO from, PO to, boolean ignoreSearchKey){
		copyValues (from, to, false, ignoreSearchKey);
	}
	 ...
	 * Created,Updated,IsActive,AD_Client_ID,AD_Org_ID,Processing,Posted
	 * </span>
	 * </p>
	 * <p style="font-weight:bold;">
	 * Es requerido reinicializar las columnas relacionadas a contabilidad, workflows, etc. que NO deben copiar el valor de otro documento
	 * </p>
	 *
	 * @param from old, existing & unchanged PO
	 * @param newValues - true: forces to copy the FROM new values (Expert:Lama)
	 * @param ignoreSearchKey - true: ignore value/documentNo columns (Expert:Lama)
	 * @param to new, not saved PO
	 */
	public static void copyValues(PO from, PO to, boolean newValues, boolean ignoreSearchKey) {
		s_log.fine("From ID=" + from.get_ID() + " - To ID=" + to.get_ID());
		// Different Classes
		if (from.getClass() != to.getClass()) {
			for (int i1 = 0; i1 < from.m_oldValues.length; i1++) {
				// KeyColumn
				if (from.p_info.isVirtualColumn(i1) || from.p_info.isKey(i1)) {
					continue;
				}
				String colName = from.p_info.getColumnName(i1);
				// Ignore Standard Values
				if (colName.startsWith(COLUMN_Created) //
					|| colName.startsWith(COLUMN_Updated)//
					|| colName.equals(COLUMN_IsActive)//
					|| colName.equals(COLUMN_AD_Client_ID) //
					|| colName.equals(COLUMN_AD_Org_ID)//
					|| colName.equals("Processing")//
					|| colName.equals("Posted") // Lama: Columna reservada
				) {
					continue; // ignore
				} else if (ignoreSearchKey && colName.equals("Value") //
					|| colName.equals("DocumentNo")) {
					; // ignore Value/DocumentNo columns - Expert:Lama
				} else {
					for (int i2 = 0; i2 < to.m_oldValues.length; i2++) {
						if (to.p_info.getColumnName(i2).equals(colName)) {
							if (from.is_new() || newValues) {
								to.m_newValues[i2] = from.get_Value(i1);
							} else {
			...
			for (int i = 0; i < from.m_oldValues.length; i++) {
				...
				if (colName.startsWith(COLUMN_Created) //
					...
					if (from.is_new() || newValues) {
						to.m_newValues[i] = from.get_Value(i);
					} else {
	...
	/**
	 *  Copy the last values of From to new values of To.
	 *  Does not copy Keys and AD_Client_ID/AD_Org_ID
	 * 	@param from old, existing & unchanged PO
	 *  @param to new, not saved PO
	 *  @author Expert:Lama
	 */
	public static void copyValuesNew (PO from, PO to){
		copyValues (from, to, true, false);
	}
	...
	protected boolean load (ResultSet rs)
	{
		...
		final StringBuilder errorWhile = new StringBuilder();
		//  load column values
		for (index = 0; index < size; index++)
		{
			if (p_info.isVirtualColumn(index))	{//	if rs constructor used
				continue;//Lama: en lugar de provocar la excepcion
			}
			String columnName = p_info.getColumnName(index);
			...
			try
			{
				if (clazz == Integer.class) {
					...
					//m_oldValues[index] = decrypt(index, DB.convertTimeZone(rs.getTimestamp(columnName), Env.BASE_TIMEZONE, Env.getTimezone(getCtx())));
...
//							+ " (" + p_info.getColumnClass(index) + ") - " + e);
					if(errorWhile.length()==0){
						errorWhile.append("PO.load(rs) - Table: ").append(p_info.getTableName());

						e.printStackTrace();
					}
					errorWhile.append(Constantes.NEWLINE);
					errorWhile.append(index);
					errorWhile.append(" Column: ").append(p_info.getColumnName(index));
					errorWhile.append(" (").append(p_info.getColumnClass(index)).append(") - ");
					errorWhile.append(e);
					success = false;
				}
			}
		...
		if(!success && errorWhile.length() > 0){
			log.log(Level.SEVERE, errorWhile.toString());
		}
	...
	protected void setStandardDefaults()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			else if (colName.equals(COLUMN_Created) || colName.equals(COLUMN_Updated))
				m_newValues[i] = DB.getDbTimestamp();
			else if (colName.equals(p_info.getTableName() + "_ID"))    //  KeyColumn
				m_newValues[i] = I_ZERO;
			else if (colName.equals(COLUMN_IsActive))
				m_newValues[i] = new Boolean(true);
			else if (colName.equals(COLUMN_AD_Client_ID))
				m_newValues[i] = new Integer(Env.getAD_Client_ID(p_ctx));
			else if (colName.equals(COLUMN_AD_Org_ID))
	...
	{
		set_ValueNoCheck (COLUMN_AD_Client_ID, new Integer(AD_Client_ID));
	}	//	setAD_Client_ID
	...
	{
		Integer ii = (Integer)get_Value(COLUMN_AD_Client_ID);
		if (ii == null)
			return 0;
		return ii.intValue();
	}	//	getAD_Client_ID
	...
	{
		set_ValueNoCheck (COLUMN_AD_Org_ID, new Integer(AD_Org_ID));
	}	//	setAD_Org_ID
	...
	{
		Integer ii = (Integer)get_Value(COLUMN_AD_Org_ID);
		if (ii == null)
			return 0;
		return ii.intValue();
	}	//	getAD_Org_ID
	...
	{
		set_Value(COLUMN_IsActive, new Boolean(active));
	}	//	setActive
	...
	{
		Boolean bb = (Boolean)get_Value(COLUMN_IsActive);
		if (bb != null)
	...
	{
		return (Timestamp)get_Value(COLUMN_Created);
	}	//	getCreated
	...
	{
		return (Timestamp)get_Value(COLUMN_Updated);
	}	//	getUpdated
	...
	{
		Integer ii = (Integer)get_Value(COLUMN_CreatedBy);
		if (ii == null)
			return 0;
		return ii.intValue();
	}	//	getCreateddBy

	public String getCreatedBy(final boolean nameByRole) {
		if(nameByRole) {
			return MUser.getUserName(getCtx(), getCreatedBy());
		} else {
			return MUser.getNameOfUser(getCreatedBy());
		}
	}

	public String getUpdatedBy(final boolean nameByRole) {
		if(nameByRole) {
			return MUser.getUserName(getCtx(), getUpdatedBy());
		} else {
			return MUser.getNameOfUser(getUpdatedBy());
		}
	}
	...
	{
		Integer ii = (Integer)get_Value(COLUMN_UpdatedBy);
		if (ii == null)
			return 0;
		return ii.intValue();
	}	//	getUpdatedBy
	...
	{
		set_ValueNoCheck (COLUMN_UpdatedBy, new Integer(AD_User_ID));
	}	//	setAD_User_ID
...
/**************************************************************************
	...
	public boolean save()
	{
		...
		{
			log.saveError("FillMandatory", Msg.getElement(getCtx(), COLUMN_AD_Org_ID));
			return false;
		}
		//	Should be Org 0
		if (getAD_Org_ID() != 0)
		{
			//INPROCESS
			boolean reset = (get_AccessLevel() == ACCESSLEVEL_SYSTEM || get_AccessLevel() == ACCESSLEVEL_CLIENTSYSTEM);
		...
		//for client to system access level, the client must be always System
		if(get_AccessLevel() == ACCESSLEVEL_CLIENTSYSTEM)
			setAD_Client_ID(0);
		...
		try
		{
			...
			if (e instanceof ConfirmException && confirmPanel != null) {
				if (!confirmPanel.confirm(e.getMessage())) {
					log.log(Level.WARNING, "beforeSave - " + toString(), e);
					//log.saveError("Error", e, false);
					if (localTrx != null) {
						localTrx.rollback();
		...
		try {
			...
			if (newRecord)
			{
				...
				else
				{
					...
					// Lama: si es nuevo registro, se borran los Ids generados
	 ...
	 * @see #save()
	 */
	public void saveEx() throws MedsysException
	{
		if (!save()) {
			String msg = null;
	 ...
	 *	@return true if saved
	 */
	private boolean saveFinish (boolean newRecord, boolean success, boolean updateIndex)
	{
		//	Translations
		if (success)
		{
		...
		try
		{
			...
			if (success && updateIndex) {
				try {
					QuickSearchTables.checkTables(this.getClass(), p_info.getTableName(), (Integer) m_IDs[0], m_trxName, p_ctx);
				} catch (Exception ex) {
					log.log(Level.WARNING, "QuickSearchTables.checkTables", ex);
				}
			}
			// Expert: Lama - Patient Log
			if (success && p_info.getAD_Table_ID() == MEXMEPaciente.Table_ID) {
				final MEXMEPatientLog log = new MEXMEPatientLog(getCtx(), m_trxName);
				log.saveLog(get_ID(), newRecord ? MEXMEPatientLog.EVENTCHANGELOG_Insert : MEXMEPatientLog.EVENTCHANGELOG_Update, null, 0);
			}
	...
	public void saveReplica (boolean isFromReplication) throws MedsysException
	{
	 ...
	 * @see #saveEx(String)
	 */
	public void saveEx(String trxName) throws MedsysException
	{
		set_TrxName(trxName);
		saveEx();
	}
	...
	protected boolean saveUpdate()
	{
		...
		table.setId(p_info.getAD_Table_ID());
		table.setName(p_info.getTableName());
		table.setPrintName(p_info.getTablePrintName());

		User user = new User();
		user.setId(Env.getAD_User_ID(getCtx()));
		user.setName(Env.getAD_User_Name(getCtx()));

		LogEntry logEntry = new LogEntry();
		logEntry.setCreated(System.currentTimeMillis());
		logEntry.setTable(table);
		logEntry.setUser(user);
		logEntry.setRecordId(get_ID());
		logEntry.setOrgId(Env.getAD_Org_ID(getCtx()));
		logEntry.setClientId(Env.getAD_Client_ID(getCtx()));
		logEntry.setType(X_AD_ChangeLog.EVENTCHANGELOG_Update);

		if (session == null) {
			logEntry.setSessionId(0);
		} else {
			logEntry.setSessionId(session.getAD_Session_ID());
		}
		...
		for (int i = 0; i < size; i++)
		{
			...
			if ((p_info.isSearchColumn(i) || "IsActive".equals(p_info.getColumnName(i)) ||
					"Value".equals(p_info.getColumnName(i)) || "Name".equals(p_info.getColumnName(i)) ||
					"Description".equals(p_info.getColumnName(i))) && is_ValueChanged(i)) {
				updateIndex = true;
			}
			if (value == null
					|| p_info.isVirtualColumn(i)) {
			...
			//
			//	updated/by
			if (columnName.equals(COLUMN_UpdatedBy))
			{
			...
			else if (columnName.equals(COLUMN_Updated))
			{
				if (updated) {
					continue;
				}
			...
			if (columnName.equals("DocumentNo"))
			{
				...
				if (strValue.startsWith("<") && strValue.endsWith(">"))
				{
					...
					if (value == null)	//	not overwritten by DocType and not manually entered
						//value = DB.getDocumentNo(AD_Client_ID, p_info.getTableName(), m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
						value = DB.getDocumentNo(p_ctx, p_info.getTableName(), false, m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
				}
				else {
			...
			} else if (c == Boolean.class)
			{
				...
				sql.append(
						DB.TO_DATE(
								...
								false //p_info.getColumnDisplayType(i) == DisplayType.Date
						)
				);
			} else {
			...
			)
			{
				...
				column.setId(p_info.getColumn(i).AD_Column_ID);
				column.setName(p_info.getColumn(i).ColumnName);
				column.setPrintName(p_info.getColumn(i).ColumnPrintName);
				Data data = new Data();
				data.setColumn(column);
				data.setNewValue(newV);
				data.setOldValue(oldV);
		...
		if (changes)
		{
			...
			{
				Timestamp now = DB.getDbTimestamp();
				set_ValueNoCheck(COLUMN_Updated, now);
			...
			return saveFinish (false, ok, updateIndex);
		}

		//  nothing changed, so OK
		return saveFinish (false, true, updateIndex);
	}   //  saveUpdate
	...
	private boolean saveNew()
	{
				...
				&& m_KeyColumns[0].endsWith("_ID"))	//	AD_Language, EntityType
		{
			...
			if (no <= 0)
			{
				...
				return saveFinish (true, false, true);
			}
		...
		if (index != -1)
		{
			...
			if (value == null || value.length() == 0)
			{
				...
				if (value == null) {	//	not overwritten by DocType and not manually entered
					//value = DB.getDocumentNo(getAD_Client_ID(), p_info.getTableName(), m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
					value = DB.getDocumentNo(p_ctx, p_info.getTableName(), false, m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
				}
		...
		if (index != -1)
		{
			...
			{
				//value = DB.getDocumentNo (getAD_Client_ID(), p_info.getTableName(), m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
				value = DB.getDocumentNo (p_ctx, p_info.getTableName(), true, m_trxName); //Expert: twry Cambio para multiorg y obtener el nivel de acceso
				set_ValueNoCheck(columnName, value);
			}
		}
		...
		logEntry.setOrgId(Env.getAD_Org_ID(getCtx()));
		logEntry.setClientId(Env.getAD_Client_ID(getCtx()));
		logEntry.setType(X_AD_ChangeLog.EVENTCHANGELOG_Insert);
		...
		for (int i = 0; i < size; i++)
		{
			...
			try
			{
				...
				} else if (c == Boolean.class)
				{
					...
					if (value.toString().length() == 0) {
						// [ 1722057 ] Encrypted columns throw error if saved as null
						// don't encrypt NULL
						sqlValues.append(DB.TO_STRING(value.toString()));
					} else {
						sqlValues.append(encrypt(i,DB.TO_STRING(value.toString())));
					}
				} else if (DisplayType.isLOB(dt)) {
			...
			if (m_IDs.length == 1 && !p_info.isVirtualColumn(i)) {
				Column column = new Column();
				column.setId(p_info.getColumn(i).AD_Column_ID);
				column.setName(p_info.getColumn(i).ColumnName);
				column.setPrintName(p_info.getColumn(i).ColumnPrintName);
...
//			eErrorBD = DB.eError;
		...
		return saveFinish (true, ok, true);
	}   //  saveNew
	...
	public boolean delete (boolean force)
	{
		...
		try
		{
			...
			catch (Exception e)
			{
				...
				log.saveError("Error", e, false); //	throw new DBException(e);
				return false;
			}
			...
			/*INICIO EXPERT - ASAEL Guardar el error en una variable para desplegarlo en ambiente web*/
			ValueNamePair vp = CLogger.retrieveError();
			if (vp != null) { //EXPERT.- RMontemayor
				DBError = vp.getName();
			}		          //EXPERT.- RMontemayor
			/*FIN EXPERT - ASAEL*/
			success = no == 1;
			...
			else
			{
				if (success) {
					...
					if (session == null){
						...
						logEntry.setOrgId(Env.getAD_Org_ID(getCtx()));
						logEntry.setClientId(Env.getAD_Client_ID(getCtx()));
						logEntry.setType(X_AD_ChangeLog.EVENTCHANGELOG_Delete);
						logEntry.setSessionId(session.getAD_Session_ID());
						...
						//Salvamos el ID antes de eliminarlo para removerlo del índice.
						ilTemporalId = (Integer) m_IDs[0];
						// Housekeeping
						m_IDs[0] = I_ZERO;
						if (m_trxName == null) {
							log.fine("complete");
			...
			try
			{
				...
				if (success) {
					try {
						QuickSearchTables.checkTables(this.getClass(), p_info.getTableName(), ilTemporalId, m_trxName, p_ctx);
					} catch (Exception ex) {
						log.log(Level.WARNING, "QuickSearchTables.checkTables", ex);
					}
				}
	 ...
	 * @see #delete(boolean)
	 */
	public void deleteEx(boolean force) throws MedsysException
	{
		if (!delete(force)) {
			String msg = null;
			ValueNamePair err = CLogger.retrieveError();
	 ...
	 */
	public void deleteEx(boolean force, String trxName) throws MedsysException
	{
		set_TrxName(trxName);
		deleteEx(force);
	}
	...
	/**
	 * 	Insert Accounting Records for Client. This one uses the client
	 *  from the context instead of the client of the records.
	 *	@param acctTable accounting sub table
	 *	@param acctBaseTable acct table to get data from
	 *	@param whereClause optional where clause with alisa "p" for acctBaseTable
	 *	@return true if records inserted
	 */
	protected boolean insert_Accounting_Client(String acctTable,
			String acctBaseTable, String whereClause) {
		if (s_acctColumns == null	//	cannot cache C_BP_*_Acct as there are 3
				|| acctTable.startsWith("C_BP_"))
		{
	 ...
	 */
	protected boolean insert_Accounting_Client(String acctTable,
			String acctBaseTable, String whereClause, int AD_Client_ID) {
		if (s_acctColumns == null	//	cannot cache C_BP_*_Acct as there are 3
				|| acctTable.startsWith("C_BP_"))
		{
			s_acctColumns = new ArrayList<String>();
		...
		//	..	SELECT
		sb.append(") SELECT ").append(get_ID())
		.append(", p.C_AcctSchema_ID, ").append(AD_Client_ID).append(",0,'Y', SysDate,")
		.append(getUpdatedBy()).append(",SysDate,").append(getUpdatedBy());
		for (int i = 0; i < s_acctColumns.size(); i++)
			sb.append(",p.").append(s_acctColumns.get(i));
		//	.. 	FROM
	...
	//expert : mrojas : comentado tempralmente hasta reorganizar codigo
	/**
	 *      Set the accounting document associated to the PO - for use in POST ModelValidator
	 *      @return Doc Document
	 *
	...
	public static List<String> modified(PO from, PO to, boolean newValues,
			boolean ignoreSearchKey){
		...
		for (int i = 0; i < from.m_oldValues.length; i++)
		{
			...
			if (colName.startsWith(COLUMN_Created)
				...
				;	//	ignore
			else if(ignoreSearchKey && colName.equals("Value") || colName.equals("DocumentNo"))
				; // ignore Value/DocumentNo columns - Expert:Lama
			else{
				if(from.is_new() || newValues){
					if(to.m_newValues[i] != from.m_newValues[i])
						columns.add(colName);
				}else{
					if(to.m_newValues[i] != from.m_oldValues[i])
						columns.add(colName);
				}
			}
	...
	/** No modificar */
	public POInfo getP_info() {
		return p_info;
	}


	public boolean load (Map<String, MetaData> locMap)
	{
		...
		//  load column values
		for (index = 0; index < size; index++)// numero de columnas del PO
		{
			String columnName = p_info.getColumnName(index);
			Class<?> clazz = p_info.getColumnClass(index);
			int dt = p_info.getColumnDisplayType(index);
			...
			{
				/**********/
				String key = String.valueOf(p_info.getAD_Column_ID(columnName));
				MetaData reg = locMap.get(key);// Nombre de columna, id de columna

				// si la tabla es la misma
				Object data = null;
				if(reg.getTableName()!=null){
					if ( reg.getTableName().equals(p_info.getTableName())
							|| reg.getTableName().equalsIgnoreCase(p_info.getNickname()))
						data = reg.getValue();
				} else {
					continue;//data = reg.getValue();
				}
				...
				if (clazz == Integer.class)
					m_oldValues[index] = decrypt(index, (Integer)data);
				else if (clazz == BigDecimal.class)
					m_oldValues[index] = decrypt(index, (BigDecimal)data);
				else if (clazz == Boolean.class)
					m_oldValues[index] = new Boolean ("Y".equals(decrypt(index, (String)data)));
				else if (clazz == Timestamp.class)
					m_oldValues[index] = decrypt(index, (Timestamp)data);
				else if (DisplayType.isLOB(dt))
					m_oldValues[index] = get_LOB (data);
				else if (clazz == String.class)
					m_oldValues[index] = decrypt(index, (String)data);
				else {
					m_oldValues[index] = null;//loadSpecial(rs, index);
					log.finest("(NOP) - " + p_info.getColumnName(index));
				}
				...
				//	NULL
				if (data == null && m_oldValues[index] != null)
					m_oldValues[index] = null;
				//
				if (CLogMgt.isLevelAll())
					log.finest(String.valueOf(index) + ": " + p_info.getColumnName(index)
	...
	public String getNickName() {
		if( p_info!=null && nickName == null && p_info.isMultiTables())
		{
			nickName = "t"+String.valueOf( p_info.getAD_Table_ID() );
			p_info.setNickname(nickName);
		}
		return nickName;
	}

	public void setNickName(String nickName) {
		this.p_info.setNickname(nickName);
	...
	/**
	 * Cadena con las caracteristicas del objeto
	 * @param namesColumns: Nombres de columnas a visualizar
	 * @return Cadena con los valores y nombres de columnas
	 */
	public String getDescriptionPO(boolean val) {
		if(descriptionPO==null || descriptionPO.equals("")) {
			StringBuilder descripcionDelObjeto = new StringBuilder();

			String[] namesColumns = getP_info().getSearchColumns();

			if(namesColumns!=null){

				for (int index = 0; index < namesColumns.length ; index++)
				{
					Object values = get_Value(namesColumns[index]);
					// Nombre
					descripcionDelObjeto.append(getP_info().getColumnLabel(getP_info().getColumnIndex(namesColumns[index])));
					descripcionDelObjeto.append(": ");
					// Valor
					descripcionDelObjeto.append(Utilerias.cambioValor(values, index));
					descripcionDelObjeto.append(" \r\n ");
				}

				descriptionPO = descripcionDelObjeto.toString();
			}
		} else {
			descriptionPO = " ";
		}
...
//	public LabelValueBean eErrorBD = null;

	public static String getKeyName(Properties ctx, String tableName, boolean orgCero){
		if(orgCero){
			return tableName+"_"+Env.getAD_Client_ID(ctx)+"_0";
		} else {
			return tableName+"_"+Env.getAD_Client_ID(ctx)+"_"+Env.getAD_Org_ID(ctx);
		}
	}
	public static String getKeyName(PO retValue){
		if(retValue != null){
			return retValue.get_TableName()+"_"+retValue.getAD_Client_ID()+"_"+retValue.getAD_Org_ID();
		} else {
			return "";
		}
	}
	...
	/**Set Custom Columns
	 * @param columns Map<ColumnName, Value>
	 * @author Lorena Lama
	 */
	public final void set_CustomColumns(Map<String, Object> columns) {
		if (columns != null) {
			for (Entry<String, Object> entry : columns.entrySet()) {
				this.set_CustomColumn(entry.getKey(), entry.getValue());
			}
		}
	}

	@SuppressWarnings("unchecked")
	public <T extends PO> T getPOModel(Properties ctx, int tableID, int recordID, String trxName) {
		T po = null;
		if(tableID > 0){
			po = (T) MTable.get(ctx, tableID).getPO(recordID, trxName);
		}
		return po;
	}
	...
	/**
	 * Actualiza una columna CLob
	 * @param columnClob Nombre de la columna a actualizar
	 * @param charArray contenido de la columna CLog
	 */
	public void updateClob(String columnClob, char[] charArray) {
		final StringBuilder sql = new StringBuilder();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			sql.append("SELECT ");
			sql.append(columnClob);
			sql.append(" FROM ");
			...
			log.log(Level.SEVERE, "Saving Clob: "+sql.toString(), e);
		}
		finally {
			DB.close(rs, pstmt);
			pstmt = null;
	...
	/** @return Notas relacionadas al registro */
	public List<MNote> getAD_Notes() {
		return is_new() ? new ArrayList<MNote>() : MNote.getList(getCtx(), get_Table_ID(), get_ID(), get_TrxName());
	}

	/**
	 * Revisa si el objeto es nuevo (para el caso que se haya hecho un save y posteriormente un rollback.
	 *
	 * @param trxName transaction name
	 */
	public void forceIsNew() {
		if (!is_new() && get_ID() > 0) {
			try {
				final int recordIdBD = new Query(getCtx(), get_TableName(), get_TableName() + "_ID=? ", null).setParameters(get_ID()).firstId();
				if (recordIdBD <= 0) {
					m_createNew = true;
					m_IDs[0] = I_ZERO;
				}
			} catch (Exception e) {
				log.log(Level.SEVERE, "forceIsNew: " + toString(), e);
			}
		}
	...
	/** @return Translated name from column Name */
	public String getMsgTranslate(String columnName){
		return Msg.translate(Env.getAD_Language(getCtx()), columnName);
	}

	@Override
	public Object onCompare(int type) {
		Object comparable;
		if (type == COLUMN_Updated.hashCode()) {
			comparable = getUpdated();
		} else if (type == COLUMN_UpdatedBy.hashCode()) {
			comparable = getUpdatedBy(true);
		} else if (type == COLUMN_Created.hashCode()) {
			comparable = getCreated();
		} else if (type == COLUMN_CreatedBy.hashCode()) {
			comparable = getCreatedBy(true);
		} else {
			comparable = "";
		}
		return comparable;
	...
	/**
	 * Dispara el workflow con la accion del parametro
	 * valida que el estatus sea el esperado y actualiza la informacion del objeto
	 *
	 * @param errorList Listado de errores (Obligatorio, debe estar vacio para iniciar el WF)
	 * @param docAction Accion del workflow (obligatorio)
	 * @param docStatus Estatus esperado como resultado del workflow (obligatorio)
	 * @param trxName Transaccion
	 */
	public void startWorkflow(ErrorList errorList, String docAction, String docStatus, String trxName) {
		try {
			if (StringUtils.isNotBlank(docAction) && StringUtils.isNotBlank(docStatus)
				&& errorList.isEmpty() && this instanceof DocAction) {
				DocAction poModel = (DocAction) this;
				this.set_TrxName(trxName);
				if (poModel.processIt(docAction) && //DocAction.ACTION_Complete
					docStatus.equals(poModel.getDocStatus())) {//DocAction.STATUS_Completed
					if (!poModel.save()) {
						errorList.add(new Error(Error.EXCEPTION_ERROR, new MedsysException().getLocalizedMessage()));
					}
				} else {
					errorList.add(new Error(Error.EXCEPTION_ERROR, Msg.parseTranslation(getCtx(), poModel.getProcessMsg())));
				}
			}
		} catch (Exception ex) {
	...
	/** Manda al log la sentencia SQL (Insert/Update/Delete), solo cuando esta en modo desarrollo */
	public void logSQL(StringBuffer sql) {
		if (Ini.APP_MODE_DEV.equals(Ini.getProperty(Ini.P_APPLICATION_MODE))
			&& (p_info.getTableName().contains("Patient")
			|| (!p_info.getTableName().startsWith("C_AcctProcessor")
				&& !p_info.getTableName().endsWith("Log")
				&& !p_info.getTableName().contains("Fact_Acct")
				&& !p_info.getTableName().equals("AD_Session")
				&& !p_info.getTableName().contains("PInstance")))) {
			log.severe(sql.toString());
			log.severe(m_trxName);
		} else {
			log.finest(sql.toString());
			log.finest(m_trxName);
		}
	}
	
	protected String getMandatoryMsg(final String columnName) {
		return Utilerias.getAppMsg(getCtx(), "error.model.mandatory", Msg.translate(getCtx(), columnName));
	}
	
	protected String getVirtualMsg(final String columnName) {
		return Utilerias.getAppMsg(getCtx(), "error.model.virtual", Msg.translate(getCtx(), columnName));
	}
	
	protected String getRefValueMsg(final String columnName) {
		return Utilerias.getAppMsg(getCtx(), "error.model.reference", Msg.translate(getCtx(), columnName));
	}
	
	/**
	 * Por medio de nombre de la columna busca si el registro
	 * ya existe previamente
	 * @param columnName
	 * @return
	 */
	public int alreadyExists(final String columnName) {
		int no = -1;
		if(m_KeyColumns!=null && m_KeyColumns[0]!=null && m_KeyColumns.length==1
				&& m_IDs!=null && m_IDs[0]!=null && m_IDs.length==1){

			final StringBuffer sql = new StringBuffer ("SELECT ").append(m_KeyColumns[0])
					.append(" FROM ").append(p_info.getTableName())
					.append(" WHERE IsActive = 'Y' AND ")
					.append(m_KeyColumns[0]).append(" <> ").append(m_IDs[0]);


			//
			int _index = p_info.getColumnIndex(columnName);
			if(_index>0){
				if(m_oldValues!=null && m_oldValues[_index]!=null && m_oldValues[_index] instanceof String)
					sql.append(" AND ").append(columnName).append(" = ").append(DB.TO_STRING(String.valueOf(m_oldValues[_index])));
			}

			no = DB.getSQLValue(m_trxName, sql.toString());
			logSQL(sql);
			log.fine("#" + no);
		}
		return no;
