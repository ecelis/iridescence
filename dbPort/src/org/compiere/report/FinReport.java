...
public class FinReport extends SvrProcess
{
	...
	/** Remove all nulls values			*/
	private boolean 			removingNulls = false;
	...
	private final StringBuffer		m_parameterWhere = new StringBuffer();
	/**	The Report Columns				*/
	...
	{
		final StringBuffer sb = new StringBuffer ("Record_ID=")
		.append(getRecord_ID());

		paReportID = getRecord_ID();
		...
		for (final ProcessInfoParameter element : para) {
			...
			if (element.getParameter() != null) {
				if ("C_Period_ID".equals(name)) {
					p_C_Period_ID = element.getParameterAsInt();
				} else if ("PA_Hierarchy_ID".equals(name)) {
				...
				} else if ("C_Project_ID".equals(name)) {
				...
				} else if ("C_SalesRegion_ID".equals(name)) {
					p_C_SalesRegion_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("C_Campaign_ID".equals(name)) {
					p_C_Campaign_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("User1_ID".equals(name)) {
					p_User1_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("User2_ID".equals(name)) {
					p_User2_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("UserElement1_ID".equals(name)) {
					p_UserElement1_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("UserElement2_ID".equals(name)) {
					p_UserElement2_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("DetailsSourceFirst".equals(name)) {
					p_DetailsSourceFirst = "Y".equals(element.getParameter());
				} else if ("PA_ReportCube_ID".equals(name)) {
					p_PA_ReportCube_ID = element.getParameterAsInt();
				} else if("OrgTrx_ID".equals(name)) {
					p_OrgTrx_ID = ((BigDecimal)element.getParameter()).intValue();
				} else if ("RemoveNulls".equals(name)){
					if("Y".equals(element.getParameter())){
						removingNulls = true;
					}
				} else {
		...
		if (p_OrgTrx_ID != 0) {
			m_parameterWhere.append(" AND ").append("AD_OrgTrx_ID = "+p_OrgTrx_ID);
		}
		//Fin Omar de la Rosa
		//	Optional BPartner
		...
		if (p_User1_ID != 0) {
			m_parameterWhere.append(" AND ").append(MReportTree.getWhereClause(getCtx(),
					p_PA_Hierarchy_ID, X_C_AcctSchema_Element.ELEMENTTYPE_UserList1, p_User1_ID));
		}
		//  Optional User2_ID
		if (p_User2_ID != 0) {
			m_parameterWhere.append(" AND ").append(MReportTree.getWhereClause(getCtx(),
					p_PA_Hierarchy_ID, X_C_AcctSchema_Element.ELEMENTTYPE_UserList2, p_User2_ID));
		}
	...
	private void setPeriods()
	{
		...
		final String sql = "SELECT p.C_Period_ID, p.Name, p.StartDate, p.EndDate, MIN(p1.StartDate) "
			+ "FROM C_Period p "
	...
	protected String doIt() throws Exception
	{
		...
		if ( p_PA_ReportCube_ID > 0 )
		{
			final MReportCube cube = new MReportCube(getCtx(), p_PA_ReportCube_ID, get_TrxName());
		...
		final int PA_ReportLineSet_ID = m_report.getLineSet().getPA_ReportLineSet_ID();
		final StringBuffer sql = new StringBuffer ("INSERT INTO T_Report "
		...
		//	Create Report
		final X_PA_Report paReport = new X_PA_Report(getCtx(), paReportID, get_TrxName());

		if (paReport.getJasperProcess_ID() > 0) {
			final MProcess mProcess = new MProcess(getCtx(), paReport.getJasperProcess_ID(), get_TrxName());
			if (StringUtils.isNotEmpty(mProcess.getJasperReport())) {
				// Financial Report
				final HashMap<String, Object> params = new HashMap<String, Object>();
				params.put("T_Report_AD_PInstance_ID", getAD_PInstance_ID());
				params.put("FECHA", new MPeriod(getCtx(), p_C_Period_ID, get_TrxName()).getName());
				params.put("ORGDEST", new MOrg(getCtx(), p_Org_ID, get_TrxName()).getName());
				Reporter reporter = null;

				try {
					commit();
					reporter = new Reporter(params, StringUtils.replace(mProcess.getJasperReport(), ".jasper", ""), getCtx(), null);
					final JasperPrint jasperPrint = reporter.generar();
					final ReportViewerProvider prov = ReportCtl.getReportViewerProvider();
					if (jasperPrint != null) {
						prov.openViewer(jasperPrint, Utilerias.getMessage(getCtx(), null, "imag.reporte"), true);
					}
				} catch (final Exception e) {
	...
	private void insertLine (final int line) {
		log.info(m_lines[line].toString());
		...
		for (int col = 0; col < m_columns.length; col++) {
			...
			if (m_lines[line].getPAAmountType() != null) {				//	line amount type overwrites column
				final String sql = m_lines[line].getSelectClause (true);
				select.append (sql);
				...
				final String sql = m_columns[col].getSelectClause (true);
				select.append (sql);
			...
			if (p_PA_ReportCube_ID > 0) {
				...
				select.append(" FROM Fact_Acct fa WHERE ");

				if (DB.isOracle()) {
					select.append("TRUNC(DateAcct, 'DD') ");
				} else if (DB.isPostgreSQL()) {
					select.append("DATE_TRUNC('day', DateAcct) ");
				}
			}
			...
			if (!m_lines[line].isPostingType()) {	//	only if not defined on line
				...
				final String PostingType = m_columns[col].getPostingType();
				if (PostingType != null && PostingType.length() > 0) {
					select.append(" AND PostingType='").append(PostingType).append("'");
				}
				// globalqss - CarlosRuiz
				if (PostingType.equals(X_PA_ReportColumn.POSTINGTYPE_Budget)
						&& m_columns[col].getGL_Budget_ID() > 0) {
		...
		if (update.length() > 0) {
			...
			final int no = DB.executeUpdate(update.toString(), get_TrxName());
	...
	private void doCalculations()
	{
		...
		for (int line = 0; line < m_lines.length; line++)
		{
			...
			BigDecimal lUtilidadEjercicio = Env.ZERO;
			...
			// Se pueda utilizar el cálculo de la utilidad del ejercicio en curso
			// como un cálculo más en la elaboración de los reportes financieros.
			if (m_lines[line].isCalculationTypeNetIncome()){

				// Periodo seleccionado o el actual
				final MPeriod period = new MPeriod(getCtx(), p_C_Period_ID, get_TrxName());

				// Calculo de la utilidad del ejericicio // 1476230.81
				lUtilidadEjercicio = CierreEjercicio.obtieneUtilidadPerdida(
						getCtx(),
						new MYear(getCtx(), period.getC_Year_ID(), get_TrxName()),
						period,
						period.getAD_Client_ID(),
						Env.ZERO,
						get_TrxName());

				// Actualizar la tabla del reporte en la columna cero
				final StringBuffer sb = new StringBuffer ("UPDATE T_Report SET ")
				.append(" Col_0 = ").append (lUtilidadEjercicio)
	...
	private String getLineIDs (final int fromID, final int toID)
	{
		...
		for (final MReportLine m_line : m_lines) {
			final int PA_ReportLine_ID = m_line.getPA_ReportLine_ID();
	...
	private void insertLineDetail()
	{
		...
		final String sql_select = "SELECT e.Name, fa.Description "
			+ "FROM Fact_Acct fa"
	...
	private void insertLineSource (final int line) {
		...
		final String variable = m_lines[line].getSourceColumnName();
		...
		//	Insert
		final StringBuffer insert = new StringBuffer("INSERT INTO T_Report ");
		insert.append("(AD_PInstance_ID, PA_ReportLine_ID, Record_ID,Fact_Acct_ID,LevelNo ");
		...
		for (final MReportColumn m_column : m_columns) {
			insert.append(", ");
			//	No calculation
			if (m_column.isColumnTypeCalculation()) {
				insert.append("NULL");
				continue;
			}
			...
			} else if (m_column.getPAAmountType() == null) {
				insert.append("NULL");
				continue;

			} else {
				select.append (m_column.getSelectClause (true));
			}
			...
			if (p_PA_ReportCube_ID > 0) {
				...
				//	Get Period info
				select.append(" FROM Fact_Acct fb WHERE ");
				if (DB.isOracle()) {
					select.append("TRUNC(DateAcct, 'DD') ");
				} else if (DB.isPostgreSQL()) {
					select.append("DATE_TRUNC('day', DateAcct) ");
			...
			final FinReportPeriod frp = getPeriod (m_column.getRelativePeriod());
			if (m_lines[line].getPAPeriodType() != null) {			//	line amount type overwrites column
				if (m_lines[line].isPeriod()) {
			...
			} else if (m_column.getPAPeriodType() != null) {
				if (m_column.isPeriod()) {
					select.append(frp.getPeriodWhere());
				} else if (m_column.isYear()) {
					select.append(frp.getYearWhere());
				} else if (m_column.isNatural()) {
					select.append(frp.getNaturalWhere("fb"));
				} else {
					select.append(frp.getTotalWhere());
				}
			...
			//	PostingType
			if (!m_lines[line].isPostingType()) {		//	only if not defined on line
				final String PostingType = m_column.getPostingType();
				if (PostingType != null && PostingType.length() > 0) {
					select.append(" AND fb.PostingType='").append(PostingType).append("'");
				}
				// globalqss - CarlosRuiz
				if (PostingType.equals(X_PA_ReportColumn.POSTINGTYPE_Budget)
					&& m_column.getGL_Budget_ID() > 0) {
						select.append(" AND GL_Budget_ID=" + m_column.getGL_Budget_ID());
				}
				// end globalqss
			}
			//	Report Where
			final String s = m_report.getWhereClause();

			if (s != null && s.length() > 0) {
				select.append(" AND ").append(s);
			}
			//	Limited Segment Values
			if (m_column.isColumnTypeSegmentValue()) {
				select.append(m_column.getWhereClause(p_PA_Hierarchy_ID));
			}
		...
		final StringBuffer where = new StringBuffer(m_lines[line].getWhereClause(p_PA_Hierarchy_ID));
		final String s = m_report.getWhereClause();
		if (s != null && s.length() > 0) {
			if (where.length() > 0) {
				where.append(" AND ");
		...
		//	Set Name,Description
		final StringBuffer sql = new StringBuffer ("UPDATE T_Report SET (Name,Description)=(")
	...
	private void insertLineTrx (final int line, final String variable)
	{
		...
		//	Period restriction
		final FinReportPeriod frp = getPeriod (0);

		if (DB.isOracle()) {
			insert.append(" AND TRUNC(DateAcct, 'DD') ");
		} else if (DB.isPostgreSQL()) {
			insert.append(" AND DATE_TRUNC('day', DateAcct) ");
		}
		...
		if (p_OrgTrx_ID != 0)
		 {
			insert.append(" AND AD_OrgTrx_ID= "+p_OrgTrx_ID);
		//Fin Omar de la Rosa
		}
		...
		final int no = DB.executeUpdate(insert.toString(), get_TrxName());
	...
	private void deleteUnprintedLines()
	{
		for (int line = 0; line < m_lines.length; line++)
		{
			...
			{
				final String sql = "DELETE FROM T_Report WHERE AD_PInstance_ID=" + getAD_PInstance_ID()
	...
	/**************************************************************************
	 *	Delete Columns where col_0 is null
	 */
	private void removeNulls() {
		final StringBuilder sql = new StringBuilder("DELETE FROM T_Report WHERE AD_PInstance_ID=" + getAD_PInstance_ID())
		.append(" AND Col_0 IS NULL AND Col_1 IS  NULL AND Col_2 IS NULL AND Col_3 IS NULL  ")
		.append(" AND Col_4 IS NULL AND Col_5 IS NULL AND Col_6 IS NULL ")
		.append(" AND Col_7 IS NULL AND Col_8 IS NULL AND Col_9 IS NULL ")
		.append(" AND Col_10 IS NULL AND Col_11 IS NULL AND Col_12 IS NULL ")
		.append(" AND Col_13 IS NULL AND Col_14 IS NULL AND Col_15 IS NULL ")
		.append(" AND Col_16 IS NULL AND Col_17 IS NULL AND Col_18 IS NULL ")
		.append(" AND Col_19 IS NULL AND Col_20 IS NULL ");
		final int no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no > 0) {
			log.fine("Columns  - #" + no);
		}
	...
	private void scaleResults() {
		...
		{
			final String factor = m_columns[column].getFactor();
			if ( factor != null )
			{
				int divisor = 1;
				if ( "k".equals(factor) ) {
				...
				final String sql = "UPDATE T_Report SET Col_" + column
				+ "=Col_" + column + "/" + divisor
	...
	private MPrintFormat getPrintFormat()
	{
		...
		if (createNew)
		{
			pf = MPrintFormat.createFromTable(Env.getCtx(), 544);//	T_Report
		...
		final int count = pf.getItemCount();
		for (int i = 0; i < count; i++)
		{
			...
			else if (ColumnName.startsWith("Col"))
			{
				...
				if (index < m_columns.length)
				{
					...
					{
						final BigDecimal relativeOffset = m_columns[index].getRelativePeriod();
						final FinReportPeriod frp = getPeriod (relativeOffset);
			...
			else if ("SeqNo".equals(ColumnName))
			{
				if (pfi.isPrinted()) {
					pfi.setIsPrinted(false);
				}
			...
			else if ("LevelNo".equals(ColumnName))
			{
				if (pfi.isPrinted()) {
					pfi.setIsPrinted(false);
				}
