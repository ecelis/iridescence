...
public class MSequence extends X_AD_Sequence
{
	...
	private static final String USE_PROC_FOR_SEQ = "useProcedureForSeq";
...
//					if (hasEntityType && ( ! compiereSys ) && ( ! isExceptionCentralized(TableName) ) ) {
//
...
//			USE_PROCEDURE = true;
		...
		Properties pathsProps = WebEnv.getXPT_Properties();
		if(pathsProps.containsKey(USE_PROC_FOR_SEQ)) {
			USE_PROCEDURE = "Y".equals(pathsProps.getProperty(USE_PROC_FOR_SEQ));
		}
...
//						hasEntityType = true;
					...
					else
					{
						...
						if (compiereSys) {
							updateSQL =
									conn.prepareStatement("UPDATE AD_Sequence SET CurrentNextSys = CurrentNextSys + ? WHERE AD_Sequence_ID = ?");
							retValue = rs.getInt(2);
						} else {
							updateSQL =
									conn.prepareStatement("UPDATE AD_Sequence SET CurrentNext = CurrentNext + ? WHERE AD_Sequence_ID = ?");
							retValue = rs.getInt(1);
						}
						try {
							updateSQL.setInt(1, incrementNo);
			...
			catch (Exception e)
			{
				...
				try
				{
					...
					s_log.log(Level.SEVERE, TableName + " - " + e.getMessage(), e1);
				}
			}
			finally
			{
			...
			if (rs.next())
			{
				...
				if (USE_PROCEDURE)
				{
					...
					? nextIDByYear(conn, AD_Sequence_ID, incrementNo, calendarYear)
							: nextID(conn, AD_Sequence_ID, compiereSys);
				}
				else
				{
	 ...
	 *	@return document no or null
	 */
	//public static synchronized String getDocumentNo (int  AD_Client_ID, String TableName, String trxName) //twry
	public static String getDocumentNo (Properties ctx, String TableName, boolean value, String trxName)
	{
		...
		int AD_Client_ID = Env.getContextAsInt(ctx, "#AD_Client_ID"); //twry
		...
		String selectSQL = ""; //Expert : Twry Inicio Cambios para las secuencias de multiOrg
		//Excluimos las tablas del diccionario de datos con columna DocumentNo
		if(!TableName.startsWith("A_") && !TableName.startsWith("AD_")) {

			//Consulta
			selectSQL = " SELECT sequence.CurrentNext, sequence.CurrentNextSys, sequence.IncrementNo, "
				+ " sequence.Prefix, sequence.Suffix, sequence.AD_Sequence_ID "
				+ " FROM AD_Sequence sequence "
				+ " WHERE sequence.Name LIKE (?) "
				+ " AND sequence.IsActive='Y' AND sequence.IsTableID='N' AND sequence.IsAutoSequence='Y' ";

			//con el nivel de acceso del tipo de documento deacuerdo al proceso de multiorg
			int AD_Table_ID = MTable.getTable_ID(TableName);
			MTable t = new MTable(ctx, AD_Table_ID, trxName);
			if (t.getAccessLevel().equals(MTable.ACCESSLEVEL_ClientToSystem)) {
				selectSQL = MEXMELookupInfo.addAccessLevelSQL(ctx, selectSQL, TableName, "sequence");
			} else {
				selectSQL = MEXMELookupInfo.addAccessLevelSQL(ctx, selectSQL, "C_DocType", "sequence");
			}

			//Ordenar por organizacion
			selectSQL = selectSQL + " ORDER BY sequence.AD_Org_ID DESC, sequence.AD_Client_ID DESC  ";

		} else {//Expert : Twry Fin Cambios para las secuencias de multiOrg
			//Consulta Original de compiere
		...
		}//Expert : Twry  Cambios para las secuencias de multiOrg

		//System.out.println("SQL Sequence documentNo : " + selectSQL);
		...
		try
		{
			...
			pstmt = conn.prepareStatement(selectSQL,
					ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
			//pstmt.setString(1, PREFIX_DOCSEQ + TableName + "@%"); //twry proximo considere que hay
			pstmt.setString(1, PREFIX_DOCSEQ + TableName);
			if(TableName.startsWith("A_") || TableName.startsWith("AD_")) {//twry
				pstmt.setInt(2, AD_Client_ID);
			}//twry
	...
	{
		return get(ctx, tableName, null, null);
	}
	...
	{
		return get(ctx, tableName, null, trxName);
	}
	 ...
	 *	@return Sequence
	 */
	public static MSequence get (Properties ctx, String tableName, String whereClause, String trxName)
	{
		StringBuilder sql = new StringBuilder(Constantes.INIT_CAPACITY_ARRAY);
		sql.append("SELECT * FROM AD_Sequence ")
		.append(" WHERE UPPER(Name) = ? ")
		.append(" AND IsTableID = 'Y' ");
		//Liz de la Garza - Condicion where
		if (whereClause != null) {
	...
	public static MSequence getADSequenceIDFromSequenceName (Properties ctx, String tableName)
	{
	...
	public MSequence (Properties ctx, int AD_Client_ID, String tableName, String trxName)
	{
		this (ctx, 0, trxName);
		/*
		 * freyes Modificacion para tomar ad_org_id de contexto y no un 0
		 * (Cambio para Secuencias y c_doctype a nivel de organizacion)
		 */
		setClientOrg(AD_Client_ID, Env.getAD_Org_ID(Env.getCtx()));//	Client Ownership
	 ...
	 */
	private static boolean forceValue(boolean value , String TableName) {
		...
		try {
			StringBuilder sql = new StringBuilder(" SELECT c.* FROM AD_Column c ")
			.append(" INNER JOIN AD_Table t on (c.AD_Table_ID = t.AD_Table_ID ) ")
			.append(" WHERE UPPER(t.TableName) LIKE UPPER(?) and c.ColumnName = 'Value' ");


			pstmtSql = DB.prepareStatement(sql.toString(),null);
			pstmtSql.setString(1, TableName);

			rsSql = pstmtSql.executeQuery();

			if (rsSql.next()) {
				column = new MColumn(Env.getCtx(), rsSql, null);
			}
	...
	public static Integer getSecuencia (int AD_Client_ID, String name, String trxName)
	{
		...
		//
		if (CLogMgt.isLevel(LOGLEVEL))
			s_log.log(LOGLEVEL, name + " - CompiereSys=" + compiereSys  + " [" + trxName + "]");
		...
		try
		{
			...
			{
				s_log.warning ("(Table) - no record found - " + name);
				MSequence seq = new MSequence (Env.getCtx(), AD_Client_ID, name, null);
				next = seq.getNextID();
				seq.save();
			}
			rs.close();
	...
	/**
	 * Obtiene MSequence a partir del name
	 * @param ctx
	 * @param name
	 * @return sequence
	 *
	 * **/
	public static MSequence getSequence(Properties ctx, String name){

		MSequence sequence = null;
		String sql = "SELECT * FROM AD_Sequence "
			+ "WHERE isActive= 'Y' AND Name like ? ";
		...
		try
		{
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next())
				sequence = new MSequence(ctx, rs, null);
