...
public abstract class Doc
{
	...
	public static int[]  documentsTableID = new int[] {
		...
		//expert : gisela lee : se posteara matchpo antes que inout
		...
		//expert : gisela lee : se intentara postear antes que inout
		//MMatchPO.Table_ID,		    //  M_MatchPO
		//expert : gisela lee : se intentara postear antes que inout
		MProjectIssue.Table_ID,		//	C_ProjectIssue
		MRequisition.Table_ID,		//	M_Requisition
		MEXMEPresupuestoEgr.Table_ID, // Card. 895	Import and Post Budget: EXME_Presupuesto Presupuesto
		MEXMEPresupuestoModif.Table_ID // Card. 895	Import and Post Budget: EXME_PresupuestoModif Modificaciones al presupuesto
	};
	...
	public static String[]  documentsTableName = new String[] {
		...
		MEXMEPresupuestoEgr.Table_Name, // Card. 895	Import and Post Budget: EXME_Presupuesto Presupuesto
		MEXMEPresupuestoModif.Table_Name // Card. 895	Import and Post Budget: EXME_PresupuestoModif Modificaciones al presupuesto
	};
	...
	/**	AR Debit Memo          */
	public static final String 	DOCTYPE_ARDebitMemo     = MDocType.DOCBASETYPE_ARDebitMemo; //Expert. TTPR # Posteo Notas de Debito
	...
	/**	AR Debit Memo          */
	public static final String 	DOCTYPE_APDebitMemo     = MDocType.DOCBASETYPE_APDebitMemo; //Expert. TTPR # Posteo Notas de Debito
	...
	/** GL                      */
	public static final String 	DOCTYPE_GLDocument       = "GLD";// Card. 895	Import and Post Budget: Presupuestos



	//  Posting Status - AD_Reference_ID=234     //
	public static final int POSTEDSTATUS_AD_Reference_ID=234;
	/**	Document Status         */
	...
	//expert : gisela lee : cambios de jjanke en version 26b
	/** Elemento de esquema definido por el usuario */
	private MAcctSchemaElement beanUserElement1 = null;
    /** Elemento de esquema definido por el usuario */
	private MAcctSchemaElement beanUserElement2 = null;
	/**	Document Status         */
	public static final String STATUS_Unposted         = "Z";
	...
	public static Doc get (MAcctSchema[] ass, int AD_Table_ID, ResultSet rs, String trxName)
	 {
		...
		} else if(AD_Table_ID == MEXMEPresupuestoEgr.Table_ID) {// Card. 895	Import and Post Budget: Presupuesto
			doc = new Doc_PresupuestoEgr(ass, MEXMEPresupuestoEgr.class, rs, trxName);
		} else if(AD_Table_ID == MEXMEPresupuestoModif.Table_ID) {// Card. 895	Import and Post Budget: Presupuesto
			doc = new Doc_PresupuestoEgr(ass, MEXMEPresupuestoModif.class, rs, trxName);
		}
	...
	Doc (MAcctSchema[] ass, Class clazz, ResultSet rs, String defaultDocumentType, String trxName)
	{
		...
		// Agregar el esquema contable de system al contexto
		m_ctx.setProperty("$C_DefaultAcctSchema_ID", String.valueOf(loadDefaultAcctSchema ()));
	...
	/** Generar o no asiento contable para moviemiento */
	protected boolean			p_PostMovement = true;
	/** Document Date				*/
	protected Timestamp			m_DateConvert = null;
	...
	public final String post (boolean force, boolean repost)
	{
		...
		//  Create Note
		if (!STATUS_Posted.equals(p_Status))
		{
			//  Insert Note
			...
			//	Text
			errorMsg = new StringBuffer (Msg.getMsg(Env.getCtx(), AD_MessageValue));
			if (p_Error != null)
				errorMsg.append(" (").append(p_Error).append(")");
			String cn = getClass().getName();
			errorMsg.append(" - ").append(cn.substring(cn.lastIndexOf('.')))
			...
			note.setTextMsg(errorMsg.toString());
			note.save();

			log.severe(
					"Posting table " + get_TableName()
					+ ", Record_ID = " + get_ID()
					+ " - " + errorMsg.toString()
			);
		}
		...
		if (STATUS_Posted.equals(p_Status) && errorMsg == null) {
			p_Error = null;
		} else {
			p_Error = errorMsg.toString();
		}
	...
	private final String postLogic (int index)
	{
		...
		for (int f = 0; f < facts.size(); f++)
		{
			...
			//  balanceSource
			if (!fact.isSourceBalanced())// -0.01
			{
				fact.balanceSource(); // 0.13
				if (!fact.isSourceBalanced()) // 0.12
					return STATUS_NotBalanced;
			}
	...
	public boolean isBalanced()
	{
		...
		if (retValue)
			...
			log.warning("NO (isBalanced) - " + toString());
		return retValue;
	}	//	isBalanced
		...
		while (it.hasNext() && convertible)
		{
			...
			if (C_Currency_ID != acctSchema.getC_Currency_ID())
			{
				BigDecimal amt = MConversionRate.getRate (C_Currency_ID, acctSchema.getC_Currency_ID(),
					getDateConvert()==null?getDateAcct():getDateConvert(), getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
				if (amt == null)
				{
					convertible = false;
	...
	public boolean isPeriodOpen()
	{
		...
		if (open)
			...
			log.warning("NO (isPeriodOpen) - " + toString());
		return open;
	}	//	isPeriodOpen

	/**
	 *	is On Consignment
	 *  @return true if is On Consignment
	 */
	public boolean isPostMovement()
	{
		if (p_PostMovement){
			log.warning("Yes PostMovement - " + toString());
		} else {
			log.fine("NO PostMovement - " + toString());
		}
		return p_PostMovement;
	}	//	isPeriodOpen
	...
	/** Account Type - Payment - Prepayment */
	public static final int 	ACCTTYPE_C_Prepayment_Employee = 900;
	/** Account Type - Payment - Prepayment *///E_Expense_Acct
	public static final int 	ACCTTYPE_E_Expense_Employee = 901;
	...
	// Card. 895	Import and Post Budget:
	/** Presupuesto de Egresos Aprobado
	C_AcctSchema_GL.PresEgrAprob_Acct
	Configurar el valor:	8.2.1 */
	public static final int     ACCTTYPE_PresAprobado = 821;

	/** Presupuesto de Egresos por Ejercer
	C_AcctSchema_GL.PresEgrxEjer_Acct
	Configurar el valor:	8.2.2 */
	public static final int     ACCTTYPE_PresEjercer = 822;

	/** Modificaciones al Presupuesto de Egresos Aprobado
	C_AcctSchema_GL.ModPresEgrAprob_Acct
	Configurar el valor:	8.2.3 */
	public static final int     ACCTTYPE_PresModificado = 823;

	/** Presupuesto de Egresos Comprometido
	C_AcctSchema_GL.PresEgrComp_Acct
	Configurar el valor:	8.2.4 */
	public static final int     ACCTTYPE_PresComprometido = 824;

	/** Presupuesto de Egresos Devengado
	C_AcctSchema_GL.PresEgrDeve_Acct
	Configurar el valor:	8.2.5 */
	public static final int     ACCTTYPE_PresDevengado = 825;
	/** Presupuesto de Egresos Ejercido
	C_AcctSchema_GL.PresEgrEjer_Acct
	Configurar el valor:	8.2.6 */
	public static final int     ACCTTYPE_PresEjercido = 826;
	/** resupuesto de Egresos Pagado
	C_AcctSchema_GL.PresEgrPaga_Acct
	Configurar el valor:	8.2.7 */
	public static final int     ACCTTYPE_PresPagado = 827;
	/** Ley de Ingresos Estimada
	C_AcctSchema_GL.LeyIngEsti_Acct
	Configurar el valor:	8.1.1 */
	public static final int     ACCTTYPE_PresLeyIngEstimada = 811;
	/** Ley de Ingresos por Ejecutar
	C_AcctSchema_GL.LeyIngxEjec_Acct
	Configurar el valor:	8.1.2 */
	public static final int     ACCTTYPE_PresLeyIngEjecutar = 812;
	/** Ley de Ingresos Devengada
	C_AcctSchema_GL.LeyIngDeve_Acct
	Configurar el valor:	8.1.4 */
	public static final int     ACCTTYPE_PresLeyIngDevengada = 814;
	/** Ley de Ingresos Recaudada
	C_AcctSchema_GL.LeyIngReca_Acct
	Configurar el valor:	8.1.5 */
	public static final int     ACCTTYPE_PresLeyIngRecaudada = 815;
	/** Superávit Financiero
	C_AcctSchema_GL.SuperFina_Acct
	Configurar el valor:	9.1 */
	public static final int     ACCTTYPE_PresSuperavitFinanciero = 91;
	/** ADEFAS
	C_AcctSchema_GL.Adefas_Acct
	Configurar el valor:	9.3 */
	public static final int     ACCTTYPE_PresADEFAS = 93;
	// Fin Card. 895	Import and Post Budget:
	 ...
	 *  Expert: #Post,Cost And Price : Se agrego a cada consulta
	 *  AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC
	 *  para obtener la cuenta del cliente y en caso de no exitir
	 *  el registro busca a nivel system
	 */
	public int getValidCombination_ID (int AcctType, MAcctSchema as)
	{
		...
		if (AcctType == ACCTTYPE_Charge)	//	see getChargeAccount in DocLine
		{
			...
			if (cmp == 0)
				...
				sql = "SELECT CH_Expense_Acct FROM C_Charge_Acct WHERE C_Charge_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			else
				sql = "SELECT CH_Revenue_Acct FROM C_Charge_Acct WHERE C_Charge_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_Charge_ID();
		}
		else if (AcctType == ACCTTYPE_V_Liability)
		{
			sql = "SELECT V_Liability_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_V_Liability_Services)
		{
			sql = "SELECT V_Liability_Services_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_C_Receivable)
		{
			sql = "SELECT C_Receivable_Acct FROM C_BP_Customer_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_C_Receivable_Services)
		{
			sql = "SELECT C_Receivable_Services_Acct FROM C_BP_Customer_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_V_Prepayment)
		{
			sql = "SELECT V_Prepayment_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_C_Prepayment)
		{
			sql = "SELECT C_Prepayment_Acct FROM C_BP_Customer_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_C_Prepayment_Employee)
		{
			sql = "SELECT E_Prepayment_Acct FROM C_BP_Employee_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_E_Expense_Employee)
		{
			sql = "SELECT E_Expense_Acct FROM C_BP_Employee_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		...
		{
			sql = "SELECT B_UnallocatedCash_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		else if (AcctType == ACCTTYPE_BankInTransit)
		{
			sql = "SELECT B_InTransit_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		else if (AcctType == ACCTTYPE_PaymentSelect)
		{
			sql = "SELECT B_PaymentSelect_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		...
		else if (AcctType == ACCTTYPE_DiscountExp)
		{
			sql = "SELECT a.PayDiscount_Exp_Acct FROM C_BP_Group_Acct a, C_BPartner bp "
				+ "WHERE a.C_BP_Group_ID=bp.C_BP_Group_ID AND bp.C_BPartner_ID=? AND a.C_AcctSchema_ID IN (?,?) ORDER BY a.AD_Client_ID DESC ";
			para_1 = getC_BPartner_ID();
		}
		...
		{
			sql = "SELECT B_Asset_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		else if (AcctType == ACCTTYPE_InterestRev)
		{
			sql = "SELECT B_InterestRev_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		else if (AcctType == ACCTTYPE_InterestExp)
		{
			sql = "SELECT B_InterestExp_Acct FROM C_BankAccount_Acct WHERE C_BankAccount_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_BankAccount_ID();
		}
		...
		{
			sql = "SELECT CB_Asset_Acct FROM C_CashBook_Acct WHERE C_CashBook_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_CashBook_ID();
		}
		else if (AcctType == ACCTTYPE_CashTransfer)
		{
			sql = "SELECT CB_CashTransfer_Acct FROM C_CashBook_Acct WHERE C_CashBook_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_CashBook_ID();
		}
		else if (AcctType == ACCTTYPE_CashExpense)
		{
			sql = "SELECT CB_Expense_Acct FROM C_CashBook_Acct WHERE C_CashBook_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_CashBook_ID();
		}
		else if (AcctType == ACCTTYPE_CashReceipt)
		{
			sql = "SELECT CB_Receipt_Acct FROM C_CashBook_Acct WHERE C_CashBook_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_CashBook_ID();
		}
		else if (AcctType == ACCTTYPE_CashDifference)
		{
			sql = "SELECT CB_Differences_Acct FROM C_CashBook_Acct WHERE C_CashBook_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_CashBook_ID();
		}
		...
		{
			sql = "SELECT W_Differences_Acct FROM M_Warehouse_Acct WHERE M_Warehouse_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			//  "SELECT W_Inventory_Acct, W_Revaluation_Acct, W_InvActualAdjust_Acct FROM M_Warehouse_Acct WHERE M_Warehouse_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
		...
		{
			sql = "SELECT PJ_Asset_Acct FROM C_Project_Acct WHERE C_Project_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_Project_ID();
		}
		else if (AcctType == ACCTTYPE_ProjectWIP)
		{
			sql = "SELECT PJ_WIP_Acct FROM C_Project_Acct WHERE C_Project_ID=? AND C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = getC_Project_ID();
		}
		...
		{
			sql = "SELECT PPVOffset_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		else if (AcctType == ACCTTYPE_CommitmentOffset)
		{
			sql = "SELECT CommitmentOffset_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		...
		C_AcctSchema_GL.PresEgrAprob_Acct
		Configurar el valor:	8.2.1 */
		else if (AcctType == ACCTTYPE_PresAprobado)
		{
			sql = "SELECT PresEgrAprob_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		...
		C_AcctSchema_GL.PresEgrxEjer_Acct
		Configurar el valor:	8.2.2 */
		else if (AcctType == ACCTTYPE_PresEjercer)
		{
			sql = "SELECT PresEgrxEjer_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		...
		C_AcctSchema_GL.ModPresEgrAprob_Acct
		Configurar el valor:	8.2.3 */
		else if (AcctType == ACCTTYPE_PresModificado)
		{
			sql = "SELECT ModPresEgrAprob_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		...
		C_AcctSchema_GL.PresEgrComp_Acct
		Configurar el valor:	8.2.4 */
		else if (AcctType == ACCTTYPE_PresComprometido)
		{
			sql = "SELECT PresEgrComp_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		...
		C_AcctSchema_GL.PresEgrDeve_Acct
		Configurar el valor:	8.2.5 */
		else if (AcctType == ACCTTYPE_PresDevengado)
		{
			sql = "SELECT PresEgrDeve_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Presupuesto de Egresos Ejercido
		C_AcctSchema_GL.PresEgrEjer_Acct
		Configurar el valor:	8.2.6 */
		else if (AcctType == ACCTTYPE_PresEjercido)
		{
			sql = "SELECT PresEgrEjer_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** resupuesto de Egresos Pagado
		C_AcctSchema_GL.PresEgrPaga_Acct
		Configurar el valor:	8.2.7 */
		else if (AcctType == ACCTTYPE_PresPagado)
		{
			sql = "SELECT PresEgrPaga_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Ley de Ingresos Estimada
		C_AcctSchema_GL.LeyIngEsti_Acct
		Configurar el valor:	8.1.1 */
		else if (AcctType == ACCTTYPE_PresLeyIngEstimada)
		{
			sql = "SELECT LeyIngEsti_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Ley de Ingresos por Ejecutar
		C_AcctSchema_GL.LeyIngxEjec_Acct
		Configurar el valor:	8.1.2 */
		else if (AcctType == ACCTTYPE_PresLeyIngEjecutar)
		{
			sql = "SELECT LeyIngxEjec_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Ley de Ingresos Devengada
		C_AcctSchema_GL.LeyIngDeve_Acct
		Configurar el valor:	8.1.4 */
		else if (AcctType == ACCTTYPE_PresLeyIngDevengada)
		{
			sql = "SELECT LeyIngDeve_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Ley de Ingresos Recaudada
		C_AcctSchema_GL.LeyIngReca_Acct
		Configurar el valor:	8.1.5 */
		else if (AcctType == ACCTTYPE_PresLeyIngRecaudada)
		{
			sql = "SELECT LeyIngReca_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** Superávit Financiero
		C_AcctSchema_GL.SuperFina_Acct
		Configurar el valor:	9.1 */
		else if (AcctType == ACCTTYPE_PresSuperavitFinanciero)
		{
			sql = "SELECT SuperFina_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		/** ADEFAS
		C_AcctSchema_GL.Adefas_Acct
		Configurar el valor:	9.3 */
		else if (AcctType == ACCTTYPE_PresADEFAS)
		{
			sql = "SELECT Adefas_Acct FROM C_AcctSchema_GL WHERE C_AcctSchema_ID IN (?,?) ORDER BY AD_Client_ID DESC ";
			para_1 = -1;
		}
		// Fin Card. 895	Import and Post Budget:
		else
		...
		try
		{
			...
			if (para_1 == -1){   //  GL Accounts
				...
				pstmt.setInt (2, Env.getContextAsInt(getCtx(), "$C_DefaultAcctSchema_ID"));// Expert: #Post,Cost And Price
			...
			else
			{
				...
				// Expert: Proyecto #102 Posteo, costos y precios
				// Cuentas contables de System por defecto
				pstmt.setInt (3, Env.getContextAsInt(getCtx(), "$C_DefaultAcctSchema_ID"));// Expert: #Post,Cost And Price
			}
	 ...
	 */
	protected void setDateConvert(MInvoice nota) {
		if (nota.getRef_Invoice_ID() > 0 && X_C_DocType.DOCBASETYPE_ARCreditMemo.equals(nota.getC_DocType().getDocBaseType())) {

			if (X_C_Invoice.TRXTYPE_PartialCreditNoteCustomer.equals(nota.getTrxType())) {
				m_DateConvert = nota.getDateInvoiced();

			} else {
				final MInvoice factura = new MInvoice(getCtx(), nota.getRef_Invoice_ID(), null);
				m_DateConvert = factura.getDateInvoiced();
			}
		}
	}
	 ...
	 * 	Set Date Convert
	 */
	protected Timestamp getDateConvert ()
	{
	 ...
	 * Default AcctSchema
	 * Expert: #Post,Cost And Price
	 * El esquema contable de System
	 */
	private int loadDefaultAcctSchema ()
	{
		//
		...
		try
		{
			...
			if (rs.next()) {
				return rs.getInt("C_AcctSchema_ID");
			}
	...
	/**
	 * Elemento del esquema cont. definido por el usuario
	 * @param m_C_AcctSchema_ID Esquema
	 * @return
	 * @throws Exception
	 */
	public String getBeanUserElement1(final int m_C_AcctSchema_ID) throws Exception {
		if(beanUserElement1==null && m_C_AcctSchema_ID>0){
			beanUserElement1 = MAcctSchemaElement.getAcctSchemaElementUserElement(
					Env.getCtx(),   m_C_AcctSchema_ID, X_C_AcctSchema_Element.ELEMENTTYPE_UserElement1,  m_trxName);
		}

		if(beanUserElement1==null){
			return "";
		} else {
			if(beanUserElement1.getAD_Column()==null){
				return "";
			} else {
				return beanUserElement1.getAD_Column().getColumnName();
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	public String getBeanUserElement2(final int m_C_AcctSchema_ID) throws Exception {
		if(beanUserElement2==null && m_C_AcctSchema_ID>0){
			beanUserElement2 = MAcctSchemaElement.getAcctSchemaElementUserElement(
					Env.getCtx(),   m_C_AcctSchema_ID, X_C_AcctSchema_Element.ELEMENTTYPE_UserElement2,  m_trxName);
		}
		if(beanUserElement2==null){
			return "";
		} else {
			if(beanUserElement2.getAD_Column()==null){
				return "";
			} else {
				return beanUserElement2.getAD_Column().getColumnName();
			}
		}
	}
