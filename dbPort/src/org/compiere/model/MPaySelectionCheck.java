 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: MPaySelectionCheck.java,v 1.5 2006/08/11 02:26:22 mrojas Exp $
 */
...
//		ps.save();
		...
		MPaySelectionCheck psc = payment.createPaySelectionCheck();
//		= new MPaySelectionCheck(ps, PaymentRule);
...
//		psc.save();
		...
		if (payment.getC_Invoice_ID() != 0) {
			psl = new MPaySelectionLine (ps, 10, psc.getPaymentRule());
			...
			if (pAllocs.length != 0) {
				...
				for (int i = 0; i < pAllocs.length; i++) {
					...
					if (pAlloc.getC_Invoice_ID() != 0) {
						...
						psla = new MPaySelectionLine (ps, 10 * (i+1), psc.getPaymentRule());
	 ...
	 */
	public static int confirmPrint (MPaySelectionCheck[] checks, MPaymentBatch batch, String concept)
	{
		...
		for (int i = 0; i < checks.length; i++)
		{
			...
			payment.setDescription(concept);
			//	Existing Payment
...
//				payment.setDeCscription("TEST F");
				...
				if (check.getQty() == 1 && psls != null && psls.length == 1)
				{
					...
					//**Expert:Juan
					//int inv = psl.getC_Invoice_ID();
					//int paym = psl.getC_Payment_ID();
					if (psl.getC_Invoice_ID() <= 0) //&& psl.getC_Payment_ID() > 0)
					...
					payment.setDescription(concept == null ? psl.getC_Invoice().getDescription(): concept);
				}
				else{
	...
	public MPaySelectionCheck (MPaySelectionLine line, String PaymentRule)
	{
		...
		/*Expert:Juan*/
		//MInvoice minv =  line.getInvoice();
		MPayment mpay = line.getPayment();
		//int socio =  mpay.getC_BPartner_ID();
		if (mpay.getC_BPartner_ID() > 0)//Juan 
			setC_BPartner_ID (line.getPayment().getC_BPartner_ID());//Juan
		else//Juan
	...
	public void addLine (MPaySelectionLine line)
	{
		...
		//int soci = line.getPayment().getC_BPartner_ID();
		//int soc = line.getInvoice().getC_BPartner_ID();
		//if (getC_BPartner_ID() != line.getInvoice().getC_BPartner_ID() )//original
		if (getC_BPartner_ID() != line.getInvoice().getC_BPartner_ID() && getC_BPartner_ID() != line.getPayment().getC_BPartner_ID())//Juan
	...
	public void setParent(MPaySelection parent) {
	...
	/**
	 * 
	 * @param C_PaySelection_ID ID de la Seleccion de Pago
	 * @param Ctx Contexto
	 * @return Cantidad de Cheques Generados en una Seleccion de Pago
	 */
	public static int countCheck(int C_PaySelection_ID, Properties Ctx) {
		int checks = 0;
		
		String sql = "SELECT COUNT(*) " + "FROM C_PaySelectionCheck "
					+ "WHERE C_PaySelection_ID=?";
		StringBuilder sb=new StringBuilder(MEXMELookupInfo.addAccessLevelSQL(Ctx, sql, MPaySelectionCheck.Table_ID));
