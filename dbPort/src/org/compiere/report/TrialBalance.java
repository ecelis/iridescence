...
public class TrialBalance extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		//
		//EXPERT Omar de la Rosa
		if(p_PA_Hierarchy_ID != 0 && p_AD_OrgTrx_ID != 0){
			m_parameterWhere.append(" AND AD_OrgTrx_ID in (select node_id from ad_treenode where ad_tree_id = ").
			append("(select ad_tree_id from ad_tree where ad_tree_id = ").
			append("(select ad_tree_orgtrx_id from PA_HIERARCHY where PA_HIERARCHY_ID = ").
			append(p_PA_Hierarchy_ID).append(")) and parent_id = ").append(p_AD_OrgTrx_ID).append(")");
		}
		//Fin Omar de la Rosa
		setDateAcct();
	...
	private void setDateAcct()
	{
		...
		if (p_DateAcct_From != null)
		{
			if (p_DateAcct_To == null)
				p_DateAcct_To = DB.getTimestampForOrg(Env.getCtx());
			return;
		}
	...
	private void createBalanceLine()
	{
		...
		sql.append(", " + DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ",").append(getAD_User_ID())
			.append("," + DB.TO_DATE(DB.getTimestampForOrg(Env.getCtx())) + ",").append(getAD_User_ID()).append(",");
		//	C_AcctSchema_ID, Account_ID, AccountValue, DateTrx, DateAcct, C_Period_ID,
	...
	private void createDetailLines()
	{
		...
		sql.append(" FROM Fact_Acct WHERE AD_Client_ID=").append(getAD_Client_ID())
			...
			.append(" AND DateAcct >= ").append(DB.TO_DATE(p_DateAcct_From, true));
			if (DB.isOracle()) {
				sql.append(" AND TRUNC(DateAcct, 'DD') <= ").append(DB.TO_DATE(p_DateAcct_To, true));
			} else if (DB.isPostgreSQL()) {
				sql.append(" AND DATE_TRUNC('day', DateAcct) <= ").append(DB.TO_DATE(p_DateAcct_To, true));
			}
		//
		int no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no == 0)
