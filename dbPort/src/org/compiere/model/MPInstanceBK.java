 ...
 */
public class MPInstanceBK extends X_AD_PInstance
{
	
	/** Static logger			*/
	private static CLogger s_log = CLogger.getCLogger(MPInstanceBK.class);
	 ...
	 */
	public MPInstanceBK (Properties ctx, int AD_PInstance_ID, String ignored)
	{
	 ...
	 *	@param ignored no transaction support
	 */
	public MPInstanceBK (Properties ctx, ResultSet rs, String ignored)
	{
		super(ctx, rs, null);
	}	//	MPInstanceBK
	 ...
	 */
	public MPInstanceBK (MProcess process, int Record_ID)
	{
		this (process.getCtx(), 0, null);
		setAD_Process_ID (process.getAD_Process_ID());//EXPERT:: Truena en esta linea cuando se hace la confirmacion
		setRecord_ID (Record_ID);
		setAD_User_ID(Env.getAD_User_ID(process.getCtx()));
		if (!save()){		//	need to save for parameters
			s_log.log(Level.SEVERE, " --save() 01 " );
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if(!pip.save()){
				s_log.log(Level.SEVERE, " --MPInstanceBK.pip.save() 01 " );
			}
		}
	}	//	MPInstanceBK
	 ...
	 *	@param Record_ID record
	 */
	public MPInstanceBK (Properties ctx, int AD_Process_ID, int Record_ID)
	{
		this(ctx, 0, null);
		s_log.log(Level.SEVERE, " --MPInstanceBK. 01 AD_Process_ID; " + AD_Process_ID);
		setAD_Process_ID (AD_Process_ID);
		setRecord_ID (Record_ID);
		setAD_User_ID(Env.getAD_User_ID(ctx));
		setIsProcessing (false);
		s_log.log(Level.SEVERE, " --MPInstanceBK. 01 FIN ");
	}	//	MPInstanceBK
	...
	public void setAD_Process_ID (int AD_Process_ID)
	{
		...
		if (AD_Role_ID != 0)
		{
			...
			if (access == null || !access.booleanValue()){
				s_log.log(Level.SEVERE, " Cannot access Process with role: ");
	...
	{
		StringBuffer sb = new StringBuffer ("MPInstanceBK[")
			.append (get_ID())
