...
public class MMessage extends X_AD_Message
{
	...
	/**
	 * Obtenemos un mmessage en el idioma con el que se ingreso
	 * @param ctx
	 * @param MessageId id de la tabla ad_message
	 * @param language
	 * @return el mensaje encontrado en su caso
	 */	
	public static String getMessage (Properties ctx, int MessageId, String language){
		String retValue = "";
		final StringBuilder sql = new StringBuilder(" SELECT coalesce(trl.msgtext, msg.msgtext) FROM AD_Message msg ");
		sql.append(" LEFT JOIN AD_Message_Trl trl ON (trl.ad_message_id = msg.ad_message_id AND trl.ad_language =? ) ");
		sql.append(" WHERE (msg.ad_message_id = ? or trl.ad_message_id = ? ) AND msg.isactive = 'Y'");
		PreparedStatement pstmt = null;
		try
		{
