...
public class TimeUtil
{
	...
	{
		if (time == 0)
			time = DB.getTimestampForOrg(Env.getCtx()).getTime();
	...
	{
		if (dayTime == null)
			return getDay(DB.getTimestampForOrg(Env.getCtx()).getTime());
	...
	{
		if (day == null)
			day = DB.getTimestampForOrg(Env.getCtx());
		GregorianCalendar cal = new GregorianCalendar(Language.getLoginLanguage().getLocale());
	...
	{
		if (dateTime == null)
			dateTime = DB.getTimestampForOrg(Env.getCtx());
		if (offset == 0)
			return dateTime;
		//
		GregorianCalendar cal = new GregorianCalendar();
	...
	{
		return isValid (validFrom, validTo, DB.getTimestampForOrg(Env.getCtx()));
	}	//	isValid
	...
	{
		if (dayTime == null)
			dayTime = DB.getTimestampForOrg(Env.getCtx());
		GregorianCalendar cal = new GregorianCalendar(Language.getLoginLanguage().getLocale());
	...
	/**
	 * Trunca una fecha y hora a solo la fecha a las 00:00
	 * 
	 * @param time
	 *            Milisegundos
	 * @return Fecha truncada
	 */
	public static Date truncateDay(long time) {
		GregorianCalendar cal = new GregorianCalendar();
		cal.setTimeInMillis(time);
		cal.set(Calendar.SECOND, 0);
		cal.set(Calendar.MILLISECOND, 0);
	...
	public static Date truncateDay(Date date) {
		return truncateDay(date.getTime());
	}

	/**
	 * Trunca una fecha y hora a solo la fecha a las 23:59
	 * 
	 * @param time
	 *            Milisegundos
	 * @return Fecha truncada
	 */
	public static Date truncateEndDay(long time) {
		GregorianCalendar cal = new GregorianCalendar();
		cal.setTimeInMillis(time);
	...
	public static Date truncateEndDay(Date date) {
		return truncateEndDay(date.getTime());
	}

	/**
	 * Obtiene la fecha de hoy con hora 00:00
	 * 
	 * @return Fecha de hoy
	 */
	public static Date truncateDay() {
		return truncateDay(System.currentTimeMillis());
	}

	/**
	 * Obtiene la fecha de hoy con hora 23:59
	 * 
	 * @return Fecha de hoy
	 */
	public static Date truncateEndDay() {
		return truncateEndDay(System.currentTimeMillis());
	}

	/**
	 * Convierte la hora local a la de la organizacion, </br>despues la trunca a las 00:00 y despues la convierte nuevamente a la hora local
	 * 
	 * @param ctx
	 ...
	 * @return
	 */
	public static Date getInitialRange(Properties ctx, Date date) {
		return DB.invertConvert(ctx, TimeUtil.truncateDay(DB.convert(ctx, date)));
	}
	 ...
	 * @return
	 */
	public static Timestamp getInitialRangeT(Properties ctx, Date date) {
		return DB.invertConvert(ctx, TimeUtil.truncateDay(DB.convert(ctx, date)));
	}

	/**
	 * Convierte la hora local a la de la organizacion, </br>despues la trunca a las 23:59 y despues la convierte nuevamente a la hora local
	 * 
	 * @param ctx
	 *            Contexto de aplicacion
	 * @param date
	 ...
	 * @return
	 */
	public static Date getFinalRange(Properties ctx, Date date) {
		return DB.invertConvert(ctx, TimeUtil.truncateEndDay(DB.convert(ctx, date)));
	}
	 ...
	 * @return
	 */
	public static Timestamp getFinalRangeT(Properties ctx, Date date) {
		return DB.invertConvert(ctx, TimeUtil.truncateEndDay(DB.convert(ctx, date)));
	}

	/**
	 * Convierte la hora local actual a la de la organizacion, </br>despues la trunca a las 00:00 y despues la convierte nuevamente a la hora local
	 * 
	 * @param ctx
	 *            Contexto de aplicacion
	 * @return
	 */
	public static Date getInitialRange(Properties ctx) {
		return getInitialRange(ctx, new Date());
	}

	/**
	 * Convierte la hora local actual a la de la organizacion, </br>despues la trunca a las 23:59 y despues la convierte nuevamente a la hora local
	 * 
	 * @param ctx
	 *            Contexto de aplicacion
	 * @return
	 */
	public static Date getFinalRange(Properties ctx) {
		return getFinalRange(ctx, new Date());
	}
	
	/**
	 * Ajusta los minutos de una fecha para coincidir con el minimo
	 * FIXME: metodo temporal creado por diferencia de versiones de joda-time*.jar entre dbPort (1.6 jboss) y eCareSoftWeb (2.1)
	 * @param date 
	 * @param min
	 * @return
	 */
	public static Date roundMinutes(final Date date, final int min) {
		if (date == null) {
			return null;
		} else if (min <= 0) {
			return date;
		}
		return roundMinutes(new DateTime(date), min);
	}
	 ...
	 * @return
	 */
	public static Date roundMinutes(final DateTime dateTime, final int min) {
		if (dateTime == null) {
			return null;
		} else if (min <= 0) {
			return dateTime.toDate();
		}
		final int diff = dateTime.getMinuteOfHour() % min;
		if (diff != 0) {// minimo
			return dateTime.minusMinutes(diff).toDate();
		} else {
			return dateTime.toDate();
		}
	}
	...
	public static boolean equals(final Date date1, final Date date2) {
		if (date1 == null && date2 == null) { // both null
			return true;
		} else if (date1 == null) { // one is null
			return false;
		} else {
			return date1.equals(date2); // none is null, then compare
		}
	}
	
	/**
	 * Valida si el rango de fechas está dentro de los días permitidos
	 * 
	 * @param days
	 *            Días máximos del rango
	 * @param date
	 *            Fecha inicial
	 * @param date2
	 *            Fecha final
	 * @return Si/No es válido
	 */
	public static boolean isValidRange(int days, Date date, Date date2) {
		...
		if (date != null && date2 != null) {
			...
			int value = Math.abs(Days.daysBetween(dateTime, dateTime2).getDays());
	...
	/**
	 * De {@link Timestamp} a {@link GregorianCalendar}
	 * 
	 * @param timestamp
	 *            Fecha
	 * @return Conversión
	...
	/**
	 * De {@link GregorianCalendar} a {@link XMLGregorianCalendar}
	 * 
	...
	/**
	 * De {@link Timestamp} a {@link XMLGregorianCalendar}
	 * 
	 * @param timestamp
	 *            Fecha
	 * @return Conversión
	...
	public static XMLGregorianCalendar toXMLGregorianCalendar(Timestamp timestamp) {
		return toXMLGregorianCalendar(toGregorianCalendar(timestamp));
	}
